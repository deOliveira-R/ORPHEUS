...
==== Provenance (recovered 2026-07-13) ====

This tracked copy was RESTORED on 2026-07-13 after the source was lost.
History: committed at this very path by ``b8f2159c`` (2026-04-22,
"research(cp): c_in-aware split basis + scale calibration frontier
(E1-E3.7)"), mass-deleted by ``15486f66`` (2026-04-30, "chore(derivations):
mass-delete shipped/falsified diagnostics"). The surviving working copy
lived on untracked in ``scratch/derivations/diagnostics/`` — the CP rank-n
protocol test's subprocess worker (``tests/cp/test_peierls_rank_n_protocol.py``)
imports ``run_scalar_f4`` from there via a hardcoded devcontainer path —
until it too vanished, leaving only its compiled
``__pycache__/diag_cin_aware_split_basis_keff.cpython-312.pyc``.

Recovery ran through the surviving CPython-3.12 bytecode: the pyc's embedded
``co_filename`` pointed back to this tracked path, where git history still
held the exact blob. Verification of bytecode-behavioral equivalence
(2026-07-13, CPython 3.12.13 / numpy 2.5.1 / scipy 1.18.0 / sympy 1.14.0,
runtime = the ``cd0e93f5`` 2026-04-22 scan-era orpheus tree):

1. ``compile()`` of this source (minus this provenance section) is
   marshal-BYTE-IDENTICAL to the surviving pyc payload (35 087 B; zero
   structural diffs across all 25 code objects; pyc header source size
   33 395 B == git blob size). Same floating-point program by construction.
2. ``run_scalar_f4`` bit-equal (rel Δk = 0.0), pyc vs source, at
   (τ, ρ) ∈ {(5, 0.3), (5, 0.5), (10, 0.5), (20, 0.5)} with r_0 = ρ·τ,
   R = τ, (Σ_t, Σ_s, νΣ_f) = (1, 1/3, 1), for both quadratures
   (n_panels, p_order, n_ang) = (2, 4, 16) and (3, 6, 32); e.g.
   τ=5, ρ=0.3, (2,4,16): k = 1.4969177868595174 on both sides.
3. Bit-equal likewise: ``run_split_basis`` (N=(1,1,1)), ``solve_k_eff``,
   ``compute_W_split_basis`` (τ=0 and τ=2), ``mu_crit``/``c_of_mu``/
   ``mu_of_c``/``chord_oi``, the three basis factories, and the stdout of
   both ``test_*`` self-checks — 26/26 checks, 0 mismatches.

IMPORT SURFACE — re-homed 2026-07-13 onto the post-``bda76faf``
(2026-05-01) derivations layout: the era imports
``orpheus.derivations.peierls_geometry`` / ``orpheus.derivations._kernels``
became ``orpheus.derivations.continuous.peierls_nystrom.geometry`` and
``orpheus.derivations.common.kernels`` (every consumed symbol survives
by name). The re-home is the ONLY change relative to the recovered
blob besides this docstring. The era→HEAD peierls refactors (Q1–Q6
quadrature-recipe migrations, the 18→2 angular-primitive collapse) were
verified k-compatible empirically: at HEAD (CPython 3.14.3 /
numpy 2.4.4) ``run_scalar_f4`` reproduces the era bit-table — all
8 points of {(5,0.3),(5,0.5),(10,0.5),(20,0.5)} × {(2,4,16),(3,6,32)}
— to **≤ 4.4e-15 relative** (FP-associativity level), five orders
below the 1e-6 tolerance of the pinned ``F4_REFERENCE_BASELINE``
acceptance in ``tests/cp/test_peierls_rank_n_protocol.py``. To
reproduce the 2026-04-22 scan-era runtime exactly instead::

    git archive cd0e93f5 orpheus | tar -x -C /tmp/era_repo
    PYTHONPATH=/tmp/era_repo python diag_cin_aware_split_basis_keff.py
"""
