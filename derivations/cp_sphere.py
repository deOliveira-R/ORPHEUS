...
def _kinf_from_cp(
    ...
    Builds the matrices A and B such that k_inf is the largest
    eigenvalue of A^{-1} B:

        A[i·ng+g, j·ng+g'] = δ_{ij} Σ_t V - P^T Σ_s V
        B[i·ng+g, j·ng+g'] = P^T χ ν Σ_f V
    """
        ...
        for g in range(ng):
            ...
            for j_reg in range(N_reg):
                for gp in range(ng):
                    ...
                    A_mat[row, col] -= (
                        Pji * V_arr[j_reg] * sig_s_mats[j_reg][gp, g]
                    )
                    B_mat[row, col] += (
                        Pji * V_arr[j_reg]
                        * chi_mats[j_reg][g]
