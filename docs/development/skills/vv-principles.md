...
8. **A gate that fires but cannot fail** — nine classes, each green on every input. (1) *compiled-out `assert`*: `python -O` strips a bare `assert` the pytest rewriter never touched — a collected test module is safe, a helper, a generator or production code is not; rewrite as `raise` / `np.testing.assert_*`. (2) *tautological companion guard*: a predicate that is always true (`assert a != b or abs(a - b) == 0.0` is `P or ¬P`); ask what input makes it fail. (3) *signature-tautological gate*: an invariance claim whose producer's signature never admits the knob; `inspect.signature` the producer chain first, and gate the signature itself. (4) *misattributed strict xfail*: an xfail hides ANY failure, including one before the documented assertion; run `--runxfail` and read every message. (5) *self-satisfied `pytest.raises`*: `with pytest.raises(E): raise err` where `err` was built in the test body touches no production code. (6) *skip-swallowed sentinel*: a broad `except` that converts any failure into a skip; run `-rs` and read the reasons — a reason containing an exception message is a dead gate. (7) *decayed `catches` marker*: a genuine catcher goes blind when its fixture or config drifts, without anyone touching it — a marker is a claim with a shelf life; re-drop the bug at review. (8) *published invocation never gated*: for every recipe a doc publishes as a command, one gate must consume the STRING, not the API. (9) *imperative `pytest.xfail()`*: raises at once, so the body never runs and the row reports `x` forever; grep `pytest\.xfail\(` and ask what event retires it. **check:** before believing a "gate is blind" verdict, verify the mutation actually BIT (installing `__post_init__` on a dataclass declared without one is a no-op), and verify the decoder (instrument-doctrine X2). **tell:** a tripwire that cannot trip — green under `-O`, never red; a skip reason that is an exception message; an `x` that never turns. [case](../evidence/test-design-modes.md#mode-8-compiled-out)
