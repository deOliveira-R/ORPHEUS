...
11. **Gate-never-executes-the-rewired-path** — the named twin is green AND its asserts fire, but its run never reaches the changed production line. **check:** sentinel-instrument that exact line (a file-write or counter — NOT a bare `assert`, NOT a print) and confirm the gate executes it before crediting it. A test that builds the input packet with a surrogate that RECOMPUTES the production formula pins the CONSUMER's threading and is blind to the PRODUCER — mutate the consumer, it reds; mutate the stamp, it stays green; a pytest-plugin wrap of the internal call is strictly stronger than a file-write probe. **tell:** a slow twin named as evidence for a NEW reader with zero non-docstring callers. [case](../evidence/test-design-modes.md#mode-11-never-executes)
