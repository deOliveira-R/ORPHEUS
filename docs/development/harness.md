...
|---|---|---|---|
| CLAUDE.md + `.claude/rules/*.md` + the auto-memory index — inherited by every dispatch that does not set `omitClaudeMd` | main agent and every Key agent | ≈64.6K tokens | budgeted in each page's `harness:` front matter; every always-on file is generated (`tests/test_harness_generated.py` fails on a hand-maintained rule), so the sum `python -m tools.harness --check` prints is the whole block but the memory index: six rule cores + CLAUDE.md ≈11.5K (`[M]` 2026-09-21 chars/3.6, after the re-evaluation), with the memory index ≈2.5K on top. Three rules are path-scoped and load when a matching file is touched, in the main agent and in a sub-agent alike: `coding-standards` ≈2.0K under `orpheus/**`, `tests/**`, `derivations/**`, `tools/**`; `plan-authoring` ≈7.8K under `.claude/plans/**`; `vv-testing` ≈1.0K under `tests/**` (ruled 2026-09-21 at the diff review, after a first ruling had kept the two larger ones always-on). The keep − omit probe reads the whole block, memory index included, at **13 872** tokens (`[M]` 2026-09-21, haiku first turn on the branch tip before the merge, session `5ba526f0`: keep 50 378 − omit 36 506; 13 850 on `f5f2cf2d` with the same scoping; 28 705 after K3b, 30 655 after P1, 70 966 before the restructure); a code session that also opens a plan pays ≈21.3K by the estimate
