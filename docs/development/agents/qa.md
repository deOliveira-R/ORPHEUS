...
The first pass is adversarial and unhedged: *how would I break this* (the input that makes it silently wrong, the reassuring direction first) and *how would I make it 100× better*. The second pass, written separately, re-evaluates: each attack survives or is withdrawn with the reason the design had. A second-pass verdict never bounds the first ([the brief](../workflows.md#the-brief)).
