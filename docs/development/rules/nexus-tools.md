...
instruments in `coding-standards` A.2 ([case](../evidence/nexus-tools.md#2026-04-restructuring-grep-misses)). The
`explorer` agent (Nexus skills preloaded) is the exploration delegate for
open-ended, multi-file work. Founding cases: [the evidence page](../evidence/nexus-tools.md).
...
`([^a-z_])Gram` return 1
([case](../evidence/nexus-tools.md#2026-08-26-the-ugrep-fixture)). This is
...
- check: use `\b…\b`, or `-P` with a lookbehind, or drop the anchor; never put
  ...
  such reads ([case](../evidence/nexus-tools.md#2026-09-17-git-grep-has-no-word-boundary)).
...
- **Deferred tools, and the escape hatch is main-agent-only.** If
  ...
  ([case](../evidence/nexus-tools.md#2026-08-19-a-sub-agent-has-no-toolsearch)),
...
- **The standalone `Grep` and `Glob` tools are gone**, so
  ...
  ([case](../evidence/nexus-tools.md#2026-06-14-the-removed-grep-and-glob-tools)).
- **Stale graph.** The trigger is a query returning an unexpected result: zero
  ...
  server auto-reloads.
- **Git worktrees** ([L22](../evidence/lessons.md#l22-worktree-not-main)): the
...
- **`session_briefing` warns when files the graph INDEXES have changed, not
  ...
  describing the checkout exactly while the branch name has moved on ([case](../evidence/nexus-tools.md#2026-08-16-the-briefings-silence)).
