...
`[M]` measured, with the command or file:line; `[R]` reasoned; `[HYPOTHESIS]` proposed; `[REFUTED YYYY-MM-DD]` with what refuted it; `[LANDED <hash>]`; `[REMEDIED YYYY-MM-DD @<hash>]` a recorded defect repaired. **A bare number with no marker will be read as measured.** The census clauses below are instances of X2 and cite it; the protocol that meets them, shape by shape, is the `instrument-doctrine` skill § X2.

- **[M]-SCOPE** `[M]` certifies a measurement HAPPENED, not that it answers its sentence. check: ask what command produced it; unstated means unmarked. tell: a "so"/"therefore" with the measured half on one side. [case](../evidence/plan-authoring.md#2026-08-10-solve-sn-therefore), [case](../evidence/plan-authoring.md#2026-09-20-omitclaudemd-memory-index)
