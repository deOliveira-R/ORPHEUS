...
**Living document.** The measure of this rule is the SURPRISE: a session, reading a plan in good faith, believed something the tree contradicts. The surprise log lives in [the evidence page](../evidence/plan-authoring.md#surprise-log). A new surprise is appended there as a `### <date> <two or three words>` entry, the plan is corrected in place per §3, and the clause here gains a link to the entry; a new clause only if the mechanism generalises past its campaign. A mechanism that keeps recurring is a signal for a tool, not for another paragraph (the [harness page](../harness.md)); a clause that has stopped catching anything merges or retires, or the file becomes a hot surface that costs more to read than the surprises cost to hit. Target, falsifiable, per campaign and counted at its close-out: `k catches over N surprises`. A catch is a plan error a clause of this rule flagged at review, before any session acted on it (recorded in the campaign's plan, with the clause); a surprise is one a session acted on (logged on the evidence page, each entry naming its clause and, since 2026-09-21, its campaign, so N is countable from the log). N falling means the rule works; N steady while k rises means the clauses catch at review and not at writing, a question of where the rule loads (its tier, the [harness page](../harness.md)) and never a reason for another paragraph; a clause no catch and no surprise has named across several campaigns merges or retires. `check:` is the mechanical check, `tell:` how the failure looks.
...
- **MEANS-IS-HYPOTHESIS** Label and date every means proposed before the investigation; an unlabelled means reads as a decision. [template](../evidence/plan-authoring.md#phase-entry-template)
...
- **PRECEDENT-LAYER** A two-layer precedent (data + binder, model + view, kernel + driver) cited by one of its layer names inverts the conclusion instead of degrading it. check: map the new objects onto BOTH layers and keep the mapping that preserves field count and verb count; in a data/binder split the data verbs return ARRAYS and only the binder returns OPERATORS, so the import direction refutes a mapping before any `isinstance` chain does. tell: a "thin data class" mapped onto the precedent's BINDER (`[M]` 2026-08, CS4b: a two-field class cited as the rich one while the six-verb class was the analogue; read at the right layer the precedent argued FOR the design the proposal opposed).
...
- **NAME-FREE-IS-NOT-NAME-GOOD** Before adopting a proposed NAME grep the PROSE corpus (`lessons.md`, `plans/`, `agent-memory/`, `gh issue list --search`), not only code. tell: a hit inside a refutation; free because rejected. [case](../evidence/plan-authoring.md#2026-08-26-sweep-cache-name)
- **NAME-CARRIES-A-VERB** check: grep the prose corpus for the proposed name AND the verb the object would own. tell: the wanted primitive already exists in MUTATING form on another type, or a charter has already ruled its REALIZATION and not only its name (`[M]` 2026-09, `Pencil.at(σ)`: the name was the campaign's and the verb already shipped as a mutating solver method, the strongest argument for the carve).
...
- **[M]-SCOPE** `[M]` certifies that a measurement happened; it does not certify that the measurement answers the sentence it sits in. check: name the command that produced it, and mark only the half of the sentence that command answers. tell: a "so" or a "therefore" with the measured half on one side of it. [case](../evidence/plan-authoring.md#2026-08-10-solve-sn-therefore), [case](../evidence/plan-authoring.md#2026-09-20-omitclaudemd-memory-index)
...
- **QUANTIFIER** Every universal ("every family", "none of the callers") carries its DENOMINATOR: `[M] 5 of 8 sites` (X2). The unmeasured members are where the defect survives. [case](../evidence/plan-authoring.md#2026-08-11-h2-every-family)
- **QUANTIFIER-AT-WRITE-UP** Run the check when PUBLISHING, not when measuring (X2). check: how many did I look at, and is that number in the sentence? [case](../evidence/plan-authoring.md#2026-08-19-brief-method-claim)
- **CONSTANT-DENOMINATOR** The denominator you are least likely to write is the one held FIXED across every row (X2). [case](../evidence/plan-authoring.md#2026-08-15-constant-denominator-parity)
- **PREDICATE** A fraction needs its FILTER: write what the numerator COUNTS (X2). `10 of 56 methods take a project_root arg` is re-runnable; `56 methods / 10` is not. [case](../evidence/plan-authoring.md#2026-08-16-fraction-needs-filter)
- **VALIDATE-THE-FILTER** A zero from a filter is evidence only after the filter has found a known member of every shape expected (X1). tell: a dropped member looks identical to an absent one, and so does ugrep's silent zero (`code-search`). A census keyed to one member of a FAMILY misses the family: when a CONTAINER moves, census the container, anchored on its SEPARATOR (`grep -rnE "_nexus/|graph\.(db|json|html)"`), because the bare container name returns everything that shares it (`_nexus` matched every `mcp__nexus__*` tool name, 559 KB; [L55](../evidence/lessons.md#l55-census-by-spelling)). [case](../evidence/plan-authoring.md#2026-08-19-filter-case-class)
- **VIEWPORT** Never `head` an enumeration that feeds a decision, window structured source with `sed -n`, or regex a nested literal. check: MEMBERSHIP questions are parsed (AST), not grepped (X2); read whole, or state `wc -l`. tell: two filters disagree. [case](../evidence/plan-authoring.md#2026-08-25-viewport-head-clip), [case](../evidence/plan-authoring.md#2026-08-27-two-filters-disagree)
- **DENOMINATOR-THAT-IS-A-GATE** A guard named as the denominator bounds the arm it is CALLED on, not the question (X2). tell: a plan row summarised from a code comment, carrying that comment's scope while dropping the sentence that limited it. [case](../evidence/plan-authoring.md#2026-08-26-carrying-quadrature-gate)
- **STRUCTURAL-DENOMINATOR** For "the rebuild loses X" or "these are duplicates", enumerate against the TYPE, never against the concept you are chasing (X2). [case](../evidence/plan-authoring.md#2026-09-01-field-list-denominator)
...
- **DEFINITION-FILE-ONLY** The inverse exclusion: a self-check whose whole population is the defining module certifies "the def plus one call" (X2). [case](../evidence/plan-authoring.md#2026-09-05-definition-file-census)
- **RESUME-BLOCK-M** An `[M]` count in a compaction point is a claim with a predicate (X2): `[M] 6 direct call sites in <file>` was a one-file census. tell: the neighbouring `[R] check first` claims were checked for free; the `[M]` lied. [case](../evidence/plan-authoring.md#2026-09-05-compaction-point-exclusion)
- **A-LIST-IS-N-CENSUSES** Counts listed in one breath imply a shared scope that never existed; each owes its predicate AND its tree (X2). tell: one number answers a neighbouring predicate the row never names. [case](../evidence/plan-authoring.md#2026-08-31-list-of-counts)
- **CARVE-FORKS-THE-DENOMINATOR** When N classes share one body, "N surfaces" is two counts: BODIES (what a mutation battery covers) and ROLE x surface rows (what a decision is taken on) (X2). [case](../evidence/plan-authoring.md#2026-09-04-body-role-counts)
- **RENAME-SIZE** A rename row names its predicate: "9 occurrences of the IDENTIFIER; concept not yet counted" (X2). Renaming only the identifier leaves the concept spelled two ways. [case](../evidence/plan-authoring.md#2026-08-26-gram-rename-size)
- **TABLE-ROWS-ARE-CLAIMS** (X3) A table headed by a property of the tree reads wholly as a survey of what IS. Mark an aspirational row IN the row (`[HYPOTHESIS] aspirational; [M] the tree has X`), never in prose. [case](../evidence/plan-authoring.md#2026-08-07-periodic-table-row)
...
- **COLUMN-HEADER-IS-A-CLAIM** (X3) Structural vocabulary ("off-diag", "banded") asserts structure as surely as a number asserts a value, and can be wrong while every cell is right. [case](../evidence/plan-authoring.md#2026-08-29-column-header-claim)
...
- **A-BRIEF'S-METHOD-IS-A-CLAIM** (X3) A prescribed procedure asserts that its surface EXISTS on the recipient's corpus. check: state the sample it was validated on and what to do when it fails; ask whether sample and population share any member. [case](../evidence/plan-authoring.md#2026-08-19-brief-method-claim)
...
- **AXIS-CHOICE** A universal's axis (per arm, per member of a union, per branch of a dispatch, per consumer, per call site, per row of a parametrize, per family of the corpus, per sub-family of a tolerance) is a design choice prior to, and independent of, stating the denominator on the chosen axis. check: before publishing a `k of N`, ask whether the corpus is uniform IN THE DISCRIMINATING FIELD along the axis chosen; a corpus uniform there leaves an arm witness-less however precisely `N` is counted, and a single non-uniform member can be what makes the invariant spellable. tell: a well-formed `k of N` whose `N` was the easiest axis to enumerate, not the one the claim is about (`[M]` 2026-09-03: \"14 of 14 isotopes\" counted per isotope where the claim was per channel, and 2 of 13 isotopes carry an empty (n,2n) channel; test-architect's archive, the #426 section).
...
- **STATE-THE-CENSUS-PREDICATE** "Complete" states its METHOD ("complete for literal-name calls", X2); a census-exact count over visible spellings otherwise reads as the population. [case](../evidence/plan-authoring.md#2026-08-29-census-predicate-spellings)
- **MEMBERS SPELLED WITHOUT THE SYMBOL** are enumerated by the `retirement-audit` skill's census (numbered (1)–(13) with (12b), then five named spellings; each found by a red loop or by reading), never by a symbol grep.

### §6c A step that adds a gate must land with the case it catches

X1, asked at design time: **what input, existing in the tree the moment this lands, does this gate reject?** "None; it arrives in a later step" means the two steps are one. [case](../evidence/plan-authoring.md#2026-08-14-q6-reference-axis)

- **NOT-A-MUTATION-QUESTION** Mutating the SUT reddens the gate and CONFIRMS teeth (X1's control), because the mutation manufactures the witness the registry lacks. The question is about the SHIPPED INPUTS.
...
- **INERT-WITNESS** (X1) A gate's fixture must ACTIVATE the quantity the gate varies: a negative leg comparing a signed pairing against a squared norm cannot fail on any input. check: ask what the varied quantity READS on this input. [case](../evidence/plan-authoring.md#2026-09-05-inert-witness-gate)
- **MIRROR, A FIX WITH NO WITNESS** A repair that makes a defect unspellable is the strongest kind and so feels to need no test. check: X1's repair-with-no-gate procedure (mutate it; re-measure the repaired module's own suite separately). A plan's own prophecy is not a witness. [case](../evidence/plan-authoring.md#2026-09-01-fix-without-witness)
- **GATE-IS-SPELLABLE** check: take every symbol on a chartered law's RHS and check it against the datum's method list before understanding the mathematics; for an `⟺`, check the two directions separately and ask what the shipped witness can distinguish. tell: a gate whose negative control the shipped witness passes (a true-but-vacuous `⟸` under an undeclared convention), or a law whose RHS names a morphism the datum does not have (`[M]` 2026-08, CS4a: the adjoint law is necessary and sufficient for Galerkin, tightness for multiplicativity, and the ℓ=0 witness satisfied both; CS4b: no kernel carried any of `T`, `transpose`, `adjoint`, `dagger`).

### §6d A re-home must check the import edge it creates in both directions

(1) Does the MOVER depend on its old home? (2) Does the OLD HOME depend on the MOVER? Enumerate the mover's **callers** and ask which package each lives in. A done-when naming one direction of a symmetric relation reads complete and is half a check. §6b asks WHEN a call site is updated; this section asks WHERE the caller lives — §6b's own check answers "no step boundary is crossed" and misses the edge. [case](../evidence/plan-authoring.md#2026-08-26-rehome-import-cycle)
...
- **VOCABULARY-ECHO** (X3) A row written in a rule's own words ("no new edge either way") reads as that rule having been APPLIED. check: cite the rule's MEASUREMENT (`[M] geometry -> transport = 0 -> 1, forbidden`), not its phrasing. tell: a rule's phrase with no `[M]` beside it. [case](../evidence/plan-authoring.md#2026-08-27-vocabulary-echo-row)
...
- **EXISTENCE-IS-NOT-RELEVANCE** §8's grep is a WIRING check and cannot tell load-bearing from inert, so it yields a confident hazard with zero traffic. check: price every branch with a POSITIVE CONTROL stronger than your change (X1); if it reddens nothing the honest deliverable is a WITNESS, not a warning. [case](../evidence/plan-authoring.md#2026-08-31-wiring-inert-branch)
- **DUPLICATES-IS-A-UNIVERSAL** A bit-identity claim owes its denominator (X2) and is routinely asserted without ever being run (`coord` 3/3, `face_areas` 2/3). [case](../evidence/plan-authoring.md#2026-08-27-duplicates-universal-claim)
...
- **PROXY THE WORK REMOVES** A metric that stands in for the quantity (a proxy) holds today, so nobody notices it is a proxy until the work removes what it keyed on. check: read what the metric KEYS ON; confirm that field is the one the work changes. [case](../evidence/plan-authoring.md#2026-08-17-nexus-82-f5)
...
- **ACCEPTANCE ARTIFACTS** X1's inverse question: if the campaign fully FAILED, would this artifact move? Activation, not name, qualifies a canary; count the calls into the carved path. [case](../evidence/plan-authoring.md#2026-08-28-canary-acceptance-artifact)
