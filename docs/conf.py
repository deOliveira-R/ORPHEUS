...
extensions = [
    ...
    'myst_parser',                          # docs/development/**.md — the harness-independent source (plan harness_context_budget K7)
]
...
# `docs/development/` is authored in MyST Markdown: it is the SOURCE of
# the `.claude/` rule and skill cores (generated by
# `tools/docs/generate_harness.py`), and Markdown nests inline markup
# that RST cannot (#422). Heading anchors are the link targets the
# cores use (`file.md#heading-slug`); a broken one fails `-W`.
source_suffix = {'.rst': 'restructuredtext', '.md': 'markdown'}
myst_enable_extensions = ['colon_fence']
# One definition of the anchor-level cap, owned by the generator because it must check
# links against it and cannot import conf.py without executing the whole Sphinx config.
from tools.docs.generate_harness import HEADING_ANCHOR_LEVELS
...
_GENERATORS = [
    ...
    ("tools.verification.generate_error_index", "error-catalogue index",
     "build-finished"),
    # The `.claude/` harness view — rule cores, skill cores, the lessons
    # index, agent role blocks — GENERATED from docs/development/ (the
    # harness-independent source; plan harness_context_budget.md K7).
    # Nothing in the build includes the outputs, so the event is free.
    ("tools.docs.generate_harness", "harness view", "builder-inited"),
]
