...
.. admonition:: Key Facts (the point-set layer)
   ...
   - ⛔ **2.3 and 3.1 are ENABLERS: they repair nothing, and 3.1's two
     ...
     **zero** outside their own module — `[M]` ten occurrences of the
     ...
     ``tests/gates/numerics/test_manifold.py``. ✅ ``quotient_map`` acquired
...
(``tests/gates/numerics/test_basis_domain.py::test_d6``).
...
``tests/gates/data/test_energy_grid.py``) from the multigroup *index* axis
...
hand-written ``new_space=`` in ``tests/gates/numerics/test_measure.py``) and
...
``tests/gates/numerics/test_manifold.py::TestTypeLaws::test_the_names_reproduce_the_retired_string_tags``.
...
.. (vv-status rationale) manifold-procesi-schwarz is a
   ...
   tests/gates/numerics/test_manifold.py::TestQuotient
...
.. note::
   ...
   **prefix, and that is a historical artefact rather than a claim.**
   ...
   — `[M]` ``tests/gates/numerics/test_slab_orbit_space.py:258`` carries
...
.. (vv-status note) manifold-s2-mod-so2 is the INSTANCE of
   ...
   tests/gates/numerics/test_manifold.py::TestQuotient::{test_the_procesi_schwarz_matrix_matches_the_hand_derivation,
   ...
   (a) [M] tests/gates/numerics/test_slab_orbit_space.py:258,
   ...
   algebra-of-record SymPy-identity shape (tests/gates/derivations/*_symbolic.py
...
.. (vv-status rationale) manifold-axial-stabiliser is a group-theoretic
   ...
   tests/gates/numerics/test_symmetry.py + tests/gates/numerics/test_manifold.py do
...
.. (vv-status rationale) manifold-axial-invariant-rings is an IDENTITY
   ...
   @pytest.mark.foundation in tests/gates/numerics/test_manifold.py.
...
.. (vv-status rationale) manifold-group-as-component-and-cosets and
   ...
   ``tests/gates/numerics/test_symmetry.py``, which carry
...
.. (vv-status rationale) manifold-orbit-dimension-law is a STRUCTURAL
   ...
   tests/gates/numerics/test_manifold.py::TestR4AnOrbitSpacesDimensionIsATheorem,
...
.. (vv-status rationale) manifold-s2-mod-mirror is the second INSTANCE
   ...
   literal, by tests/gates/numerics/test_manifold.py::
...
.. code-block:: python
   ...
   # tests/gates/numerics/test_manifold.py
   #   ::TestTheSigmaYFoldIsExpressibleAndDiscriminating
...
The pin is
``tests/gates/numerics/test_manifold.py::TestQuotient::test_the_derived_orbit_space_agrees_with_the_hand_written_table``,
...
map by hand in
``tests/gates/numerics/test_manifold.py::test_archimedes_about_z_is_the_labelled_equation_verbatim``.
...
.. (vv-status rationale) manifold-reynolds-projector is a
   ...
   tests/gates/numerics/test_manifold.py::TestR4TheCoordinateChartPairIsTheReynoldsProjector
...
.. (vv-status rationale) manifold-map-functoriality: A structural law
   ...
   ``foundation`` test
   ``tests/gates/numerics/test_manifold.py::TestManifoldMap::test_functoriality_the_pushforward_of_a_composite_is_the_composite_of_pushforwards``
...
over ``tests/gates/sn/solve/test_pl_order_does_not_move_the_infinite_medium_flux.py``
...
.. warning::
   ...
   in ``tests/gates/numerics/test_manifold.py``, where `[M]` ``quotient_map``
...
.. (vv-status rationale) manifold-quotient-pushforward: A derivation
   ...
   is gated by the ``foundation`` module ``tests/gates/numerics/test_manifold.py``
   ...
   with ``tests/gates/numerics/test_registry.py`` (the registry's answer read
...
**The gates.** Section E of ``tests/gates/numerics/test_basis_domain.py``,
...
.. (vv-status rationale) manifold-gamma-slot-stage-zero: A statement of
   ...
   ``tests/gates/numerics/test_registry.py`` —
...
``tests/gates/test_layer_imports.py``'s fresh-interpreter gate, which R2
...
The gates live in ``tests/gates/numerics/test_manifold.py``: `[M]` 2026-09-02,
...
``tests/gates/numerics/test_legendre_basis.py`` (**34** rows) and
``tests/gates/numerics/test_descent.py`` (**20** rows) — the intrinsic laws of
...
``tests/gates/numerics/test_manifold.py`` collects **240** rows, of which
...
.. note::
   ...
   **A second module carries the CONSUMER-side gates**, and it is where
   ...
   ``tests/gates/numerics/test_basis_domain.py``, `[M]` **24** collected rows
