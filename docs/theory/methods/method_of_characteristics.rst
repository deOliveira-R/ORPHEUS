...
in ``orpheus/derivations/discrete/moc/equations.py``.
...
.. admonition:: ERR-019 — Missing weight factor (caught during development)
   ...
   **Lesson:** Always verify the transport sweep with a heterogeneous
   ...
   See ``docs/theory/verification/error_catalog.rst`` ERR-019.
...
The verification evidence for the MOC solver — the 102-test
four-level suite, the homogeneous/heterogeneous eigenvalue cases,
the ray/azimuthal/polar convergence studies, and the per-segment
MMS gate on the flat-source spatial operator — lives in the
verification part:
:doc:`/theory/verification/method_of_characteristics`.  The ERR-019
investigation history below stays with this chapter's derivations.
The auto-generated :doc:`/theory/verification/matrix` reports
