...
The 1-D slab balance is derived in :doc:`slab_one_group`; the 2-D
Cartesian balance in :doc:`cartesian_multid`.
...
Cartesian wavefront and its octant dependency graph in
:doc:`cartesian_multid`. The sections below build the
multi-dimensional LD system and the curvilinear machinery.
...
The k-eigenvalue outer loop (:class:`~orpheus.numerics.iteration.KEigenvalue`, power
iteration), the operand requirements, the inverse-realisation
choice, and the cross-solver consumer map are documented in
:doc:`slab_multigroup`; this section retains the multi-D boundary
Gauss-Seidel schedule.
