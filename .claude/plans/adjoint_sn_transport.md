...
>
> Verification spec (test-architect, the gate chain): `.claude/plans/p6_adjoint_verification_spec.md`.
...
> via the **#276 K_iso reframe** — durable plan `.claude/plans/kiso_cross_model_extraction.md`. The
...
> + `.H` CAP_SOLVE) — phase spec ~line 130; verification `.claude/plans/a3_solve_transpose_verification.md`
...
- **A0 — verification spec (DONE).** test-architect gate chain at `.claude/plans/p6_adjoint_verification_spec.md`.
...
- **A2 — `S†` via iso-path modernization (CLEAN-BEFORE-EXTEND; user-steered 2026-06-28).** The aniso
  ...
  - **A2a (forward cleanup):** route the full Legendre scattering (P0 + aniso) through
    ...
    `.claude/plans/a2_iso_modernization_verification.md`): the **0-ULP aniso canary STAYS 0-ULP**
