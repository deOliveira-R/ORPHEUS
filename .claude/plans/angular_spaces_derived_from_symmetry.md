...
### 2.2b EXECUTED — the Γ-slot: a geometry admits a FOLD of its domain, and asks the owed symmetry ON the orbit space (2026-09-02, landed `a7c8de6d`)
...
| 2.2b | ⭐ **NEW 2026-09-02 — the Γ-slot, SPLIT off 2.2** (user-ruled): stage 0 admits a rule on `S²/H` for `H ⊆ Γ` (lattice, derived); stage 1 asks Γ-invariance THROUGH the entry's `quotient_map`. `[M]` today `cylinder.admits_domain(folded)` False AND `admits_symmetry` False; `select_quadrature` 0 production callers | ⏏ yes | ✅ **LANDED 2026-09-02 `a7c8de6d`** — stage 0 through the descent arrow + what it SPENDS; the owed symmetry asked ON the orbit space through `Quotient.induced_action` (one closure for `is_invariant`, `orbit_certificate` and `ordinate_permutation`); read the *2.2b EXECUTED* section |
...
✅ **2.2b (the Γ-slot) LANDED 2026-09-02 as `a7c8de6d`** — the commit titled `feat(numerics): a
...
| ✅ | ⭐ **2.2b** | LANDED 2026-09-02 `a7c8de6d` — the Γ-slot: a geometry admits a FOLD of its domain (stage 0 through the descent arrow), and the owed symmetry is asked ON the orbit space (`Quotient.induced_action`, the normaliser, one closure); read the *2.2b EXECUTED* section |
...
| **3** | **2.2b** — the Γ-slot | ✅ LANDED 2026-09-02 `a7c8de6d`, its own commit |
