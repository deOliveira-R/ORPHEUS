# Brief — memory distillation, owner pass: test-architect

**Workflow:** memory distillation (the harness campaign, `.claude/plans/harness_context_budget.md`, ⏸ COMPACTION POINT #11). **Phase:** PROPOSE. You are the OWNER of `.claude/agent-memory/test-architect/`; you propose, the orchestrator evaluates and applies, one commit per owner. **You edit nothing under `.claude/agent-memory/`, `.claude/agents/`, `.claude/rules/` or `.claude/skills/`: every output is a NEW file under `scratch/_memory_distillation/test-architect/`.**
...
`archivist/lessons.md` had reached 3820 lines / 254 KB, `test-architect` 2808 /
...
- `lessons.md` 3 514 lines (the hot digest, "read whole every dispatch"); `lessons_archive.md` 10 906 lines (cold, sections L1–L86); `MEMORY.md` 109 lines (loaded whole every dispatch; §2 is 45 campaign lines).
- References from `docs/`, `.claude/plans/` into `.claude/agent-memory/test-architect/lessons.md`: 0. Into the agent's topic files: 247 lines. So a line inside `lessons.md` retires with no external blast radius; a topic FILE does not retire in this pass (see scope).
- A default `Read` returns 2 000 lines: read `lessons.md` in pages (`offset`/`limit`) until you have seen line 3 514, or the newest entries are the ones you never read.
...
You hold, every dispatch, the skills `vv-principles`, `instrument-doctrine`, `retirement-audit`, `nexus-verification`, `nexus-impact`, `numerical-bug-signatures`, `coding-elegance`, `algebra-of-record`, and the always-on rules `cardinal`, `articulation`, `instrument-doctrine`, `process-discipline`, `workflows`, `code-search`, `nexus-tools`; touching `tests/**` loads `vv-testing` and `coding-standards`, touching `.claude/plans/**` loads `plan-authoring`. A digest entry that restates a check, a tell, an anti-pattern, a mode or a clause of any of these is a duplicate: it retires, and at most one line remains that cites the clause (`vv-principles` Mode 8 (3); `instrument-doctrine` X1; `process-discipline` "Mutation-testing an uncommitted file"). Adjudicate by READING both sides: open the rule file (`.claude/rules/<name>.md`) or the skill (`.claude/skills/<name>/SKILL.md`) and name the heading or item number in the table; a verdict from memory of the rule is not a verdict. A founding case the rule does not carry is not a duplicate: it is either a lesson that stays (a specific failure → correction) or an uplift candidate (law 3: the rule gains the check or the tell that would have caught it; you propose the clause, the orchestrator edits the rule's source).
...
Is any lesson identity-level, applied on essentially every task, core to HOW you work (a definition-of-done bar, a standing review discipline)? Propose it as a standing directive sentence for `AGENT.md` in `agent_md.md`, no war story, no codenames, and leave the specific instance in the digest with a `→ now in AGENT.md` pointer. Be conservative: a thin `AGENT.md` of sharp identity principles beats a bloated one. Read the current `AGENT.md` first: "THE SPINE" and §0.5/§0.6/§1.5 may already carry what you would promote.
