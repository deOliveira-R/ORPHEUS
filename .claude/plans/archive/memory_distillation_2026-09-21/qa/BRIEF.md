# Brief — memory distillation, owner pass: qa

**Workflow:** memory distillation (the harness campaign, `.claude/plans/harness_context_budget.md`, ⏸ COMPACTION POINT #11). **Phase:** PROPOSE. You are the OWNER of `.claude/agent-memory/qa/`; you propose, the orchestrator evaluates and applies, one commit per owner. **You edit nothing under `.claude/agent-memory/`, `.claude/agents/`, `.claude/rules/` or `.claude/skills/`: every output is a NEW file under `scratch/_memory_distillation/qa/`.**
...
`archivist/lessons.md` had reached 3820 lines / 254 KB, `qa` 2808 /
...
- `lessons.md` 1 076 lines (the hot digest, read every dispatch); `lessons_archive.md` 5 936 lines (cold, `L-001`..`L-072`); `MEMORY.md` 154 lines (loaded whole every dispatch); 7 topic files.
- References from `docs/`, `.claude/plans/` into `.claude/agent-memory/qa/lessons.md`: 4. The 4 reference lines into `qa/lessons.md` are prose mentions in `.claude/plans/harness_context_budget.md` (the campaign that distilled qa once, 2026-09), not anchors: a rewrite breaks nothing. qa's digest was distilled once already in that campaign; this pass applies laws 2 and 3 to what remains, and the same per-line test.
