# Brief — memory distillation, owner pass: cross-domain-attacker

**Workflow:** memory distillation (the harness campaign, `.claude/plans/harness_context_budget.md`, ⏸ COMPACTION POINT #11). **Phase:** PROPOSE. You are the OWNER of `.claude/agent-memory/cross-domain-attacker/`; you propose, the orchestrator evaluates and applies, one commit per owner. **You edit nothing under `.claude/agent-memory/`, `.claude/agents/`, `.claude/rules/` or `.claude/skills/`: every output is a NEW file under `scratch/_memory_distillation/cross-domain-attacker/`.**
...
`archivist/lessons.md` had reached 3820 lines / 254 KB, `cross-domain-attacker` 2808 /
...
- `lessons.md` 1 282 lines (the hot digest, read every dispatch); no archive file exists; a war story moving cold goes to a NEW `lessons_archive.md` that you propose whole as `archive_additions.md`; `MEMORY.md` 100 lines (loaded whole every dispatch); 58 topic files (frame memos).
- References from `docs/`, `.claude/plans/` into `.claude/agent-memory/cross-domain-attacker/lessons.md`: 0. 857 reference lines from `docs/` and `.claude/plans/` into cross-domain-attacker topic files: they are graph nodes and stay; the table may mark one as archaeology but nothing retires here.
...
You load no project rule and no project memory index: every dispatch you hold your `AGENT.md`, your own memory, the skills `cross-domain-frames`, `nexus-exploring`, and the rule sentences a brief pastes under "Rules that apply to you" (the generated list, one line per rule that declares a `brief:`):
