...
- **B4 — `LinearOperator[V]` generic → an ARCHITECTURAL CARVE (DETOUR), evolved into the TYPED-CARRIER GRID carve.** Not a typing patch. Progress (2026-06-22):
  ...
  - **ACTIVE plan: `.claude/plans/archive/typed_carrier_grid_carve.md`** (approved, cold-pickup §0). Operators speak typed `Field` carriers by completing the (angular⊗moment)×(flux⊗source) grid; the domain/codomain two-param split lands here (Phase A) with a real consumer (non-endomorphic `HarmonicMomentProjection`/`Reconstruction`). SUPERSEDES Phase 1 of `operator_inverse_algebra_carve.md`.
...
## Relations
#226 (the pyright-signal effort), the prior `.claude/plans/archive/pyright_cluster_triage.md` (superseded counts: 691→502 orpheus, undefined-var 67→11, missing-imports 0), `reference_pyright_lsp_rooting.md`, `issue_226_operator_generics_map.md`. #258 (units.py pint-stub debt) + #254/#255 feed here.
