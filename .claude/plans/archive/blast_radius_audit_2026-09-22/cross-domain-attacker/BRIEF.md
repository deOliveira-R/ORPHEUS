# Brief — topic-file blast-radius audit, owner: cross-domain-attacker

**Workflow:** the harness campaign's close-out (`.claude/plans/harness_context_budget.md`, "Memory distillation — CLOSE-OUT (2026-09-21)", owed item (1)). **Phase:** AUDIT — read and propose. You are the OWNER of `.claude/agent-memory/cross-domain-attacker/`; you propose, the orchestrator evaluates and applies, one commit per owner. **This dispatch is READ-ONLY on tracked files:** you write exactly one file, `scratch/_blast_radius/cross-domain-attacker/proposal.md` (untracked), and edit nothing else.

## Artefacts from the previous phase

- Your distillation table, whose "topic files judged archaeology" section named these candidates BY NAME without reading them: `.claude/plans/archive/memory_distillation_2026-09-21/cross-domain-attacker/table.md`.
- The candidates (1 files):
- `.claude/agent-memory/cross-domain-attacker/elegance_smell_rank_non_monotone.md`
- Their inbound references, measured 2026-09-22 by the orchestrator: `scratch/_blast_radius/cross-domain-attacker/referrers.md` (a claim; re-run any line you build on).
- Your live digest and index: `.claude/agent-memory/cross-domain-attacker/lessons.md`, `.claude/agent-memory/cross-domain-attacker/MEMORY.md` (and `lessons_archive.md` / `_archive/` where you have one).
