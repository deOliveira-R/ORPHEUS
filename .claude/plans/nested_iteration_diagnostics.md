...
| F4 | ✅ **REMEDIED 2026-08-09 by N3 (see `:223`) — past tense as of that date.** Both shipped defaults **WERE SHORT** at d=3 zero-leakage — incl. with scattering: need@1e-8 = 830 (c=0.5) vs `have 200`; need@1e-12 = 1441 vs `have 1000`. ⛔ `[M]` 2026-08-13 @ `0f5ca91c`: `grep -rn "max_inner" orpheus/` returns **no `200`/`1000` literal on any SN path**; every entry ships `max_inner: int \| None = None` → 1308/1961. **Do NOT quote this row as a live defect** — #350's option 3 did, and had no target. | same |
| F5 | ✅ **REMEDIED 2026-08-09 by N3 (see `:223`) — past tense as of that date.** The tolerance factor between the two default families should be **~1.5-1.75x** by F3; the tree **SHIPPED 5x**. ⛔ `[M]` 2026-08-13: the shipped ratio is now **1308 : 1961 = 1.4992** against F3's theoretical 1.500. The 5× is gone. | same |
