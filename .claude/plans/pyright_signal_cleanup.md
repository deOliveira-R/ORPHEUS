...
### Resume here (next)
B3 SN-production Optional-narrowing is largely cleared (the StreamingTerms win took diamond+cell_balance; only ~6 reportOptional* left in `orpheus/sn`). Remaining `orpheus/sn` is now **reportArgumentType (32) / reportAttributeAccessIssue (14) / reportReturnType (8)** dominated — heterogeneous, NOT simple narrowing. Top files: geometry.py (12), scattering.py (13), solver.py (11), boundary_realizer.py (9), sweep_graph.py (6). Still skip `pole_angular_closure.py` + loss_representation closure-access (#236-deferred).
- Pick coherent single-root-cause clusters next (e.g. the `BC.vacuum`/`OperatorSum.solve`/`AngularFlux.bulk` attribute-access family flooding tests is likely one missing-member root cause). Then **B4** (`LinearOperator[V]` generic — FunctionSpace/NDArray mismatch), **B5** (union dispatch), **B6**+**Workstream C** (derivations), **Workstream A** (user-gated hook).
- **Corrected paths** (last session's note had them wrong): ratchet test is `tests/test_pyright_ratchet.py` (NOT `tests/_harness/`); `test_reduced_operator.py` is under `tests/geometry/` (NOT `tests/sn/operators/`).
