...
- **B4 — `LinearOperator[V]` generic (SURGICAL, user-steered per `delegation.md`)**: boundary_realizer.py (8: `TensorProductOperator`/`IncomingSourceOperator` → `LinearOperator[Unknown]`), iteration.py (`LinearOperator[V].solve` ×3), solver.py (`object`→`LinearOperator[V]`). Needs test-architect proactive dispatch + the design map (`.claude/agent-memory/explorer/issue_226_operator_generics_map.md`). Do NOT batch-autonomously. **This is the single biggest remaining structural cluster — recommend it as the next focused, user-steered session.**
