...
> The plan is committed (`99f108f`); implementation is UNDERWAY through **S8b (`08cb9cf`)** — see the
> ⭐ CURRENT STATUS block immediately below (RESUME AT S8c — the Scattering/Fission fibration resolve).
...
- **NEXT = S8c (the LAST S8 sub-stage — Scattering/Fission fibration resolve).** `ScatteringOperator` (`scattering.py:~286/1208`) + `FissionOperator` (`fission.py:~389`) dispatch `apply` on the carrier TYPE via `@singledispatchmethod` over {FullField/TimedFullField, ScalarFlux, AngularFlux, ndarray} — so the single-`V` `LinearOperatorMixin` typing is a partial lie for these two (the fibration finding, DESIGN REVISION block). Resolve = make the multi-carrier typing HONEST. ⚠ **LEAST-specified sub-stage** — at pickup FIRST scope what "resolve" concretely means (a typing-only honesty fix — e.g. an `@overload`/Protocol-typed `apply` — vs a dispatch restructure); it may be a small typing change OR warrant a brief design pass before implementing. Gate **C6**: dispatch parity (each `@apply.register` arm returns the right type+value after re-typing; `test_operators_apply_typed.py` extends for FullField input; the iso-P0/n2n/aniso value paths byte-identical). Likely additive/bit-identical (typing honesty, not a value change). After S8c → **S8 COMPLETE** (then S9 BoundaryMomentField, test-architect FIRST; S10 χ-source DEFERRED; S11 Field→L2 UNDER REVIEW).
