...
**STATUS: LIVING — original round CLOSED 2026-08-30 (§1–§12); step-3 round §14; step-4 round §16 (2026-08-30, EXECUTING). Live resume surface: **§27.8 COMPACTION POINT #11** (the consumers campaign — step 2 LANDED 2026-09-14; step 3 next). Earlier points §13/§15/§17/§18/§20–§25 are history. No deferred forks remain open except the §16.3 factory probe (ruled separately).**
...
### 27.1 R-cc2 DEVELOPED — the recommended articulation (cross-domain attack round 2, 2026-09-08, `scratch/_consumers/attacker_terminal_object.md`; PROPOSED, for the user's ruling — the tree claims below re-verified by the main agent 2026-09-12)

**The articulation.** A Problem maps its generating data to a TERMINAL OBJECT
`Terminal(p) = (V, G, K, 𝒜 : Λ → Hom(V,V), q : Λ → V)` — the space, its metric, its cone, a FAMILY of operators over a parameter domain Λ, and a source family. **Problem-side** iff a change moves `V`, `G`, `K`, the family `𝒜`, `q`, or Λ. **Strategy-side** iff it picks a POINT or PATH in Λ, or a way to invert `𝒜(λ)` there (resolvent, splitting, schedule, Krylov vs SI, tolerances, window). **Solution** records the path, the point reached, and the gauge that picked a representative. This answers the user's objection exactly: a resolvent changes the answer fundamentally AND is Strategy-side, because *the Problem already contains every `s`* — α and neutron noise are ONE terminal object (the pencil `(A, T)`, `T = M[1/v]`) under two Strategies, where the solution-set rule needed two Problems.

| kind | Λ | terminal | the Problem's LAST step | the Solution's FIRST act |
|---|---|---|---|---|
| k | ℂ | `𝒜(λ) = A − λF`, `q ≡ 0` | the pair `(A, F)` on ONE space | pick the spectral point + an inversion |
| source-driven | `{•}` | `𝒜 = A`, `q` | the affine system `(A, q)` — a zero-parameter pencil | pick an inversion |
| α / transient | ℂ | `𝒜(α) = A + αT` | the pair `(A, T)` — `[M]` `posing_filtration_charter.md:625` already rules the VERB `Pencil.at(σ)`: α is a COEFFICIENT SHIFT `M[Σ_t + α/v]` in the multiplier algebra, so no `TimeMassOperator` summand is owed, only the `1/v` field (`[M]` 0 hits for `pose_alpha`/`GeneralizedEigenPencil`/`TimeMassOperator` in `orpheus/`; the `1/v` seat reserved at `transport/fields/_coefficient_role.py:44-50`) | pick the root |
| noise | contour ⊂ ℂ | the same `(A, T)` + `q(s)` | the same pencil | evaluate along the contour |
| adjoint | same | `(𝒜†, q†)` | NOTHING new — `† = G⁻¹(·)ᵀG` is determined by Problem data | choose the dual face |

The 2×2 `(M present?) × (q present?)` = kernel / affine / spectrum / transfer — all four cells ship (`[M]` the kernel cell is `LossKernelGauge`, `sn/mesh/augmented_mesh.py:1042-1072`). Λ and the kind are DERIVED, never fields (⛔ `kind: str` / `is_eigenvalue: bool` refuted by its own live instance: `derivations/common/continuous_reference.py:129-171`'s `ProblemSpec` carries `external_source | None` AND `is_eigenvalue: bool`, a redundant pair spellable inconsistently at `[M]` 22 sites / 9 files; and `Solution.keff: float | None` is discriminated at `solution.py:543-549` under a header spelled `# ── Discrimination ──`).

**The three-type boundary as a GATE.** Problem: nothing on it is a choice — every member is a function of the generating data alone. Strategy: nothing on it moves the limit past tolerance — two values are a vv Mode-9 pair. Solution: `solve(sol.problem, sol.strategy)` reproduces it bit-identically (`[M]` today a `Solution` carries NEITHER coordinate).

**Placements the gate decides (each `[M]`):** `WithinGroupSystem` → SPLIT (its 4 fields are `loss` + `space` = the terminal object, `implicit_operator` + `explicit_gains` = one chosen splitting; its invariance docstring `coupled_system.py:340-341` is refuted by `_select_si_splitting`). `MatrixInverseOperator` → Strategy, already typed as one (`numerics/matrix_inverse_operator.py:43-45`). `ScheduledInvertibleOperator` → Strategy. ⛔ **`HomogeneousProblem.multiplication = MatrixInverseOperator(loss) @ production` is Strategy-side on the wrong type** — its own docstring (`homogeneous/solver.py:339-341`, re-verified) says *"constructing the matrix inverse explicitly IS the strategy choice"*; the hub should own the PAIR `(loss, production)` (both already cached) and the solver compose the resolvent. The hub's `cached_property` list under the gate: KEEP the spaces + `bc` + `reduced` + `loss_kernel_gauge`; GAIN `mat_xs`, `S`, `N₂ₙ`, `L + C`, `loss`, ONE `F`, and `k_pencil` / `source_system(q)`; NEVER gain an `.inverse()`, a splitting, `A⁻¹F`, a schedule, tolerances, `eigenvalue_method`, or the σ-keyed `_coll_cache`.

**Last step of the Problem vs first of the Solution — recommended: the Problem's LAST step (the user's tendency), with an ORDERING constraint.** Five arguments converge (no choice enters the build; `†` needs it — today's absence is exactly why the adjoint re-derives `F`; Problem morphisms transport it; R-cc3's identity needs a terminal witness; the shipped hub already holds it in two halves). ⛔ `WithinGroupSystem` must be split FIRST, or moving it drags `inner_schedule` onto the Problem. Measurable consequence (the gate): `[M]` `build_within_group_system` runs once per OUTER step today (`sn/solver.py:2094`, inside `solve_fixed_source`; 5 call sites in the module) — once per Problem after; a spy on the module symbol asserts the count EXACTLY, not `> 0`.

**SN is the sole outlier; diffusion and homogeneous are the model.** `[M]` the forward SN k-problem's two operators live on DIFFERENT spaces — `A` from `build_within_group_system` on the composite (`coupled_system.py:456-463`), `F` = `IsotropicFission.from_material_xs(mat_xs, space=mat_xs.mesh.bulk_space)` (`solver.py:1545-1548`) applied to a bare `(ng, *spatial)` array — while the ADJOINT path builds the posed pencil the forward does not (`F_posed = stack @ restrict_bulk`, `solver.py:2821`, daggered at the call site). `DiffusionSolver` composes `loss` and `fission` on ONE space (`diffusion/solver.py:255/:260`; its `__init__` sorts 7 Problem / 3 Strategy / 1 in-progress); `HomogeneousProblem` has `loss` and `production` on the pose. The fix is adopting the adjoint path's `F` spelling forward. `q` is HOMELESS (`_build_fixed_source_rhs`, `solver.py:3164`, its one construction site; `SolutionBase` has no source field) — the fixed-source twin of `scattering_order`; `q` is a VECTOR (`TimedFullField | CoupledField`), not a functional. The generating data has TWO halves — discretization (shared; the identity key's spaces/`geom_cache`/`bc`/gauge stay here) and posing (`q`, which `M`); `q` on the Problem re-keys identity per source (H2).

**Cross-kind shape — recommended: ONE Problem class per METHOD + ONE terminal-object type + kind-typed Solutions** (beaten: one class per kind — N copies of a 100 %-shared discretization; a two-layer per-kind Problem — runner-up, revisit at α/noise; separate `Pencil`/`AffineSystem` — the noise cell needs both; a `Problem` Protocol over the three hubs — the shared thing is the terminal object's TYPE). Hazards: H1 a cached pencil + the surviving MUTATING `rebind_cross_sections` (`solver.py:1591-1633` — `[M]` it IS `Pencil.at(σ)`, mutating, on the solver) = a stale-σ answer failing silently ⟹ the cached pencil and a non-mutating `.at(σ)` are ONE merge unit; H3 `loss_kernel_gauge` is σ-FREE while `loss` is not — deriving it from the pencil would make it rebuild on every `.at(σ)`, invisible to any value test; H4 `keff: float | None` is read at more sites than the two methods (`solution.py:642-656` maps `keff == 0.0` to `1.0`; 9 test occurrences); H6 `MatrixInverseOperator` construction is EAGER — moving it to a Strategy moves a failure point earlier.


### 27.2 RULED 2026-09-12 (`[R]` user, AskUserQuestion checkpoint on §27.1 — all four as recommended)

**R-cc2 RULED — the terminal object per kind, built as the Problem's LAST step.** The articulation of §27.1 governs the carve: `Terminal(p) = (V, G, K, 𝒜 : Λ → Hom(V,V), q : Λ → V)`; Problem-side iff a change moves any of those; Strategy-side iff it picks a point/path in Λ or an inversion of `𝒜(λ)`; the Solution records path, point and gauge. The pencil / affine system is the Problem's final product; every Strategy consumes only it; Λ and the kind are DERIVED, never fields.

**R-cc5 — the hub owns the PENCIL `(loss, production)`; the solver composes the resolvent.** C1's `HomogeneousProblem.multiplication = MatrixInverseOperator(loss) @ production` leaves the hub (Strategy-side on the wrong type — its own docstring said so); `solve_homogeneous_infinite` spells `MatrixInverseOperator(problem.loss) @ problem.production`; bit-identical (D5 is the wall). The hub gains the typed `k_pencil` when the terminal-object TYPE lands (step 2, third sub-step).

**R-cc6 — step 2's order (after the identity step): (i) split `WithinGroupSystem` into the terminal object (`loss` + `space`) and the chosen splitting (`implicit_operator` + `explicit_gains`); (ii) adopt the adjoint path's posed `F` spelling forward (`F_posed = stack @ restrict_bulk` — one `F` on ONE space, the composite); (iii) move the pencil onto the hub, with the `build_within_group_system` once-per-Problem spy (exact count, not `> 0`).** Each sub-step bit-identical-gated.

**R-cc7 — cross-kind shape: ONE Problem class per METHOD + ONE terminal-object TYPE + kind-typed Solutions.** The generating data has two halves — discretization (shared; the identity key) and posing (`q`, which `M`); `q` on the Problem re-keys identity per source.

Campaign order now: **step 1** identity + truncation order (R-cc3/R-cc4; explorer census `scratch/_consumers/explorer_identity_step_census.md` → test-architect → carve) — with R-cc5's hub amendment landed as a small preceding commit (it touches only the coda's artifact) — **step 2** (R-cc6 i/ii/iii) — **step 3** the three-type carve of `SNSolver` (12 / 6 / 2) and the Solution's two coordinates — **step 4** the rename `SNMesh` → `SNProblem` LAST.

**✅ R-cc5 LANDED `22ec2c67` (2026-09-12)** — `HomogeneousProblem.multiplication` retired; the solver composes `MatrixInverseOperator(problem.loss) @ problem.production` (bit-identical, D5 8/8); new row `test_R5_the_hub_owns_the_pencil_and_no_inverse_of_it`; `tests/homogeneous` 89; root 427/5; sphinx clean; dead_references 0/68. Step 1 (identity + truncation order) is next: explorer census `scratch/_consumers/explorer_identity_step_census.md` landed (57 occ / 12 files of `scattering_order` in `orpheus/`, 148 keyword call sites in `tests/`; THREE disagreeing spellings of the retained order — the `SNSolver` clamp, the adjoint entries' unclamped zero-padding, the fixed-source entry's raw hand-off to DSA; ONE flipping message pin; six production equality/hash consumers incl. the hash-keyed `_GEOM_CACHE_INTERN`; every generating datum RAISES on `==`/`hash` today; precedent `Axis._identity_key`/`FunctionSpace.__eq__`); test-architect verification plan dispatched 2026-09-12. ⚠ Fork surfaced by the census (§8): R-cc3 lists the closure class in the identity; the predicate's docstring and `sn/index.rst:849-856` rule the opposite ("fields from two closures remain contractible") — two questions under one predicate (same PROBLEM vs same PHASE SPACE); put to the user.


### 27.3 RULED 2026-09-12 — step 1's two design forks (`[R]` user, AskUserQuestion)

**R-cc8 — TWO predicates.** `same_phase_space` (CONTRACTIBILITY: geometry × quadrature × layout × scheme type, by CONTENT) serves the three `solution.py` callers (`compare`, `homogenize`, `condense(adjoint=…)`) — it asks whether two solutions' FIELDS may be paired; `__eq__`/`__hash__` (full Problem IDENTITY: every generating datum by content, closure class and truncation order included) serves save-state and Solution provenance. A P0 forward and a P3 adjoint share a phase space (their fields pair) but are different Problems (a Solution records which). The predicate's docstring and `sn/index.rst:849-856` keep their contractibility ruling FOR `same_phase_space`; R-cc3's list is the identity's.

**R-cc9 — the retained scattering order is a CONSTRUCTOR DATUM of the hub, CLAMPED once at construction; every entry reads it.** `SNMesh(..., scattering_order=L)` stores `min(L, len(SigS) − 1)` as generating data; `solve_sn` / `solve_sn_adjoint` / the fixed-source entries keep their kwarg as sugar that forwards to the hub's constructor; DSA and the adjoint posing read the hub's value — the census's three disagreeing spellings (the `SNSolver` clamp, the adjoint entries' unclamped zero-padding, the fixed-source entry's raw hand-off to DSA) collapse to ONE.

**✅ step 1 PRE-CARVE ANCHORS LANDED `31b632c3` (2026-09-12)** — `tests/sn/mesh/test_problem_identity_anchors.py` (37 rows) + `tests/data/test_mixture_identity_anchors.py` (20 rows); verification plan `scratch/_consumers/test_architect_identity_step.md`. `[M]` red-befores: the d=3 vacuity is SPATIAL only; every same-data d≤2 `from_axes` pair reads `False`; `solve_sn(3)` serves `at_order {0,1}` vs `solve_sn_adjoint(3)` `{0,1,3}`. ⛔ Refutations: order-discrimination rows are VACUOUS above (0, ≥1) on the abstract library (all 12 mixtures `len(SigS) = 2`); a content key costs 1.02 ms/call on 421-group data (must be CACHED — collides with a mutable `Mixture`); a content `__hash__` ping-pongs `_GEOM_CACHE_INTERN` 2 → 6 builds. Blocking rulings O-2/O-3/O-4 put to the user.


### 27.4 RULED 2026-09-12 — step 1's blocking design rulings (`[R]` user; test-architect memo §9)

**O-2 — `SNSolver.__init__(scattering_order=)` RETIRED; `SNSolver` reads `sn_mesh.scattering_order`.** One spelling. `[M]` 51 call sites / 31 files migrate in the same commit by moving ONE keyword one constructor up onto the hub each already builds (44 test sites / 25 files, 5 top-level `derivations/` sites / 5 files, 2 in `orpheus/`).
**O-3 — `_GEOM_CACHE_INTERN` keys on CONTENT, its validation re-keyed from the closure INSTANCE to the closure CLASS.** `[M]` the table's 8 fields are bare σ-free arrays with no mesh/closure reference, so sharing between content-equal problems is correct: 2 → 1 builds (a gate pins the count exactly).
**O-4 — `Mixture` is FROZEN** (`__post_init__` writes through `object.__setattr__`); its content `__eq__`/`__hash__` on a `tobytes()` key computed once at construction; the 33 mutating test sites / 14 files migrate to `dataclasses.replace`. (`[M]` a live key costs 1.02 ms/call on 421-group data and the intern reads it 6–10×/solve.)
**O-6 — the ONE identity definition lives at `MaterialMesh._identity_key`** (axes + BC tags + `mat_map` + materials' content), EXTENDED by `SNMesh` (quadrature content, scheme type, closure class, the clamped order) and `DiffusionMesh`, exactly as `EnergyAxis`/`LegendreAxis` extend `Axis._identity_key`; `HomogeneousProblem`'s key is its mixture's content.
Adopted from the memo without a ruling (routine): O-5 `Materials` keeps `eq=False` this step; O-7 the key holds the CLAMPED order (docstring states: on the abstract library requests 1, 2, 3, 5 are the same problem; only (0, ≥1) separates); O-8 `Solution` gains no order field this step (step 3's `(problem, strategy)` pair).

**Step 1 decomposition (merge units per the memo §11.2):** **S1a** `Mixture` frozen + content identity (+ the 33 test migrations); **S1b** the identity keys (`MaterialMesh` → `SNMesh`/`DiffusionMesh`/`HomogeneousProblem`), the `Quadrature` content key, `same_phase_space` (content contractibility) replacing `is_same_phase_space` at its three callers, the intern's closure-CLASS validation — ONE commit (`__hash__` + O-3 + O-4 are inseparable); **S1c** the clamped `scattering_order` as a hub constructor datum read by `SNSolver`, the adjoint posing and DSA, the entries forwarding their kwarg, the 51-site migration. Battery arm B6 (`__hash__` → a constant) runs BEFORE S1b's edit to list the hash-spelled separation legs.

**✅ S1a LANDED `deacd897` (2026-09-12)** — `Mixture` is a VALUE: frozen, read-only copies of every array (dense fields, χ, the grid, each sparse block's canonical CSR triple), stacks stored as tuples (`Sequence[csr_matrix]` input), cached content `_identity_key` + `__eq__`/`__hash__` (the `Axis` precedent); 33 test mutations → `dataclasses.replace` (AST-migrated), 2 in-place-write rows re-posed, the RECORD class deleted, 15 xfail rows green. 13-tree gate all rc 0 (11 671 passed). Battery B6 (`SNMesh.__hash__` → constant, pre-S1b, over mesh/cache/solution/homogeneous/condensation scopes): **0 reds of 419** — no separation leg is spelled through the hub's hash today (the intern's key uses identity `__eq__`), so S1b's `__hash__` lands with no hash-spelled leg to re-pose.

**✅ S1b LANDED `2c1667b0` (2026-09-12)** — ONE identity definition at `MaterialMesh._identity_key` (axes by content: edges, boundary-law tags, labels, chart; `mat_map`; the materials' content), extended by `SNMesh` (quadrature content, scheme type; the closure CLASS in identity only); `same_phase_space` (contractibility by content, closure and order EXCLUDED — R-cc8) replaces `is_same_phase_space` at its three callers; `Quadrature` (`eq=False`) and `HomogeneousProblem` get content identity; `_GEOM_CACHE_INTERN` validates on the closure CLASS (O-3: content-equal hubs share ONE table, 2 → 1). 13-tree gate: every tree rc 0 except ONE numerics row that had pinned `Quadrature`'s unhashability as the REASON for the generator's exclusion from `Axis._identity_key` — re-posed to the ruling (provenance is not identity; the `DiscreteMeasure` leg still bites); numerics re-run green. `dead_references` caught one dead `:meth:` xref mid-carve (fixed). Battery: 4 arms, every red attributable (closure dropped from identity 2; `mat_map` dropped 1; closure folded into the pairing 3; instance validation 2); B6 pre-carve 0/419.

**✅ S1c LANDED `7b4d2b78` (2026-09-12) — STEP 1 (identity + the truncation order) is CLOSED; #459 closes with it.** `SNMesh(..., scattering_order=)` on all three constructors, clamped ONCE in `_init_core`, stored, the third identity datum (not a contractibility datum); `with_scattering_order(L)` — a Problem morphism; `SNSolver.__init__`'s kwarg retired (O-2), `_adjoint_posing_parts(sn_mesh)`, `build_within_group_system(sn_mesh, mat_xs)` and both `DSA…from_sn_mesh(sn_mesh)` read the hub; the four entries forward the kwarg through `_as_sn_mesh`. Migration `[M]` 37 AST sites / 22 files + 16 hand sites (9 fixture-provided hubs, the DSA rows, the adjoint certification, `record_for`). The seven ORDER anchor rows green (clamp table 0/1/3/5 → 0/1/1/1; the Mode-11 route spy on the adjoint entry; P0 ≠ P3 as Problems, P0 ~ P3 as phase spaces). Gates: 13 trees all rc 0 (sn 3648, derivations 1636, root 427); sphinx clean; dead_references 0/68. Battery clamp removed → 4 red; adjoint posing raw → 1 (the route spy, exactly); morphism mutating → 1 — every arm attributable. Surprises: two production residues the exact-match script did not reach (a bare-name read; a second caller of the posing helper) — pyright caught both before any gate, cost one edit; the `test_dsa_rate.py` surrogate is §6b's duck-typed-stub spelling again.
⚠ **Surprise (plan-authoring §2, the exclusion family's SIXTH member — a ROOT list):** the census said "5 top-level `derivations/` sites"; the migration walked `orpheus/` + `tests/`, so five `derivations/diagnostics/*.py` sites kept the retired kwarg — no test imports them, so no gate could have seen the `TypeError`; caught by the archivist re-censusing every root (`examples/`, `tools/` too). ERR-084 (the identity defect class) drafted, BLOCKED on `@pytest.mark.catches` decorators — its own commit next. Gap recorded: `MaterialMesh`, `Quadrature`, `DSACorrection` are not `automodule`'d anywhere.
**Step 1 ledger:** anchors `31b632c3` · S1a `deacd897` · S1b `2c1667b0` · S1c `7b4d2b78` (+ stamps). **Next: ERR-084's commit, then step 2 (R-cc6: split `WithinGroupSystem` → the posed `F` forward → the pencil onto the hub), test-architect first.**

**✅ ERR-084 LANDED `0221e19e` (2026-09-12)** — the identity defect class catalogued with its catchers (the two anchor modules); the index regenerates from the GRAPH, so a new entry needs the Sphinx build first (the reconcile gate reads 83 until then — a mechanism worth knowing, recorded here). Step 1 is fully closed; step 2 (R-cc6) opens: test-architect + explorer dispatched on the `WithinGroupSystem` split.


### 27.5 STEP 2 OPENED (2026-09-12) — the pencil; anchors `c0f0f34e`; rulings owed

Inputs: explorer census `scratch/_consumers/explorer_step2_census.md` (§0–§10) and the verification plan `scratch/_consumers/test_architect_step2.md` (1032 lines; anchors `tests/sn/architecture/test_step2_terminal_object_anchors.py`, `tests/sn/operators/test_step2_posed_fission_anchors.py` — 29 passed / 3 xfail on the unmodified tree). Five `[M]` refutations of the charter's wording, each priced:

* **F-1 — sub-step (ii)'s bit-identity is reachable ONE way.** The forward k-outer reading `FissionOperator.isotropic_energy` (the datum binding `AngularLift` exposes) is `array_equal` 200/200; the forward APPLYING `F` on the composite is 0/200 (≤ 1 nulp — a `/W` re-association) and changes the k-outer's CARRIER (`power_iteration` iterates a bare `(ng, *spatial)` scalar flux). ⟹ "one F on one space" is a VALUE change unless "one F" means one datum binding shared by identity.
* **F-2 — the `WithinGroupSystem` split is NOT a field partition.** On the seedless arm `implicit_operator` IS the bare `L + C` and `explicit_gains` the leaf triple — Problem-side objects that happen to form Jacobi; the G-S choice is made downstream (`_select_si_splitting`; `_maybe_window` re-binds S/N₂ₙ); `loss` does not expose its factors (nested `OperatorSum`) ⟹ the Problem must EXPOSE them.
* **F-3 — the §6c red-before already ships** as `test_stage_separation[cart2d-gauss_seidel]`'s strict xfail — and the split DEMOTES it to a tautology; the successor asserts `A = M − N` per Strategy value (`[M]` `array_equal` 0.0 both schedules; an ERR-056 mutation moves it 3.6).
* **F-4 — "the adjoint's posed F" is THREE spellings** (`FissionOperator`/`FullFieldSpace` seedless — the `stack @ restrict_bulk` branch never runs; `OperatorProduct`/`CoupledSpace` carrying; the forward's `IsotropicFission`/`bulk_space`); `solve_sn_adjoint_fixed_source` builds `F_posed` and discards it.
* **F-5 — H1's repair is UNWRITABLE over the generating data**: a cached pencil after `rebind_cross_sections` is `array_equal` to its pre-rebind self while a fresh build moves `max rel 0.1619`; `with_cross_sections(σ)` needs a per-cell datum not derivable from `materials`/`mat_map`, which must then enter `_identity_key`.

Census headlines `[M]`: `WithinGroupSystem` 2 constructions (both inside the builder) + 1 `replace` producer in a test; production reads 13/8/6/2 on its four fields; terminal-only and splitting-only consumer sets are clean except `_solve_source_iteration:2104` (an `isinstance` on the implicit operator for a PROBLEM fact); `build_within_group_system` 43 calls (6 production: n_outer per k-solve on the forward eigen path, once for fixed-source and adjoint; 36 tests, 33 passing `scattering_op=`; 1 derivations); two Mode-11 patches name the builder as a STRING on the solver module (`test_psi_half_coupling.py:2288/:2318`) — a move onto the hub silently un-hooks them; `weight_norm` on the solver is DEAD (0 reads); `build_coupled_system` still declares and drops a `scattering_order` kwarg (stale after S1c); the splitting choice is TRACE-only (bulk 0.0 / trace 0.997) so every bulk functional is a non-catcher; NO bit-exact wall exists on the forward k path (all eigen snapshots are `SAFETY × conv_tol` bands; the regression corpus pins no full-solve `angular_flux`); the hub's 13 cached properties are all σ-free — `loss`/`L + C`/the pencil would be the first σ_t snapshots (H1).


### 27.6 RULED 2026-09-12 — step 2's forks (`[R]` user, AskUserQuestion on §27.5)

**O-5/O-6 RULED — the σ merge unit lands in THIS step:** `SNMesh.with_cross_sections(σ)` mints a NEW Problem (the per-cell datum enters `_identity_key`); `rebind_cross_sections` retires; `mat_xs` is cached on the hub. The pencil, the cached `mat_xs` and the morphism are ONE merge unit — a depletion trajectory is a sequence of Problems.

**O-2 — agreed in principle (factors on the Problem; the splitting is a Strategy object minted from the Problem), with a PROCESS ruling:** *"the consumption will have to be designed with some forward thinking, because there are several steps necessary between pencil and lowering. We have a plan with a table of some steps to ground this development on. And we need to do this over a proper planning session in planning mode."* ⟹ the carve of (i)–(iii) waits for a PLANNING SESSION (plan mode) that designs the whole consumption chain Problem → pencil → (partition / lowering / …) → Strategy, grounded on the existing steps table (`posing_filtration_charter.md` / `operator_strategy_realization_campaign.md` — to be identified at the session's opener).

**O-3 — RE-FRAMED as a QUERY CONTRACT, not a read-through:** *"it's reasonable for the hub SNProblem to have the original operators. The consumer side in principle might not have the operators exactly as they are in the Problem side, but modified operators at the consumer side. Maybe the right question is 'does the consumer side give unmodified operators, or operators that are not guaranteed to be unmodified' — maybe the Boundary Operator was split, maybe the Scattering operator was split as well, maybe SCC has been applied to the boundary to make it a topological toroid and eliminate the SCC, etc. So we just need to think what we expect from either Problem and Solver when we query them."* ⟹ the Problem answers with its ORIGINAL (unmodified) operators; the Solver/Strategy answers with the operators AS IT USES them (split, lowered, SCC-eliminated) — two query surfaces with different guarantees; designed at the planning session.

**O-1 — the user asked for the REASON and the LONG-TERM path** (*"Explain me why you recommended 1 over 2, and does this recommendation stand as the long-term path? You can disregard the effort to re-baseline."*) — answered in the session: option 1 was a PROCESS recommendation (bit-identity inside a "bit-identical-gated" sub-step), not an architectural one; the long-term object is the composite `F` on the pencil's one space, with the bulk face a Strategy-side REDUCTION (F's rank structure `lift ∘ F_bulk ∘ restrict` lets the k-outer iterate the scalar flux) — the same object's derived face, so the values stay bit-identical while the Strategy reads the datum binding, and ≤ 1 nulp if it applies the composite; the k-outer's CARRIER is the Strategy's choice, gated on its own criteria. Recorded for the planning session.

### 27.7 Step 2 — the PLAN and its landing log live in `.claude/plans/consumers_step2_design.md` (approved 2026-09-13 in plan mode)
The design memo carries the rulings of the two checkpoints (the labelled-piece primitive; the collision cache on the `StreamingCollisionOperator` instance; every solver attribute read migrates now; the pencil's shape (B) — `OperatorPencil(lhs, rhs)` + kind-typed `EigenPosing`/`SourcePosing`, the `(M and q)` cell a composition and its subcritical-multiplying-source witness RULED IN; the names), the chain table (§3.1), the execution census (§6) and the landing log (§7). **C1 — the record splits; the Strategy value is born — ✅ LANDED `628997b1`** (R7's strict xfail XPASSed and is gone; anchor (i)'s marker gone). Next per §6.4: C2 (ONE `F` on the hub), then C3a+C3b (the σ merge unit + the hub's `system`/pencil/posings + the witness), then C4.

**Landing log (trust git):** C1 ✅ `628997b1` (the Strategy value; stamp `41210adc`) · C2 ✅ `1ce64371` (ONE `F` per Problem — `SNMesh.fission`, the record's `production`, `fission_op` retired; 2026-09-13) · C3a ✅ `6e96b0b3` + C3b-1 ✅ `06962113` + C3b-2 ✅ `c930c68f` (the σ datum + the pencil/posings + the walk seam (ii) + `source_posing`/the multiplying-source entry + `KEigenvalue` onto `EigenPosing`; ONE merge unit, ff-merged to `main` 2026-09-14, 13 trees rc=0 on the tip) · C4 ✅ the ten follow-ups filed #460–#469 (2026-09-14) — the landing log with per-unit gates is the design memo's §7; NEXT per the ruled order: step 3 (the Solution carries its posing), then the `SNMesh → SNProblem` rename LAST.

## 27.8 ⏸ COMPACTION POINT #11 (2026-09-14, written after step 2's merge; tree clean, no gate running, no agent holding the tree) — the consumers campaign's step 2 (the pencil) LANDED; the next act is step 3 (the Solution carries its posing), then the rename LAST

**State `[M]`:** `main` = `origin/main` @ `0f275510` (the σ merge unit `c930c68f` + three docs/plan stamps `207d2b07`, `3e8b126b`, `0f275510` — `[M]` `git diff --stat c930c68f..0f275510 -- '*.py'` is EMPTY, so the exit gate below certifies this tip); working tree clean but for `scratch/` and the foreign untracked `.claude/plans/harness_context_budget.md` (NOT ours to stage); no gate running. The step-2 design memo with its per-unit landing log (gates, batteries, surprises) is `.claude/plans/consumers_step2_design.md` — its ⏹ banner + §7 are the authority over its §1–§6 wherever they differ.

### 27.8.1 Step 2 — the commit table (all ff-merged to `main`; C3a+C3b-1+C3b-2 as ONE merge unit)

| unit | hash | what | gate at landing |
|---|---|---|---|
| C1 (i) | `628997b1` (stamp `41210adc`) | `WithinGroupSystem(loss, space, factors)`; `SNLossFactors`/`RadialCharacteristicFactors`; `orpheus/sn/splitting.py` — `Splitting.from_schedule` the ONE labelling site, `resolve_schedule` the ONE string→schedule site; `_select_si_splitting` retired; `InnerSolve(system, splitting, driven_gains, iterate)` | scope + wide + ROOT; battery |
| C2 (ii) | `1ce64371` | `SNMesh.fission` — the ONE composite `F` per Problem (minted through `FissionOperator.from_solver_data`); the record's `production` + `factors.fission`; `SNSolver.fission_op` retired | bit-identical on the k path; battery (the Mode-12 null: scalar `F` scaling is invisible to a normalized eigen solve — positive control = cell-local ×1.01) |
| C3a (iv) | `6e96b0b3` | σ_t is a Problem DATUM: `MaterialMesh.sigma_t_cell` (normalised, read-only, in `_identity_key` only), `with_cross_sections(σ)`, `mat_xs` cached on the hub; `rebind_cross_sections`/`weight_norm`/the `__debug__` assert retired; the geometry intern re-keyed on contractibility × closure class | scope + wide + ROOT; battery |
| C3b-1 (iii) | `06962113` | `OperatorPencil(lhs, rhs)` (`orpheus/numerics/pencil.py`) + `EigenPosing`/`SourcePosing` (`orpheus/numerics/posing.py`, `SpectralMap` with λ as ONE division); the hub's `system`/`pencil`/`eigen_posing` built ONCE (eager bind in `SNSolver.__init__`); `KEigenvalue(posing, implicit, explicit)`; `HomogeneousProblem.pencil`/`eigen_posing`; the AC-a gate | scope + wide + ROOT; the four strict-xfail anchors flipped and their markers deleted |
| C3b-2 | `c930c68f` | the walk seam (ii): `SigmaStratum`/`RawSigmaStratum`/`ScanStratum`, `bind_sigma`, `StreamingCollisionOperator.sigma_stratum` (the σ snapshot's owner; ERR-085 the stale-σ walk); `hub.source_posing(q)` = `SourcePosing(pencil.at(1), q)`; `solve_sn_multiplying_source` (refused by the hub's k-solve through `_admits_multiplying_source(k)`; the production LAGGED as an extra gain, certificate on the solved equation); the adjoint site poses `EigenPosing(OperatorPencil(implicit.H − gain.H, F_posed.H), K_MAP)` per arm; `D` follows the datum (`Mixture.p1_outflow`; `MaterialXSField.diffusion_coefficient` per cell) | scope 3087 / sn 3718 / ROOT 433; the nine-arm battery; the 13-tree exit gate |
| stamps | `207d2b07`, `3e8b126b`, `0f275510` | landing log + hashes; the +1 count CORRECTION; the ⏹ banner + L64 | docs only |

### 27.8.2 The exit gate `[M]` (13 trees, `python -O -m pytest -p no:randomly -m "not slow" -q --timeout=240`, SERIAL, detached; `scratch/_consumers/c3/gate13/summary.log`; tip `c930c68f`; 60 min wall)

| tree | passed / skipped / xfailed | wall |
|---|---|---|
| numerics | 3411 / 2 / 0 | 26 s |
| transport | 950 / 1 / 0 | 23 s |
| geometry | 727 / 4 / 1 | 9 s |
| data | 330 / 0 / 0 | 32 s |
| homogeneous | 89 / 0 / 0 | 2 s |
| diffusion | 115 / 0 / 0 | 3 s |
| cp | 141 / 0 / 0 | 64 s |
| moc | 121 / 0 / 0 | 101 s |
| mc | 40 / 0 / 2 | 107 s |
| cross_method | 81 / 0 / 0 | 13 s |
| sn | 3718 / 1 / 36 | 1011 s |
| derivations | 1636 / 13 / 11 | 2178 s |
| root | 433 / 0 / 5 | 22 s |
| **total** | **11 792 / 21 / 55**, 13 of 13 rc=0 | ≈ 60 min |

Baseline for step 3's exit prediction: **11 792 / 21 / 55** @ `c930c68f`. ⚠ The per-tree `-m "not slow"` deselections are part of this configuration (116 in sn, 67 in derivations, 16 mc, 13 cp, 8 cross_method, 3 moc, 2 data, 2 root).

### 27.8.3 Corrections that supersede older text in this plan

* §27.6's R-cc7 realization `LinearPencil(A, M=None, q=None)` is **WITHDRAWN** (step-2 memo §3.5, 2026-09-13): its "all four cells occupied" evidence mis-counted the kernel cell, and its verbs change meaning per cell. The ruling's INTENT (one Problem class per method; the pencil the Problem's last step; kind-typed Solutions) survives as shape (B): `OperatorPencil(lhs, rhs)` + `EigenPosing(pencil, spectral_map)` / `SourcePosing(operator, source)`, the `(M and q)` cell a COMPOSITION `SourcePosing(pencil.at(σ), q(σ))`. The 2026-08-19 name ruling `GeneralizedEigenPencil` maps onto `OperatorPencil` + `EigenPosing`.
* R-cc6 (ii) "ONE `F` per Problem" holds on the MINT (`[M]` 1 per hub, the anchor reads 1) and is **open on the adjoint FACE**: `_adjoint_posing_parts` is arm-asymmetric (the seedless adjoint iterates on the full-field carrier with `factors.fission`; the carrying arm on the coupled space) and the production adjoint site poses the pencil PER ARM through the Strategy's implicit/gain — the hub's coupled `eigen_posing.H()` broke the seedless adjoint (15 rows) at C3b-2. Filed **#467**; it is step 3's charter item, not a step-2 residue.
* The in-code ruling *"`L + C` is deliberately NOT cached"* (`solver.py`, the O-5 discussion at the step-2 memo §6.2) was **REWRITTEN** at C3b-2 — both halves dissolved: σ is a Problem datum (no live rebind), and the hub's `system.factors.streaming_collision` is the ONLY copy and the object the sweep inverts.
* Any text naming `SNSolver.rebind_cross_sections`, `SNSolver.mat_xs`/`scattering_op`/`n2n_op`/`fission_op`, `_select_si_splitting`, `sn_mesh._coll_cache` or the builder's `scattering_op=`/`n2n_op=` kwargs as LIVE is history (retired at C1/C2/C3a/C3b); `_pole_mirror_cache` is the ONE surviving hub memo (#468).
* The stamp commit `207d2b07`'s message explains the sn count 3717 → 3718 by the new ERR-085 marker — **wrong** (`[M]` `--collect-only -k ERR-085` collects 0 rows; the +1 is the per-commit run's one red, fixed in the same commit). Corrected at the step-2 memo §7.5 and in plan-authoring's surprise log (2026-09-14).
* `solve_sn_fixed_source` on a fissile hub KEEPS the pure-transport posing (bit-identical for every existing caller); the multiplying posing is a SEPARATE entry (`solve_sn_multiplying_source`). The step-2 memo §5.1's "the entry's default is a step-3 decision when the Solution carries its posing" stands.

### 27.8.4 Durable lessons (each already promoted; listed so the next session does not re-derive them)

* **L64** (`lessons.md`): two rewriters over one predicate double-wrap — the second's exclusion set is the first's OUTPUT; prefer one idempotent pass asserted as a no-op on re-run; `ast.parse` after every textual repair of a structural edit.
* **vv-principles #17, the AIM trap**: a battery arm whose TARGET row stays green while other rows redden hit a different object than it was aimed at — read the red SET against the target row, never its count (the P-2 arm patched `solve_sn`, not on the refusal path; re-armed on the NAMED predicate).
* **plan-authoring 2026-09-13**: a field-read census keys on the TYPE (helper-returned receivers), never on how a local is spelled. **2026-09-14**: an explanation of a measured delta is a claim and owes its `[M]` before it is published — reconcile, then write.
* Battery/test facts worth carrying: a scalar scaling of `F` is a Mode-12 NULL for a normalized eigen solve (use a cell-local ×1.01 as the positive control); `np.asarray(<CoupledField>)` is an OBJECT array, so `np.array_equal` on two of them is always `False` (a tautological "activation" row — compare `to_flat()`); `vdot` vs `np.sum` differ in the last digit and `1/(a/m) ≠ m/a` bit-wise — a spectral map spells λ as ONE division (`SpectralMap.of_quotient`).
* Process facts that held: the ROOT tree in EVERY commit's gate set (the two error-catalogue reconciliation rows go red the moment an ERR entry is assigned and green only after the Sphinx build regenerates the index — stage `.claude/skills/vv-principles/error_index.md` + `docs/theory/verification/matrix.rst` with the carve); ONE Sphinx build at a time; the archivist holds `docs/` only and never builds.

### 27.8.5 ▶ Step 3 — the Solution carries its posing (the ruled order: step 3, then the `SNMesh → SNProblem` rename LAST, #412)

**Goal (domain terms, from R-cc2 and the step-2 memo §3.1 row 9).** A Solution is the pair (Problem, posing) plus the Strategy that produced it and the records: its KIND is the posing's type (an eigen Solution carries a ray, λ and the normalization GAUGE; a source Solution carries the coset representative and, when `ker A ≠ 0`, the loss-kernel gauge), never `keff is not None`; the residual and the exit balance read `posing.residual(ψ)` / `posing.balance(ψ)` (one functional, today spelled at the SN tier as `_exit_balance_defect` and `SNSolver.compute_keff`); `q` becomes a content-keyed morphism (`with_source(q)`) rather than a method argument; the adjoint posing is `eigen_posing.H()` on BOTH arms. Charter items already filed: **#461** (the gauge on the Solution), **#462** (the balance functional at the SN tier), **#467** (the seedless adjoint on the coupled carrier); adjacent: #460 (`admits(q)`), #465 (`ProblemSpec`'s Optional pair in `derivations/` — the same kind defect one tree over; decide at the charter whether step 3 touches it).

**Dependency readiness `[M]` 2026-09-14 (what step 3 consumes from step 2, verified against the tree):** the hub carries `system`, `pencil`, `eigen_posing`, `source_posing(q)`, `fission`, `mat_xs`, `with_cross_sections` (all `hasattr` true on `SNMesh`); `SourcePosing` has `residual`, `balance`, `H` (unary); `EigenPosing` has `residual`, `rayleigh`, `balance`, `H` (nullary). NOT there, by design or by filing: `admits(q)` (#460); any gauge/posing/strategy field on the Solution — `[M]` `dataclasses.fields(SolutionBase)` = `angular_flux, scalar_flux, mesh, keff, history, radial_characteristic` (and `Solution` adds none); the balance functional at the SN tier (#462); the adjoint on both arms (#467).

**Ground counts `[M]` 2026-09-14 (re-measure at the opener — plan-authoring §7; these are the §6b populations a Solution reshape touches):** `Solution(` mints in `orpheus/`: 4 sites / 2 files (the two fixed-source entries at `solver.py:3814/:4016` + the eigen path); `.is_eigenvalue(` reads: 6 / 2 files (`orpheus/` + `tests/`); `keff is not None`: 3 / 1 file; `_build_fixed_source_rhs(`: 3 / 1 (`q`'s ONE construction site — no field on any hub or Solution); `_adjoint_posing_parts(`: 8 / 3 files; `solve_sn_multiplying_source(`: 4 / 2 files. ⚠ A field-read census of the Solution keys on the TYPE (the 2026-09-13 surprise row): grep the helpers that RETURN a `Solution`/`SolutionBase` (annotation) and read what their callers do with the result — across `tests/`, `derivations/` AND `cross_method/`, which read Solutions of several families.

**Opener obligations (in this order, before any design):** (1) reconcile this section against the tree (`git merge-base --is-ancestor` for every hash above; the ground counts re-run); (2) an explorer census of the Solution's consumers BY TYPE (the populations above + every `SolutionBase` subclass across the six method families — `dataclasses.fields` per subclass is the denominator); (3) the cross-domain question, asked once: what a Solution IS for a source-driven vs an eigen kind (the step-2 memo §3.5's table is the ground — coset vs ray, gauge, the arity of the adjoint); (4) a PLANNING SESSION in plan mode with the user's checkpoint rulings (the process ruling of §27.6 O-2 governs step 3 too: *"several steps necessary between pencil and lowering"* — the Solution reshape crosses every SN entry, the adjoint entries, the derivations' reference specs and `cross_method`); (5) test-architect BEFORE the carve (the reshape is an operator-algebra carve crossing typed ↔ adjoint ↔ multi-family boundaries — the proactive MUST trigger); (6) the entry ledger gate (`tests/sn/solve/test_every_entry_gauges_its_trace.py`) is a §6b member of ANY entry change — its row set moves with the entries.

**Checkpoint protocol run for THIS point:** everything committed and pushed (`0f275510` = `origin/main`); the memory topic file `project_cs_ladder_campaign2.md` and `MEMORY.md`'s ▶ line record step 2 as LANDED and point here; no agent memory is pending. After compaction: re-anchor from THIS section + `git log`, re-read only the step-2 memo's ⏹ banner and §7 (not §1–§6), and do NOT re-dispatch agents for questions the step-2 memos already answer (`scratch/_consumers/c3/draft.md`, the archivist/test-architect memories under `.claude/agent-memory/`).

### 27.9 Step 3 — U1 + U2 (+U2d, U2e) LANDED (2026-09-17); the PLAN and its landing log live in `.claude/plans/consumers_step3_design.md` (approved 2026-09-14 in plan mode; §9 the landing log)

**Commit table (all on `refactor/consumers-step3`, ff-merged to `main` as ONE merge unit after a 13-tree gate rc=0 on the tip `b59e4a76`; an ff-merge, no merge commit — `main` = the branch tip, which is the plan-only commit carrying this section, one commit above the gated tip):** `d946ba9d` the design memo · `42b90c4b` the pre-carve anchors + plan corrections · `f8a838a7` **U1** (`numerics/outcome.py` — `EigenOutcome`/`SourceOutcome`, `Evidence`, `ExitCertificate`; `numerics/gauge.py` — `ScaleGauge`, `KernelGauge`; `CoupledField.space`; homogeneous adopts) · `bcd9c83c` **U2** (`SolutionBase[O]`; ONE mint; the certificate; the state stored WHOLE with every flux member derived; F11 entry-side posing; the multiplying entry's `Certified(k)` + hoisted warnings + export, ERR-086; **U2d fused** — both adjoint arms on the coupled carrier, #467) · `b59e4a76` **U2e** (the 26 eigen artefacts re-baselined; the escalated instrument reads 0).

**Exit gate `[M]` (13 trees on `b59e4a76`, `scratch/_consumers/step3/gates/tip/summary.log`):** 13 trees rc=0, **11 851 passed / 21 skipped / 55 xfailed** in ~61 min wall (numerics 3441 · transport 950 · geometry 727 · data 330 · homogeneous 92 · diffusion 115 · cp 141 · moc 121 · mc 40 · cross_method 81 · sn 3740 / 1 / 36 · derivations 1636 / 13 / 11 · root 437 / 5). Reconciled per tree against step 2's exit (11 792 / 21 / 55, `scratch/_consumers/c3/gate13/summary.log`) BEFORE this was written: numerics **+30** = U1's `test_gauge.py` 15 + `test_outcome.py` 15 (`--collect-only`); homogeneous **+3** = U1's `test_homogeneous_outcome.py`; root **+4** = the fresh-interpreter gate's 2 rows × the 2 new numerics modules; sn **+22** = +23 anchor rows at `42b90c4b` (31 collected − 8 RULED xfails) − 11 RECORD rows deleted at U2 + 8 xfails flipped + 2 ERR-086 catchers; xfailed 55 → 55 (sn 36 → 44 → 36); every other tree ±0. The memo's §4 prediction (11 810–11 825) under-counted the two U1 modules (it said +10..14 for numerics).

**Corrections that supersede older text in THIS plan:** §27.8.5's "12 `.npz`" eigen artefacts are **10** (the three `p1_aniso` snapshots are `fixed_source` by their own key); its ground-count list mixed populations (step-3 memo §2.6); `SolutionBase` has exactly TWO subclasses, both SN (the "six method families" premise was wrong); `evaluate_residual` EXISTS (the step-2 memo's "0 hits" was false when written). U4 and U5 of the memo's §6.4 landed INSIDE U2 (F3/F3b ruled the state whole; #467 fused because the state-on-domain law would have refused a per-arm posing) — the memo's unit table reads as designed, its §9.3 as landed.
...
## 27.11 ⏹ The rename `SNMesh → SNProblem` (#412) LANDED (2026-09-18) — the consumers campaign's ruled LAST act; the campaign is COMPLETE

**Rulings `[R]` (2026-09-18, all four as recommended):** `sn_mesh` → `problem`; the class at `orpheus/sn/problem.py` (`sn/mesh/` keeps `method_space` + `reduced_operator`); `Solution.mesh` → `problem`; `_as_sn_mesh` → `_as_problem`. The class name `SNProblem` was ruled 2026-08-28.

**Commit table (branch `refactor/consumers-rename-412` off `main` `3f39a406`, ONE merge unit, ff-merged):** P1 `53e8d33d` (the class + the module move, 309 files) · P2 `763b1b9e` (`problem` at 3222 sites / 147 files) · P3 `bb3efbcd` (the Solution's field + the helper, 55 files) · P4 `0c6978a4` (the docs prose half, archivist, 19 pages; `Closes #412`) · **P5 `3b8e0591`** (the TYPED residue the docs pass found — 27 + 3 `mesh`-spelled `SNProblem` slots, five mesh-spelled verbs, 57 files) · **P6 `293e8614`** (one field-list pin re-keyed — the sn tree's single red on the tip) · stamp (this commit, = `main`).

**Exit gate `[M]`:** `[M]` 13 trees on `3b8e0591`: 12 rc=0 and sn **1 red** (3726 / 1 / 36 — a literal field-list pin on `_OneDimScanWalk`, spelled `mesh`); the pin re-keyed at `293e8614` and the sn tree + ROOT re-run on that tip: **sn 3727 / 1 / 36 rc=0, ROOT 437 / 5 rc=0** — the unit reads 11 845 / 21 / 55 across the 13 trees. Per tree at `3b8e0591`: numerics 3448 · transport 950 · geometry 727 · data 330 · homogeneous 92 · diffusion 115 · cp 141 · moc 121 · mc 40 · cross_method 81 · derivations 1636 / 13 / 11 · root 437 / 5 — every count identical to the campaign's last gate (11 845 / 21 / 55 once sn reads 3727): a name-only unit, as claimed — the 13 trees on the unit's tip (the stamp's parent `3b8e0591`); `sphinx -E -W` clean + `dead_references` 0 / 66 on it.

**What the census taught (memo §9.7/§9.8):** a rename is one AST pass per SURFACE, each with its predicate — the class identifier (1660 + 411 docs), the import path in FIVE spellings (dotted, two slash forms, two package-relative depths, and a by-attribute `from pkg import module` that a dotted grep cannot return), the parameter/local/attribute/keyword/string roles (six counts that sum to the census), the field on a typed receiver (the resolver + 7 hand sites on un-annotated helper parameters), and a HOMONYM module name in another family (`orpheus.diffusion.augmented_mesh`, 40 sites, left alone by anchoring on the full path) — and, found only by the docs pass censusing by ANNOTATION, the slots that held the object under ANOTHER spelling (`mesh: SNProblem` ×30, the `from_*mesh*` verbs ×5; the 2026-09-13 lesson applied to the rename itself). Two more surprises the archivist caught: a +3-character class rename leaves RST section underlines short (two `-W` reds averted — a length-changing rename owes an underline scan), and the DATED-ledger exclusion must cover EVERY replace in a pass (the verb replace leaked into one history row; restored). `mcp__nexus__rename`'s dry-run over-counted 3× (6443 vs 2120) — the per-predicate census was executed.

**Next:** no further act is chartered on this campaign. The open backlog is GitHub (#460, #471–#475 from step 3; #418 the derivations vocabulary; #227 the d≥3 sweep). The memory topic file collapses to one line.


## 27.12 ⏸ COMPACTION POINT #13 (2026-09-18, written after the rename's merge; tree clean, everything pushed, no gate/build running, no agent holding the tree) — THE CONSUMERS CAMPAIGN IS COMPLETE; the next act is a HINDSIGHT SECOND PASS over 2026-09-17/18's landings, then nothing chartered

### 27.12.1 State `[M]`
`main` = `origin/main` = the commit carrying this section (its parent `c27373b9` = the rename's stamp); branches `refactor/consumers-step3`, `-u6`, `-u7`, `-rename-412` all ff-merged and deleted; #412 CLOSED (via the P4 trailer on push). Landed 2026-09-17: step 3 U1 `f8a838a7` · U2 (+U2d) `bcd9c83c` · U2e `b59e4a76` · U6 `71612439` · U7 `38cc6ec7` (§27.9, §27.10). Landed 2026-09-18: the rename's six commits `53e8d33d` · `763b1b9e` · `bb3efbcd` · `0c6978a4` · `3b8e0591` · `293e8614` + stamp `c27373b9` (§27.11; memo §9.8). Final 13-tree reading **11 845 / 21 / 55** (name-only unit: identical per tree to step 3's close). `sphinx -E -W` clean; `dead_references` 0 / 66; `nexus errors` 86 entries / 320 catchers / 0 uncaught. **pyright `tests/`: 1295 errors at `3f39a406` (pre-rename) → 1295 after — 0 new** (`[M]` throwaway worktree, same venv). Memory: the campaign is ONE ⏹ line in `MEMORY.md`; the topic file `project_cs_ladder_campaign2.md` carries the terminal line; `reference_sn_package_layout.md` carries the #412 block; `reference_test_execution_env.md` the escalated-instrument spelling.

### 27.12.2 The hindsight second pass — the user's mandate at compaction (*"look into the latest developments in hindsight and fix anything that deserves a second pass"*); each item with the measurement that settles it
Already taken at this point (cheap, docs/docstrings only, one build): **H2** 4 docs prose sites still spelling the strategy family's parameter `mesh` (`supports(mesh, …)` ×2 `index.rst`, `pose(mesh)`, `R.supports(mesh, closure)` `loss_representation.rst`) → `problem`; **H6** 2 SN docstrings "The augmented geometry — supplies both member spaces" (`coupled_system.py:254/:614`) → "The SN Problem"; **H10** `compileall` over `derivations/ examples/ tools/` clean. Left for the pass, in priority order:
1. **H1 `[R]` the user — the transport tier names the SN hub.** `orpheus/transport/fields/_bases.py` (`from_problem_and_L`, `zeros_for_problem_and_L`, `_space_for_problem` ×3) and `orpheus/transport/source_sinks/*` (`from_problem_laws`, `from_isotropic`, `prescribed_inflow`, `from_specs`) + `radial_characteristic_field.source_from_angular` take a parameter annotated `'SNProblem'` under `TYPE_CHECKING` — a cross-method (L2) tier spelled for one method's hub. P5 renamed the SPELLING honestly (they were `mesh: 'SNProblem'`); the hindsight question is whether the CONTRACT should be the cross-method one (#219's deferred `TransportMethod` Protocol) with the verbs named for it. `[M]` to take first: which types besides `SNProblem` are ever PASSED (a runtime census over the 13 trees with a type spy) — if only SNProblem, the annotation is honest and the fork is #219's, not a rename residue.
2. **H3 an elegance/QA round on the step-3 production surface** — none was dispatched for U2/U6 (the surgical posture: main agent writes, sub-agents review): `orpheus/numerics/outcome.py`, `numerics/gauge.py`, `sn/solution.py` (the derived accessors, `compare`, the state-on-domain law), `sn/solver.py`'s `_exit_certificate`/`_balance_evidence`/`_gauge_evidence`/`_rayleigh_gap_evidence`/`_StateProductionRate`/`_returned_state`/`_as_coupled`/`_package_solution`, `numerics/convergence.py`'s `trajectory`/`leaf_iterations`. Dispatch `elegance-enforcer` + `qa` (Phase 1 harsh, Phase 2 withdraw) with the memo §3 as the design of record; every finding either fixed in-session or filed. ⚠ The known soft spots to point them at: `_gauge_evidence`'s `NotApplicable` reasons (two strings for two states — is that a `Certified`? — the entry-ledger row at `test_every_entry_gauges_its_trace.py:523` reads `Measured < 1e-10` on a "gauge ran, found nothing" config and passed, so measure what the certificate actually reads there); `SolutionDiff.keff_abs: Evidence` folded into `within_tolerance`?; `history`-era prose in `solver.py` docstrings (`grep -n "IterationHistory\|history\b" orpheus/sn/solver.py`).
3. **H4 the U2 artefact re-baseline's provenance stamp** — the 10 `.npz` carry `generator_commit = bcd9c83c` (the U2 tree); `[M]` that is the tree whose values they hold. Nothing to fix; re-verify only if `test_dd_regression` starts reading the key.
4. **H5 `mcp__nexus__rename` over-counts 3×** (6443 vs a 2120 regex census for `SNMesh`): file as a nexus issue (repo `deOliveira-R/sphinxcontrib-nexus`) with the measurement — an instrument that reports a wider surface than it edits misleads the sizing of every rename.
5. **H7 `#476` (`:noindex:` un-resolves 1081 roles)** — a docs-architecture ruling for the user; the archivist's two-sided control is the acceptance test.
6. **H8 the plan-authoring surprise log grew 4 rows in two days** (2026-09-17 ×2, 2026-09-18 ×1, plus the 09-14 row) — all §2/§6b family, all "no new clause". Per the file's own ⚠ (b) *growing without distilling*: check whether the §6b spelling inventory (now 15+ spellings across rows) should be DISTILLED into one enumerated list in §6b's body, with the rows kept as measurements.
7. **H9 the memo `consumers_step3_design.md`** now carries §9.1–§9.8 (~250 lines of landing log): fold nothing (it is the campaign's record), but its §6.4 unit table still reads U4/U5 as separate units "landed inside U2" only in prose — a reader designing against §6.4 should hit the ⏹ banner first (it does, at the top of §9; §6.4 is above it — add a one-line ⚠ at §6.4's head pointing at the banner).

### 27.12.3 Process facts a fresh session needs that live ONLY here or in scratch
The gate drivers: `scratch/_consumers/step3/gates/tip/driver.sh` (13-tree pattern), `scratch/_consumers/rename412/gates/{tip/driver.sh,p*/fast.sh}`; the rename's scripts `scratch/_consumers/rename412/probes/p{1,2,3,5}_*.py` (re-runnable censuses; `--apply` guarded) and the step-3 receiver resolver `scratch/_consumers/step3/probes/rewrite_solution_reads.py` (`FAMILY=keff|history|mesh`); the escalated regression instrument's DOTTED category; `git grep -E '\b'` is a silent zero; the archivist holds `docs/` only and runs NO Sphinx build; ONE build at a time; `.claude/plans/harness_context_budget.md` is the user's untracked file — never stage it.
