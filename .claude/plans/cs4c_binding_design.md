...
**Next (per the memo §6.4/§9.4):** U6 (`IterationHistory` retires — DESIGNED, scripts dry-run, runbook in §9.4) → U7 (#465 minimal) → U8 (corpus close: the follow-up issues F7's three + the ledger's naming blind spot + the LD-tail view; the `nexus errors` reconciliation for ERR-086; memory) → then the rename #412 LAST.
