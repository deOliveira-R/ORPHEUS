...
> **STATUS (2026-06-07): PHASE 5 ✅ COMPLETE + PUSHED. 5a ✅ + #218 ✅ (@ `4c2aa7e`) · 5b FACE-WINDOW + full-field ORACLES ✅ (@ `60e6eeb`) · 5c MOMENT-OUTPUT ✅ + docs ✅ (@ `63719a2`, origin synced, 2 commits `c7be111`→`63719a2`).** ⏭ NEXT = USER's choice: Phase 6 (#22, N-D foundation) / consistent DSA (#2) / #200 (block-inverse Krylov precond). The orthogonal reds persist: #206 cylinder-matvec (CONFIRMED pre-existing this session — numerical hand-ref divergence in `test_unified_cylinder_matches_hand_reference`, untouched by 5c's 2-D-gated path), #212 `continuous_get` hang, #214 ny=1 Mesh2D.
>
> **5c MOMENT-OUTPUT (drop the full-angular windowed-SI per-sweep transient) — DONE, principled-equivalent, 3.06× peak win, elegance PASS:** 2 commits — `c7be111` the carve + the fuller-view-oracle equivalence gate · `63719a2` the docs + the `harmonic-moment-projection` label single-sourcing. The per-anti-diagonal output of `apply_windowed` branches angular-write vs moment `+=` (`moment_buf[:,:,:,ii,jj] += einsum("nlm,ngd,n->lmgd", Y_oct, psi_avg, w_oct)`); the frontier walk + cell kernel are UNTOUCHED (window≡full angular-mode oracle stays bit-identical). The mode threads as an OPTIONAL `moment_projection` (DI, mirroring `reflect=None`; 2-D-Cartesian-only — `transport_sweep` raises on a 1-D mesh) down `transport_sweep`→`_sweep_2d_wavefront`→`_sweep_2d_scheduled`→`sweep_octant_group`→`apply_windowed`; at the resolvent surface the modes are NAMED methods `InvertibleOperator.solve_moments` / `_GaussSeidelResolvent.solve_moments` (shared parameterized body, guards single-sourced). `_MomentWindowedResolvent.solve` COLLAPSED to forwarding `base.solve_moments(...)` — the post-sweep flat `MomentProjection.apply` LEFT production (retained as the fuller-view ORACLE, pinned by `test_2d_windowed_moments_in_sweep_equal_post_projection` within the 4-ULP de-risk bound). Dead `_moment_order`/`sn_mesh` fields pruned. Moment mode returns `(moment_buf, None)` — the scalar is `moment_buf[0,0]` (Y₀⁰=1), no redundant reduction.
>
> ⭐ **Measured: 3.06× peak-memory reduction** (S8/4g/24×24: 2.26 MB → 0.74 MB; the 1.47 MB full-angular per-sweep transient eliminated; win GROWS with angular order — eliminated transient ∝ N, moment tensor fixed at (L+1)(2L+1)). De-risk: max-relative drift in-sweep-vs-flat-projection **2.74e-16 (≤ 4·N·eps), 4 ULP** (diags `diag_p5c_moment_accumulation_derisk.py` + `diag_p5c_peakmem.py`, git-excluded; the PERMANENT promotion is the equivalence gate). **Gates ALL GREEN:** windowing L1 4✓ (incl. equivalence gate) · keff_2d eigenvalue 19✓ (matches baseline — k_inf/P1/SI≡Krylov/Jacobi≡G-S/refinement) · regression snapshot within SAFETY×conv_tol (drift 6920 ULP / 9.81e-13, ~10× headroom) · sweep core+cartesian (5b oracles) 430✓ · streaming+A2D-1 matvec pin 65✓ · slab 1-D 5✓ · tests/sn/solve/ 38✓. elegance-enforcer PASS-WITH-NITS → all 4 nits fixed (unify both sweep arms through `transport_sweep`, `(moment_buf, None)` return, docstring, test sources projection from `solver.S`). Sphinx clean (1 pre-existing paramref warning). The 5c theory section is `:ref:sn-angular-windowing-in-sweep-accumulation` (operator_algebra.rst), eq `:label: harmonic-moment-projection`.
>
> **5b FACE-WINDOW (interior cochain → rolling _MovingFrontier) — DONE, bit-identical, FASTER, elegance PASS:** 6 commits — `49ecaa7` extract storage-free DD cell kernel (`cell_kernel_batch`/`residual_kernel_batch`, Pattern 2: ONE cell-math source for both walks) · `f4a0bbe` mesh-time window metadata (`window_slots`,`window_edges`) on `SweepDependencyGraph` · `d414910` add `apply_windowed`/`residual_windowed` (initially fancy-index) + the permanent **`window≡full-field` oracle test** (`tests/sn/sweep/core/test_sweep_graph_window_equivalence.py`, foundation) · `397d0e1` the **`_MovingFrontier`** class (rolling 2-diagonal, ghost x-col, `seed_x`/`seed_y`/`incoming`/`emit` cochain API) + rewrite the walks to BASIC-SLICE (contiguous slots → zero-copy views) + rewire the 2-D sweep orchestrator (`sweep_octant_group`/`_sweep_2d_scheduled` — per-octant `boundary_flux.face_view[oct]` inflow-read + outflow-shed, drop WavefrontFlux + the per-octant `.copy()`) · `dbb185f` rewire the 2-D matvec (`_apply_2d_cartesian` → `residual_windowed`; A2D-1 hash refreshed) · `bf210ab` elegance doc-fixes.
>
> ⭐ **THE DESIGN-SPACE EXPLORATION WIN (user directive "fast AND expressive"):** the window's slots are CONTIGUOUS (`arange(i0,i1+1)`) → basic-slice VIEWS where the full-field grid-diagonal walk is forced into fancy copies. Measured: `transport_sweep` **~0.71–0.80× the full-field time (a SPEEDUP, not a memory-vs-time trade)** — 8×8 3.36→2.40ms, 20×20 8.87→6.92ms, 32×32 16.60→13.25ms (median, `-O`, same machine, `diag_storageb_baseline.py`). The `_MovingFrontier` abstraction is ~free (~1.00× vs inline). Interior backing `O(N·ng·nx)` not `O(N·ng·nx·ny)`; peak −29% NOW (32×32 1885 vs 2640 KiB). De-risk + exploration diagnostics: `diag_phase0_storageb_derisk.py` (5b bit-id + 5c principled-eq + shed Gate-d, all PASS), `diag_p5b_frontier_design_space.py` (A full-field / B fancy / C basic-slice / D abstraction timings + bit-id).
>
> **Verified BIT-IDENTICAL** (all `np.array_equal`/`nulp`, not principled-eq — the face window computes identical values in identical order, recycled slots): window≡full oracle + graph 109 · octant snapshots + fixed-source 2-D + 5a windowing 12 · 2-D tier (octant/matvec/face-view/discrete-ord-2d) 20 · matvec Gate-K + bc-extraction 42 · streaming-operator (incl. A2D-1 refresh) + native-matvec + decomposition 102 · slab matvec (untouched) 23 · **eigenvalue 2-D (SI≡Krylov≡k_inf + Gate-K + #218 Jacobi≡G-S) 19 (twice)**. elegance-enforcer PASS-WITH-NITS (nits 1-2 fixed in `bf210ab`; nit 3 = commit 7).
>
> ✅ **5c + commit 8 (docs) DONE @ `63719a2` — see the STATUS block above.** The bullets below are the historical seam/provenance record (the seam was followed; ONE refinement at build time — the mode threads through `transport_sweep` itself with a 2-D-only guard, NOT a bypass, per the elegance review folding the source-arg + docstring nits):
> • **5c (commit 6) — per-level moment accumulation, drop the full angular OUTPUT** (the last full-field per-sweep transient → the LINEAR peak win, ~3× total). PRINCIPLED-EQUIVALENT (5a-style reduction-order gate, NOT bit-id — the cross-octant `+=` reorders the ordinate sum; de-risk proved drift/scale 4.8e-16 ≤ depth·eps). **SURFACE MAPPED** → `.claude/agent-memory/explorer/phase5c_moment_output_surface_bf210ab.md` (READ FIRST). **It is a moment-emitting MODE/variant, NOT a contract change** (changing `transport_sweep`'s `(angular,scalar)` return breaks the eigenvalue one-shot recon `solver.py:1547` + the Krylov full path). **THE SEAM:** thread a `moment_buf (L+1,2L+1,ng,nx,ny)` + octant-sliced `Y_oct = M.Y[oct_idx] (N_oct,L+1,2L+1)` + weights DOWN through `transport_sweep`→`_sweep_2d_scheduled`(`sweep.py:908`,`angular_flux` alloc `:963`)→`sweep_octant_group`(`:770`)→`apply_windowed`(`sweep_graph.py:586`); replace the `angular_flux_octant[:,:,ii,jj]=psi_avg` write (`:639`) with `moment_buf[:,:,:,ii,jj] += einsum("nlm,ngd,n->lmgd", Y_oct, psi_avg, weights_oct)` (the EXACT mirror of the live scalar accumulation `:636-638`; collapses to it at L=0). Then `_MomentWindowedResolvent.solve` (`solver.py:363-385`) calls the moment-emitting variant + SKIPS the post-sweep `MomentProjection` (`:374-376`). One-shot `Solution.angular_flux` recons (eigenvalue `solver.py:1547`, fixed-source `:1974`) STAY full-angular (out of scope). **GATE:** the 5a principled-eq tests stay green within `SAFETY×conv_tol` — `test_2d_anisotropic_windowing` (docstring §6 ANTICIPATES this reorder), `test_dd_regression[2d_2g_p1_aniso_dd_8x4_het_si]` (`kind="iterative"`, headroom: 6923 ULP within 1e-11), `test_keff_2d` (`TestAnisotropicScatteringKeff`+`TestSIKrylov2DEquivalence`); KEEP `test_scattering_operator::TestAnisoMomentSourcePath` green (the moment CONSUMER bit-id, untouched by 5c the producer). ⚠ `apply_windowed` does NOT return per-level `psi_avg` — the accumulation must reach INTO it (thread the args down). ⚠ `_MomentWindowedResolvent` already builds its own `spherical_harmonics(L)` Y (works at L=0 where `S.Y is None`).
> • **Commit 7 — full-field ORACLES recovered (elegance nit 3 + user "recover relinquished fuller views") ✅ DONE (`60e6eeb`):** the optimization had relinquished the end-to-end full-field SWEEP + MATVEC orchestrator paths + orphaned the typed `WavefrontFlux` (zero prod consumers). Recovered as documented VERIFICATION ORACLES (NOT production): `orpheus.sn.sweep._sweep_2d_full_field` (full interior cochain as typed `WavefrontFlux`, whole-trace ι_*/ι*, `graph.apply`) + `StreamingOperator._apply_2d_cartesian_full_field` (`graph.residual`+`edge_view`). Both share the cell kernel ⟹ no math drift. NEW end-to-end test `tests/sn/sweep/cartesian_2d/test_2d_full_field_oracle.py` (foundation, 8✓): windowed `transport_sweep`≡`_sweep_2d_full_field` (angular+scalar+boundary trace) + windowed `_apply_2d_cartesian`≡full-field (bulk+boundary residual), reflective+vacuum, het, 2g+4g — STRONGER than the per-octant walk equivalence (pins the whole orchestrator). `WavefrontFlux` un-orphaned (genuine prod consumers `sweep.py:1134`/`operator.py:1790`); commit-4 docstring claim now TRUE. A2D-1 unchanged (separate method). 106+8✓.
> • **Scattering-audit (user "did we compress scattering from a fuller view?") — CHECKED, NO recovery needed:** commit `93807aa` (5a) retired `_PerLegendreOrderScattering` (per-ℓ summand) for the factored `R·Λ·M`. But that per-ℓ kernel used the SAME `MomentProjection`/`sig_s_legendre`/`HarmonicMomentReconstruction` (per-ℓ restricted) ⟹ PROCEDURALLY independent, NOT structurally (vs the face-window oracle which guards a RISKY optimization). `R·Λ·M` is the natural EXACT lossless Pℓ form. Structural verification INTACT + green: MMS `test_curvilinear_aniso_convergence` + `TestP0AlgebraicIdentities` (uniform-flux, n2n-doubling) + `TestAnisoMomentSourcePath` bit-id (67✓). The flux fuller-view (full angular) the SI iterate drops in 5a is already recovered via the one-shot `Solution.angular_flux` recon. So NOT analogous to the face-window — nothing load-bearing to recover.
> • **Commit 8 — docs (archivist) ✅ DONE (`63719a2`):** the 5c in-sweep moment-accumulation section extends `sn-angular-windowing` in `operator_algebra.rst` (the `harmonic-moment-projection` eq, the 5a-vs-5c reduction-order contrast, the fuller-view oracle, the 3.06× win, honest scope) + a `discrete_ordinates.rst` note; the `harmonic-moment-projection` label single-sourced to `operator_algebra.rst` (dropped the dormant duplicate in `projection.py`'s non-autodoc'd docstring). The 5b `_MovingFrontier` + full-field oracles + faster-AND-linear-peak honest scope were already documented in the 5b close-out.
> ✅ **PHASE 5 COMPLETE + PUSHED @ `63719a2`** (origin synced). recovery = read the WORKTREE (L22); the STATUS block above + the 2 commit messages (`c7be111`/`63719a2`) + `.claude/agent-memory/test-architect/phase5c_moment_output_verification_plan.md` + `phase5c_moment_output_surface_bf210ab.md` are the record. **⏭ NEXT-CONTEXT PICKUP = USER's choice (Phase 6 / DSA #2 / #200).**

> **[SUPERSEDED — pre-5b STATUS below, kept for the 5a/5c context]** 5a ✅ DONE + PUSHED (origin `refactor/field-role-typing` @ `4c2aa7e`) + #218 ✅ DONE.
> Branch `refactor/field-role-typing`; 6 commits `93807aa`→`46c7336` on top of `bedc394`:
> `93807aa` scattering `R·Λ` moment→source primitive + `HarmonicMomentField`-consuming `S.apply` arm + RETIRE the per-ℓ `_PerLegendreOrderScattering` kernel · `b97d4f9` eigenvalue SI windowing (`_MomentWindowedResolvent`) · `13ca001` fixed-source SI windowing + one-shot full-angular reconstruction · `c8f3d9d` elegance fixes (`_maybe_window`/`_windowed_cold_start`/`_assemble_per_ordinate_source` + typed `scalar_flux()`) · `d19e9c2` Sphinx (`:ref:sn-angular-windowing`) · `46c7336` `_within_group_si` builder (R1→R2 depth: the SI sibling of `_within_group_krylov`; both SI sites symmetric with the 2 Krylov sites; behavior-preserving) · `4c2aa7e` `inner_schedule` threaded to the eigenvalue inner (#218 — boundary-G-S now REACHABLE on the eigenvalue path; opt-in, default Jacobi).  **7 commits `93807aa`→`4c2aa7e`.**
>
> **What 5a is (as built):** the 2-D Cartesian SI iterate (held + warm-started across every eigenvalue outer step) is now a `HarmonicMomentField` `(L+1,2L+1,ng,nx,ny)` instead of a full `AngularFlux (N,ng,nx,ny)`; `S.apply` consumes moments directly (the `R·Λ` map shared with `build_aniso_source` via `_aniso_source_from_moment_values`). Gated `sn_mesh.reduced is None` (Cartesian only — curvilinear keeps full-angular for the M-M Carlson seed, L21; Krylov stays full). `_MomentWindowedResolvent` wraps the base resolvent (Jacobi `L+C` for eigenvalue, G-S for fixed-source). Fixed-source adds a one-shot full reconstruction for `Solution.angular_flux`; eigenvalue rebuilds from the scalar flux.
>
> ⚠ **HONEST SCOPE (measured, do NOT oversell):** the win is the PERSISTENT iterate (18.3× smaller at N=110/L=1 — `diag_p5a_peak_memory.py`) + the typed moment-state. **Absolute PEAK memory only ~1.2×** — the per-sweep transient full-angular machinery (the swept output + the WavefrontFlux `psi_x`/`psi_y` + octant `.copy()` buffers — several full-angular arrays storage-A still materializes) DOMINATES the peak. **That transient is exactly 5b's target.** So 5a alone is a persistent-state + foundation win; **5b is REQUIRED to realize the full peak win.**
>
> ⚠ **NOT bit-identical — principled-equivalence (DECIDED + documented):** `S.apply(Mψ)==S.apply(ψ)` is bit-exact (de-risk `diag_p5a_moment_consuming_scatter.py`, Q1–Q4 + L11 hand-recon), but the SI convergence test necessarily moves from full-angular-L2 to moment-L2 → the 2-D iterative converged values drift within `SAFETY×conv_tol` (measured k_eff 2.4e-14, scalar_flux 6.3e-12 rel — the regression framework's `kind="iterative"` gate accepts it; Layer-2 DriftWarnings document it). G3-strict (`-W error::DriftWarning`) is the WRONG gate for these iterative cases (do not apply it to them). 1-D/sphere/cyl byte-unchanged.
>
> **Verified:** G1 eigenvalue SI≡Krylov≡k_inf 4✓ (twice), G7 anisotropic 5✓, broad sweep (matvec Gate K + operator core + block_role + phase-c gates + full 2-D eigenvalue) 115✓, scattering 68 + windowing 10 + DD + fixed-source-equiv 85✓. Sphinx clean. elegance-enforcer PASS (all 3 nits fixed). New tests: `tests/sn/solve/test_2d_anisotropic_windowing.py` + `2d_2g_p1_aniso_dd_8x4_het_si` snapshot + the `TestAnisoMomentSourcePath` moment-arm bit-id guard.
>
> **R1↔R2 symmetry CLOSED + #218 DONE (`46c7336`+`4c2aa7e`):** the user flagged that R1 (SI) was collapsed only to the PRIMITIVE level while R2 (Krylov) got the shared `_within_group_krylov` BUILDER. Added `_within_group_si` (the SI sibling, `46c7336`); both SI sites route through it. Then `inner_schedule` threaded to `SNSolver`+`solve_sn` (`4c2aa7e`) → the eigenvalue inner CAN now use boundary-G-S. **#218 CLOSED** (`Closes #218` trailer — formal close on merge). ⚠ DECISION: G-S is **OPT-IN, default stays Jacobi** for the eigenvalue path — G-S-eigenvalue is VALIDATED correct (test_keff_2d 18✓ under a temporary G-S-default incl. SI≡Krylov≡k_inf; new permanent Mode-9 pin `test_eigenvalue_jacobi_gauss_seidel_equivalence` Jacobi≡G-S), but a schedule change shifts k_eff by ~inner_tol (4.1e-11, same FP) which the keff_tol-tight (1e-12) eigenvalue snapshots can't absorb + the warm-started eigenvalue benefit is modest → keeping Jacobi-default avoids a re-baseline and preserves the deliberate Phase-3 choice. **⏭ NEXT (post-compaction): Phase 5b — interior-face storage-B (the rolling moving-frontier window).** This is THE per-sweep PEAK-memory win 5a left on the table (5a shrank the persistent iterate 18.3× but peak stayed ~1.2× because the per-sweep transient full-angular machinery — the resolvent's swept output + the WavefrontFlux `psi_x`/`psi_y` interior cochain + the per-octant `.copy()` buffers — dominates). 5b swaps the `WavefrontFlux` backing from the full per-axis face-field to a rolling 2-diagonal window (`O(N·ng·nx·ny) → O(N·ng·(nx+ny))`) + is the 3-D enabler. **A 3-LAYER carve, NOT a backing swap** (see the §"Phase 5 — 5b" body below + `wavefront_flux_carve_substrate.md`): touches the `SweepCellSlice` GLOBAL-per-level face index protocol + the `DiamondDifference` gather/scatter + the `WavefrontFlux.face()` API + a "capture outflow boundary edges as the frontier sheds them" step. Lands INDEPENDENT of 5a; 1-D is a no-op. ⚠ De-risk diagnostic FIRST (prove the rolling window bit-identical-where-applicable); L16 wall-clock gate (the diagonal advance must stay vectorised over `n_diag` — NO reintroduced per-cell Python); preserve the §3a′ axis-parametric test (3-D readiness). Dispatch test-architect for the storage-B verification plan (per the archived foundation plan §8). ⚠ recovery = read the WORKTREE (L22); RE-CHARACTERISE at pickup (L24).
