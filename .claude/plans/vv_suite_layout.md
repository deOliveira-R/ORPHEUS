...
- **R6** (the user, 2026-09-23): a time is never a gate here, absolute or calibrated: the suite runs on a laptop whose load is not reproducible. Performance matters (a faster suite asserts correctness sooner) but correctness is the criterion; a slowdown from a correctness improvement must be visible and must never block publishing. *"If correctness is not the criterion, I can give you a wrong answer arbitrarily fast."* Landed: P-2, the 2 ms slab gate, the cumprod speed-up tripwire and the solver-component profile moved to `tests/performance/bench_sn_cost.py`; the always-skipped 421-group profile and the skip-only suite-time placeholder retired; exact cost checks (call counts, allocation bytes) stay gates.
