...
**Status**: **Phase 0 + Phase 1 + Phase 2 (DD-only) + Phase 3 + Phase 4 complete (Issues 1-7 + 8 + 9-DD + 10, 11, 12, 13 + 14, 15); Wave C-extension (LD/EC/Step) and Wave F (Phase 5) pending. ERR-026 PARTIAL closure shipped in Wave E Round 3 (BC-aware FD operator); full MMS closure awaits FD-outer-face DD extrapolation follow-up.**
...
|---|---|---|---|
| Follow-up | — | (new GH issue) | **FD-operator outer-face DD extrapolation** for O(h²) curvilinear MMS convergence. Replace the cell-center face-flux approximation at the outer boundary with DD diamond closure or ghost-cell extrapolation. Once landed, the 4 xfail-strict markers come off, the curvilinear `solve_sn_fixed_source` default flips to "krylov", and ERR-026 closes fully on both constant-source AND MMS evidence. |
