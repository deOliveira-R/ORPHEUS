...
**Status**: **Phase 0 + Phase 1 + Phase 2 (DD + amendments) + Phase 3 + Phase 4 complete + Wave H Phase A + Phase B (Issue #168 architecture) complete (Issues 1-7 + 8 + 9-DD + 9.5 + 9.6 + 10, 11, 12, 13 + 14, 15 + #168 Phase A/B); Wave H Phase C (spatial-closure alignment for full ERR-026 closure) PAUSED on F_N infrastructure prereqs (cylinder F_N + 2G F_N for sphere/cylinder both unimplemented; user decision 2026-05-10 to build F_N coverage first then resume Phase C); Wave C-extension (LD/EC/Step) and Wave F (Phase 5) pending. ERR-026 PARTIAL — architectural infrastructure (BoundaryFaceFlux + PoleAngularClosure Protocols) installed through Phase B; Phase B's empirical finding: canonical M-M angular closure regresses on flat ψ unless paired with WDD spatial closure (Phase C scope when resumed). Phase C plan saved at `.claude/plans/issue_168_phase_c.md` (architectural map + 13-gate verification plan + user decisions).**
...
|---|---|---|---|
| H Phase C | **PAUSED** — F_N prereqs | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) Phase C; gated on [#170](https://github.com/deOliveira-R/ORPHEUS/issues/170) (cylinder F_N) + [#171](https://github.com/deOliveira-R/ORPHEUS/issues/171) (2G F_N) | **Spatial-closure alignment**, blocked on F_N infrastructure prereqs. Plan saved at [.claude/plans/issue_168_phase_c.md](.claude/plans/issue_168_phase_c.md) including full architectural map (apply matvec face-flux sites, eq_map ordering, `apply_transpose`-built-by-dense-probing finding) and 13-gate verification plan. **Pause reason** (research dispatched 2026-05-10): the originally-planned F_N-method/Sood La13511 absolute-value cross-check **does not cover any of the 6 deleted curvilinear regression snapshots** because (a) F_N cylinder solver is a stub (`fn_method/cylinder/__init__.py` awaits Westfall–Metcalf 1973 / Sanchez–Ganapol 1983), (b) 2G F_N for sphere/cylinder is unimplemented (Siewert–Thomas 1986 machinery missing — see existing pytest stub at `tests/derivations/test_sood_registry_wide_bare_critical.py:185`), (c) the 6 snapshots use ORPHEUS A/B mixtures with reflective BC vs F_N's vacuum-BC bare-critical regime. **User decision (this session)**: build the F_N infrastructure first, then return to Phase C with proper Sood/F_N cross-check coverage. **User decision on Phase C sequencing** (when resumed): implement WDD spatial closure first; probe Gate 1.1 (per-ordinate flat-flux residual) empirically with `MorelMontryAngularSweep` as candidate default; flip default only if Gate 1.1 passes; otherwise ship Phase C as "spatial-closure aligned; angular default flip deferred to Phase D" with a follow-up issue. ERR-026 status remains **PARTIAL** through this pause. |
| F_N infra (prereq) | — | [#170](https://github.com/deOliveira-R/ORPHEUS/issues/170) (cylinder F_N) + [#171](https://github.com/deOliveira-R/ORPHEUS/issues/171) (2G F_N) | **F_N method coverage gap closure** — required before Phase C resumes. (a) Implement F_N cylinder solver (Westfall–Metcalf 1973 + Sanchez–Ganapol 1983) at `orpheus/derivations/continuous/fn_method/cylinder/`; ground via `Ua-1-0-CY_STUB`, `PUb-1-0-CY_STUB`, `UD2O-1-0-CY_STUB` from Sood La13511. (b) Implement 2G F_N for sphere/cylinder (Siewert–Thomas 1986) at `orpheus/derivations/continuous/fn_method/multi_group/`; ground via the 10 Sood `WIDE_SLICE_STUBS` 2G bare-critical cases. Both are `algebra-of-record` Branch 1 / Branch 2 deliverables — SymPy reduction + numpy production — naturally dispatched to `method-implementer` after a `literature-researcher` memo on each paper. |
