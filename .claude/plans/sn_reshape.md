...
**Status**: **Phase 0 + Phase 1 + Phase 2 (DD + amendments) + Phase 3 + Phase 4 complete + Wave H Phase A + Phase B (Issue #168 architecture) complete (Issues 1-7 + 8 + 9-DD + 9.5 + 9.6 + 10, 11, 12, 13 + 14, 15 + #168 Phase A/B); Wave H Phase C **RESUMED 2026-05-12** with revised architecture (sweep-frame matvec rewrite, ghost-cell-via-trace-law, retire `BoundaryFaceFlux` Protocol). Original 2026-05-10 pause-on-F_N-prereqs dissolved by two post-pause developments: (1) trajectory_resolvent cylinder MR Phase 1b coverage of 5 of the 6 deleted curvilinear snapshots at machine-precision-class precision (the 6th routes to k_∞ closed-form, shape-independent); (2) user-flagged architectural inconsistency between matvec's boundary closure and §16A.3 affine BC contract, resolved via sweep-frame reformulation. Wave C-extension (LD/EC/Step) and Wave F (Phase 5) pending. ERR-026 still PARTIAL through this resumption window; Phase C closure path defined. Phase C plan saved at `.claude/plans/issue_168_phase_c.md` (revised architectural map + 13-gate verification plan + sequencing). #170/#171 (F_N route) and #190 (Billiard MR routing) + #191 (systemic ordinate-iteration cleanup) tracked as separate non-blocking work.**
...
### Wave H — Issue #168 architecture (Phases A + B + C + D complete; ERR-026 narrowed-scope PARTIAL pending #195) (DONE)
...
|---|---|---|---|
| H Phase C | ✓ DONE 2026-05-12 — sweep-frame matvec landed; the Phase D default flip + flux-shape closure shipped on `9512459..7d1cdd8`. ERR-026 PARTIAL CLOSURE remains (narrowed scope — see Phase D row + Issue #195) | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) Phase C | **Sweep-frame matvec rewrite SHIPPED** on commits `eae6f05` (API helpers), `16e33d7` (gate skeleton), `3fd1302` (matvec rewrite + BoundaryFaceFlux retirement), `3a95e3b` (MMS + cross-check gates), `d445a8f` (Sphinx + ERR-026 closure narrative). **Architectural deliverables**: (a) `SNMesh.iter_cells_by_direction(direction_sign[, mu_level_idx])` + `EquationMap.unknowns_at_cell_for_mask(i, mask)` API helpers (11 foundation tests, bit-identity to `iter_cell_visits(ordinate_idx=n)`). (b) `transport_operator_matvec_{spherical,cylindrical}` rewritten as one sweep iteration semantically — WDD diamond `psi_face_out = 2·psi_cell - psi_face_in` cell-by-cell along DAG order; BC trace law applied ONCE at the boundary edge on WDD-propagated outflow face values (§16A.3 contract honoured); vectorised across ordinates via outgoing_mask/incoming_mask (no new `for n in range(quad.N)` loops per user constraint 1). Pole-face initial condition `psi_face_in(pole) = psi_cell[0]` (Lewis-Miller §4.5) preserves the per-ordinate flat-flux invariant. (c) `solution_to_angular_flux_spherical` simplified to return only `fi` (Phase A `(fi, boundary_face_flux)` companion array retired); inward-at-boundary cell-centre slots filled with reflected-partner cell-centre as analytical extension. (d) Phase A's `BoundaryFaceFlux` Protocol DELETED entirely — module + 21 tests + `SNMesh.boundary_face_flux` field + `boundary_face_flux_closure` matvec kwarg. (e) `PoleAngularClosure` Protocol STAYS unchanged. (f) `SNStreamingOperator.apply` dispatcher cleaned up. **Verification gates** (`tests/sn/test_phase_c_*.py`): Gate 1.1 (per-ordinate flat-flux residual) PASS for Legacy + BFF on sphere/cyl; MMS-sphere FAILS (empirical decision point — sphere's pole-face WDD initial condition interacts with canonical Hébert §3.9.4 recurrence to break per-ordinate balance; cylindrical levels happen to telescope cleanly). Gate 1.2/1.3/1.4/1.5 PASS structurally. Gate 2.x — 5 Cartesian snapshots bit-identical (rtol=1e-12 PASS); 5 of 6 curvilinear snapshots regenerated under Phase C operator; 6th (P1 aniso) awaits separate generation. Gate 3.x MMS marked xfail with ERR-026 PARTIAL CLOSURE rationale. Gate 4.1 (k_∞ recovery on 2G reflective sphere) PASS at rtol<5e-4. Gate 4.2 (trajectory_resolvent crosscheck) SKIP — placeholder for Phase D. **Empirical Gate 1.1 outcome → DEFAULT FLIP DEFERRED**: per user constraint 7, curvilinear default stays `LegacyTauSymmetricInterpolation`; flip to `MorelMontryAngularSweep` + `inner_solver="krylov"` postponed to Phase D pending pole-face spatial closure refinement. **ERR-026 stays at PARTIAL CLOSURE** — Phase C aligned the spatial-closure architecture (the Phase B load-bearing precondition); the angular default flip + pole-face spatial closure refinement are Phase D's scope. **4 xfail-strict markers STAY xfail** (the underlying flux-shape ERR-026 bug remains until Phase D). Sphinx -W clean. 5 atomic commits on `refactor/sn-operator-algebra`. Closeout memo at `.claude/agent-memory/method-implementer/issue_168_phase_c_closeout.md`. |
| H Phase F | ✓ DONE 2026-05-12 — sweep-path Carlson coupled-pole seed backport; pole-cell + outer-cell defects CLOSED; ERR-026 manifestation #6 CLOSED; new #7 OPEN (residual SI-vs-Krylov O(h) WDD asymmetry) | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) Phase F | **Phase F shipped on `refactor/sn-operator-algebra`** (commits TBD). **Headline**: Phase D backported `CarlsonInwardSweep` to the apply-matvec; the SI/sweep path's TWIN bug (`psi_angle = np.zeros(...)` at `sweep.py:474` + `sweep.py:634` cylindrical) was overlooked. Phase F closed it by factoring NEW free helper `carlson_inward_sweep_from_source(Q̄, σ_t, dr, bc_outer_value) -> (ng, nx)` at `orpheus/sn/spatial/psi_half_angle_seed.py` (lines 358-416); `CarlsonInwardSweep.__call__` refactored to delegate. Wired into BOTH sweep paths (spherical at line ~472, cylindrical per-μ-level at line ~632). **Empirical**: `sphere_2g_3reg` n=40 sf[0]/sf[1] was 0.522 DIVERGING to 0.473 at n=320 → now 0.778 STABLE; cv(ψ@i=0) 0.520 → 0.404; outer-reflective sf[-1]/sf[-2] 0.887 → 0.997 (essentially CLOSED); SI-vs-Krylov k_eff gap converges O(h) at 0.286%→0.065%. **Test additions**: Gate 1.6 (DUAL of Gate 1.1 for the sweep path) at `tests/sn/test_phase_c_gates.py`; NEW `tests/sn/spatial/test_sweep_vs_apply_consistency.py` (57 foundation tests) pinning apply-vs-sweep Carlson equivalence + linearity + SI/Krylov agreement. **Snapshots**: 6 curvilinear regression snapshots regenerated under the Phase F fix; all pass Gate 4.2 at Phase E's tightened rtols (sphere 2e-2, cylinder 3e-2). Total: 170 passed + 4 xpassed in 470s. **Phase E flux-shape sentinel** (`test_phase_e_trajectory_resolvent_flux_shape_crosscheck`) STILL xfail-strict; reason updated to "Phase F closed gross divergence; residual O(h) WDD spatial-closure asymmetry between SI and Krylov paths". **ERR-026 status**: manifestation #6 (heterogeneous eigenvector shape) CLOSED; NEW manifestation #7 OPEN (SI-vs-Krylov per-cell agreement, residual O(h) WDD asymmetry — follow-up issue to be filed; viable closures: (a) sweep WDD-closure refinement to bit-identity with Krylov, or (b) flip curvilinear `inner_solver` default in snapshot generator to Krylov). **Anti-pattern surfaced**: twin-path fix incompleteness — when fixing one of two structurally-mirrored production paths, MUST audit the sister; Mode 3 wrong-term-initialization defects often appear in pairs (ERR-026 instantiated this anti-pattern twice). Closeout memo `.claude/agent-memory/method-implementer/issue_168_phase_f_closeout.md`; diagnostic memo `.claude/agent-memory/numerics-investigator/phase_f_step3_diagnostic.md`; mesh-refinement memo `.claude/agent-memory/numerics-investigator/phase_f_step2_mesh_refinement.md`; plan `.claude/plans/issue_168_phase_f_curvilinear_boundary_eigenvector.md`. |
| H Phase E | ✓ DONE 2026-05-12 — composite per-region GL for Variant α MR solvers; Gate 4.2 rtol tightened 1e-1 → 2e-2/3e-2; flux-shape sentinel surfaced (xfail-strict pending Phase F) | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) Phase E | **Phase E shipped on commits `2d3e7f2`..`6708a4a`** (refactor/sn-operator-algebra, 2026-05-12). **Headline**: a documented prototype TODO in trajectory_resolvent's MR Variant α solvers (`solve_greens_function_sphere_mr`, `solve_greens_function_sphere_mr_fixed_source`, `solve_greens_function_cylinder_mr`) used single-domain GL on `(0, R)` with σ_t kinks at material boundaries — caused non-monotone k_eff convergence under refinement (7-9% gap at production quadrature). **Phase E fix**: NEW `_composite_per_region_gl(radii, n_r_total, min_per_region=2)` helper at `greens_function.py`; piecewise GL with proportional node allocation by region thickness. Wired into all three MR entry points. **Empirical**: heterogeneous MR k_eff gap dropped from 7-9% to **1.99e-4 (sphere) / 1.75e-2 (cylinder)** at production quadrature. Gate 4.2 rtol tightened 1e-1 → 2e-2 (sphere) / 3e-2 (cylinder). **Test ceiling re-calibrations** (composite-GL trade-off): `test_mr_interface_continuity_3region` (cylinder) 1e-2 → 5e-2; `test_garcia_case1_phi_matches_at_point[r=7.0]` added explicit r=R branch with 5% tolerance. Composite-GL places nodes strictly INSIDE regions; the per-region spline must EXTRAPOLATE to boundaries (post-Phase-E artifact is extrapolation-driven). **NEW flux-shape sentinel** at `tests/sn/test_phase_c_crosscheck.py::test_phase_e_trajectory_resolvent_flux_shape_crosscheck` — parametrised over heterogeneous MR snapshots, compares L∞-normalised SN scalar_flux vs Variant α interpolated to SN cell centres. **Surfaced unresolved finding**: SN ↔ Variant α agree on k_eff (2e-4) but disagree on the EIGENVECTOR SHAPE (65% sphere, 22% cylinder per-cell). Xfailed-strict pending Phase F investigation. **ERR-026 status update**: manifestation #4 (trajectory_resolvent MR quadrature instability) CLOSED; NEW manifestation #6 (heterogeneous eigenvector shape) OPEN pending Phase F. Closeout memo `.claude/agent-memory/method-implementer/issue_168_phase_e_closeout.md`. |
| H Phase D | ✓ DONE 2026-05-12 — Carlson coupled-pole half-angle seed shipped, ERR-026 PARTIAL CLOSURE retained (narrowed scope: identity + rate CLOSED, magnitude OPEN per [#195](https://github.com/deOliveira-R/ORPHEUS/issues/195)) | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) + [#192](https://github.com/deOliveira-R/ORPHEUS/issues/192) | **Phase D shipped on commits `9512459`..`7d1cdd8`** (refactor/sn-operator-algebra). **Architectural deliverables**: (a) NEW `PsiHalfAngleSeed` Protocol family at `orpheus/sn/spatial/psi_half_angle_seed.py` with two strategies — `ZeroSeed` (Phase B regression-safety ablation knob, bit-identical) + `CarlsonInwardSweep` (canonical Hébert §3.9.4 Eqs. 3.432-3.435 inward μ=−1 zero-weight sweep; returns per-cell half-angle profile `(ng, nx)`). (b) `MorelMontryAngularSweep` composes seed strategy via dataclass field (Option α — composition into the M-M closure, NOT sibling Protocol on SNMesh; the seed is M-M-specific). (c) Spherical + cylindrical matvecs build a `CarlsonSweepContext` bundling `(sigma_t, dr, mu_quad, weights, bc_outer_value)` and pass to `pole_angular_closure` via optional kwarg; Legacy + BFF ignore. (d) Default flips: `SNMesh.pole_angular_closure` `LegacyTauSymmetricInterpolation` → `MorelMontryAngularSweep`; curvilinear `solve_sn_fixed_source` `inner_solver` `"source_iteration"` → `"krylov"`. (e) Gate 1.5 strengthened from `apply(0)=0` probe to capture-and-compare BC apply input (asserts BOTH BC apply calls per matvec match independent references). (f) Gate 4.2 trajectory_resolvent crosscheck full implementation — 5 P0 snapshots vs bare `solve_greens_function_*` calls (homogeneous closed at rtol < 1e-9 = machine precision; heterogeneous closed at rtol ≤ 1e-1 MAGNITUDE-class, tighter precision deferred to Phase E follow-up [#196]). **Critical structural finding** (Step 2 numerics-investigator diagnostic memo): the Phase D plan's original Protocol injection point at `operator.py:734-740` `psi_face_in` was empirically a no-op on flat ψ; the load-bearing site is the M-M angular recurrence's hardcoded `psi_half_left = 0` at `pole_angular_closure.py:411`. **Empirical evidence**: Gate 1.1 sphere MMS xpass (per-ordinate residual collapses from 18.88 → 1.78e-15); SI+Carlson plateaus at ~0.1 vs Krylov+Carlson at orders [3.33, 2.46] — justifies the Krylov default flip. L1 convergence-magnitude xfail-strict markers (sphere ISO/ANISO + cyl ISO/ANISO) STAY xfail because absolute-magnitude check at nx≤160 still fails (pre-asymptotic transient — error ~0.11 at nx=80, needs nx ≥ 320-640 to hit 1e-3). **3 follow-up issues**: [#193](https://github.com/deOliveira-R/ORPHEUS/issues/193) BC-realizer level-locality invariant test; [#194](https://github.com/deOliveira-R/ORPHEUS/issues/194) `verifies('hebert-3-43X')` decorator wiring on L0 tests; [#195](https://github.com/deOliveira-R/ORPHEUS/issues/195) L1 magnitude pre-asymptotic investigation + marker removal trigger. Sphinx narrative expansion across `docs/theory/discrete_ordinates.rst` (~700 LOC) + `docs/theory/boundary_conditions.rst` (~95 LOC). 8 commits total + closeout memos. Step 3 closeout at `.claude/agent-memory/method-implementer/issue_168_phase_d_step3_closeout.md`; Step 4b closeout at `.claude/agent-memory/method-implementer/issue_168_phase_d_step4_closeout.md`. **Wave H Phase A+B+C+D campaign DECLARED COMPLETE** with ERR-026 narrowed-scope PARTIAL CLOSURE pending #195. |
| F_N route (no longer prereq) | — | [#170](https://github.com/deOliveira-R/ORPHEUS/issues/170) (cylinder F_N) + [#171](https://github.com/deOliveira-R/ORPHEUS/issues/171) (2G F_N) | **Future work for independent F_N-pillar V&V coverage.** No longer gating Phase C — trajectory_resolvent + k_∞ closed-form together cover the 6 deleted curvilinear snapshots. F_N (singular-eigenfunction Fredholm) remains valuable as a structurally-distinct cross-pillar reference for future cross-pillar agreement verifications (sphere bare-critical 2G, cylinder bare-critical 1G/2G). |
