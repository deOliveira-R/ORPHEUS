...
|---|---|---|---|
| H Phase C | **RESUMED 2026-05-12** — sweep-frame matvec rewrite | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) Phase C | **Sweep-frame matvec rewrite** with revised architecture. Plan saved at [.claude/plans/issue_168_phase_c.md](.claude/plans/issue_168_phase_c.md) (architectural map + 13-gate verification plan + implementation sequencing). The 2026-05-10 pause-on-F_N-prereqs was dissolved by two post-pause developments: (1) the **trajectory_resolvent cylinder MR Phase 1b** corpus (commits `37e3e29`, `cf662a6`, `604f380`, `e10c33c`) explicitly built to close the cylinder-2G ERR-026 gap; covers 5 of the 6 deleted curvilinear snapshots at machine-precision-class precision (V_α1 / V_α1_cyl identity at α=1 + MR↔MG inheritance rtol=1e-9); 6th (P1 aniso) routes to Gate 4.1 (k_∞ closed-form, shape-independent). (2) **Cross-domain-attacker analysis** identified a structural inconsistency: matvec's BC apply consumes cell-centre values (`operator.py:533`) but the §16A.3 affine BC contract `γ_-ψ = R·G·γ_+ψ + q` requires the boundary TRACE (face values). The Phase A `BoundaryFaceFlux.DDExtrapolation` (`boundary_face_flux.py:336`) does algebraic extrapolation that ignores the BC. Bug is silent for specular(α=1) only because reflection permutation commutes with cell-centre fills; surfaces for vacuum / albedo / prescribed inflow / white BCs. **User reframing (2026-05-12)**: ghost-cell idiom maps to typed `InflowTraceSpace` vector whose value is defined by the realised BC operator. Phase C revised scope: (a) matvec restructures as one sweep iteration semantically (vectorised across ordinates; new `SNMesh.iter_cells_by_direction(direction_sign[, mu_level_idx])` API surfaces what the cell-visit graph already encodes); (b) BC trace law owns the boundary edge, consuming WDD-propagated outflow face values (not cell-centres); (c) `BoundaryFaceFlux` Protocol RETIRES (Phase A patch subsumed); (d) `PoleAngularClosure` Protocol STAYS (pole = intrinsic geometry, not external BC); (e) `apply_transpose` via dense-probing remains free. Closes Defects 1+2+3 by construction. ERR-026 stays PARTIAL through this resumption window; Phase C closure path defined with empirical Gate 1.1 probe as default-flip decision point. Verification uses bare `solve_greens_function_*` / `*_mr` entry points (per user directive — Billiard facade MR routing tracked at [#190](https://github.com/deOliveira-R/ORPHEUS/issues/190) separately; no inline workarounds). Phase C does NOT propagate new `for n in range(quad.N)` loops (vectorises across ordinates); systemic cleanup of the 14 existing sites tracked at [#191](https://github.com/deOliveira-R/ORPHEUS/issues/191). |
