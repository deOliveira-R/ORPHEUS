...
**Status**: **Phase 0 + Phase 1 + Phase 2 (DD + amendments) + Phase 3 + Phase 4 complete (Issues 1-7 + 8 + 9-DD + 9.5 + 9.6 + 10, 11, 12, 13 + 14, 15); Wave C-extension (LD/EC/Step) and Wave F (Phase 5) pending. ERR-026 PARTIAL closure shipped in Wave E Round 3 (BC-aware FD operator); full MMS closure awaits FD-outer-face DD extrapolation follow-up.**
...
17. **Wave G B9 `volume_measure` wiring blocked by bit-identity contract**: attempting to migrate `CPSolver.compute_keff` from `np.sum(sig_p * flux * V[:, None])` to `mu(sig_p * flux).sum()` revealed a ~1 ULP drift in keff. The two paths are mathematically equivalent but have different IEEE-754 reduction orders: `np.sum(M * V[:, None])` flattens C-order and reduces left-to-right (group-axis innermost); `mu(M).sum()` first contracts the cell-axis via `einsum` (returning per-group rates), then sums groups. The 11/11 snapshot regression uses `np.array_equal` (bit-exact) on captured keff, so any 1-ULP delta breaks the contract. Empirical reproduction: Mesh1D Cartesian, N=10, ng=2 → `7.499...972e-2` (new) vs `7.500...111e-2` (old). Filed as GH [#169](https://github.com/deOliveira-R/ORPHEUS/issues/169) with two resolution paths (snapshot regeneration vs `integrate` ndarray-overload reduction-order tightening). The user's "wire affordances unless there's a strong reason not to" principle is honored — bit-identity contract IS the strong reason. The affordance still exists and is exercised by the new `test_mesh.py` `volume_measure` test suite.
...
|---|---|---|---|
| Follow-up | — | GH [#168](https://github.com/deOliveira-R/ORPHEUS/issues/168) | **FD-operator outer-face DD extrapolation** for O(h²) curvilinear MMS convergence. Replace the cell-center face-flux approximation at the outer boundary with DD diamond closure or ghost-cell extrapolation. Once landed, the 4 xfail-strict markers come off, the curvilinear `solve_sn_fixed_source` default flips to "krylov", and ERR-026 closes fully on both constant-source AND MMS evidence. |
| Follow-up | — | GH [#169](https://github.com/deOliveira-R/ORPHEUS/issues/169) | **`compute_keff` migration to `volume_measure`** for `CPSolver` and `SNSolver`. Blocked from Wave G by the bit-identity contract (~1 ULP keff drift between `np.sum(M * V[:, None])` and `mu(M).sum()` due to different IEEE-754 reduction orders). Resolution path is either snapshot regeneration or tightening `DiscreteMeasure.integrate`'s ndarray-overload reduction order. See Wave G operational note 17. |
