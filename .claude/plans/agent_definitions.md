...
- **R20** (the user, 2026-09-22): scratch is temporary space. A file tracked under `scratch/` is tracked for loss prevention while it waits for a promote-or-retire decision, not as a home; `scratch/literature/` stays git-ignored (copyright, and repository size). So every probe ends promoted into `tests/` or retired (git history keeps it), and a probe stays in scratch only while its investigation is open. The user did not rule on untracking the rest of `scratch/`. `[M]` 2026-09-22 (`scratch/_r19/census.py`): 23 tracked probes in `derivations/diagnostics/`; 32 in `scratch/derivations/diagnostics/`, all tracked (the 145 files first counted there included 113 `__pycache__` files); 151 tracked files under `scratch/`; citing files: 9 theory pages, 36 tests, 1 module, 1 skill, 8 scratch notes, about 75 agent-memory files.
...
## §9 The test DAG reaches the numerics-investigator (the user, 2026-09-22; ruled: register `rests_on` now and write the definitions)

**The instruction (the user, verbatim):** *"we should find a way to get the DAG produced by the test-architect to the numerics investigator and the numerics investigator should essentially recognize that it's running diagnostics because one of the failure possibilities is that a hidden boundary (which is some node or edge on the test DAG) has not being identified, and needs to surface. So it's job can either be to identify the reason why a node is not flipping, or emerge the hidden/unidentified node."*

**Measured gap.** The test-architect's DAG lives only in its spec, "at the path the brief names" (its Return section): a campaign file that a W2 dispatch weeks later has no pointer to. The `rests_on` marker that would put the edges in the tests is named but not registered (`pyproject.toml` markers list: `verifies`, `catches`, `cap`, `sentinel`, no `rests_on`; #358 step 1). The numerics-investigator's definition never mentions the ladder: its cascade rebuilds a reduction order from scratch each time.

**Proposal `[HYPOTHESIS]`.**
1. *Transport: the DAG lives in the tests.* Register `rests_on(*test_ids)` now (#358 step 1). The test-architect writes it on every test it reuses, improves or creates, so the spec's edges land with the code. Readers: an AST pass over `tests/` today; Nexus `runtime_markers` over a `--collect-only` manifest (it enumerates no marker names; whether it keeps the marker's arguments is unmeasured); Nexus-minted `rests_on` edges and `invalidated_by` later (#358). A W2 brief names the node that fails by pytest id, and the campaign spec's path when one exists.
2. *The investigator starts from a node.* "A node flips" = its reading changes as predicted (green after the fix, red under the defect). Two outcomes, and the report names which:
   - **Localised.** Walk down the node's `rests_on` edges. A red support means the defect is lower: move there (the lowest red rung bounds it). All supports green means the defect is in what this node adds over its supports, and the cascade isolates inside that increment.
   - **Hidden node or edge.** The DAG cannot explain the failure: every node green while the answer is wrong; the cause lies in a component no node covers; a red support that did not invalidate a node resting on it (a missing edge); a node that stays green under the defect (vacuous, qa's hindsight). The minimal reproducer IS the hidden node: the deliverable is its regime or invariant, the nodes it rests on, the nodes that should rest on it, and its first red. It goes to the test-architect as a new rung, which is exactly the promotion policy's PROMOTE.
   Diagnostics exist because the DAG was silent: where the DAG is complete, reading it localises a red with no probe.
3. *Where it lands:* the numerics-investigator definition (the opening, a new first step of §2, the Return verdict: LOCALISED / HIDDEN NODE / HIDDEN EDGE); the test-architect definition (write `rests_on` in the tests, and receive a hidden node with its edges); `vv-principles` ladder section (one sentence: a failure the DAG cannot localise is a missing node or edge); the `probe-cascade` skill (reduction is a walk down the DAG when one exists); `workflows.md` W2 (the brief carries the failing node's id and the spec).

**Precedent to reconcile.** `cap(name)` + `sentinel` already encode a coarser, capability-level DAG (`.claude/plans/archive/sn_test_taxonomy.md`); `rests_on` is test-level. Whether `cap` becomes derivable from `rests_on` is #358's question, not this step's.

## §8 step 5 landed (2026-09-22)

`[M]` The triage (two numerics-investigator dispatches, `scratch/_r19/verdicts_{A,B}.md`) partitioned the 55 probes: 47 retired, 5 promoted (test-architect, `scratch/_r19/promotions.md`), 3 left open in `scratch/derivations/diagnostics/` (#283 twice, #123). 29 of the 32 probes tracked in scratch had not imported since May or June 2026. The protocol test's F.4 worker and the #123 scanner call production `solve_peierls_1g(..., boundary="white_f4")` (bit-identical to the retired evaluator on a cheap case; the slow pinned-baseline gate 6 of 6 green, 25 min). The import-resolution gate now covers `derivations/` plus the TRACKED scratch probes, with one known member per root as its control; its first red was the 29 dead probes. The promotion policy gained "a LEFT probe must still import". The archivist repointed 47 citation sites (26 for R19, 21 already dead before it). Filed: #499 (dead rank-N Marshak builder), #500 (sphere P_esc near-surface convergence).
