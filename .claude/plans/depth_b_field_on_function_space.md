...
Option β3 (2026-05-28, D-H.1b architectural decision): D-H.1b ships TimedFullField consumer migration with **implicit-zero boundary** for non-full operators (`C`, `S`, `F` return `TimedFullField` with bulk computed and boundary forced to zero). The dimensional sin (operator outputs typed as `AngularFlux` regardless of physics) persists through D-H. The proper fix — **Wave O** (BulkOperator / FullOperator / BoundaryOperator type split + Source/Residual as TimedFullField-shaped types) — is filed as **GitHub Issue #208** and lands as a dedicated wave after D-H closes. Source for the architectural insight: while introducing TimedFullField, the user observed that Residual is naturally a full-field (bulk + boundary residuals), and Sources can have non-trivial boundary parts (essential for MMS with non-vacuum BCs). Both observations dualize the asymmetry surfaced in the operator algebra. See Issue #208 for the Wave O scope and acceptance criteria.
