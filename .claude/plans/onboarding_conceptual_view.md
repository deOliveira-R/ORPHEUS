...
Landed on the branch as `ddcf6b68` (S6, S7: the labels, the drift fixes, the docstrings), `78005db3` (S1: the page), `b0e87a1b` (S2–S5: the harness sources and the generated files, the harness page); the qa repairs and this record land in the commit that carries this line; the plan was opened at `0a6901c8`:
...
- **The qa recount (§5.3, the planted second reader)** `[M]` 2026-09-21, `scratch/_claude_md/qa_recount.md`: roles 130 of 130 resolve by `importlib` (positive control: 5 of 5 fabricated targets rejected); `:ref:` 75 of 75 and `:eq:` 28 of 28 exist with 0 kind mismatches (index: 911 ref + 939 eq labels over 145 files); the CLAUDE.md generated block faithful. Thirteen statement-level findings, all repaired in the working tree before the second build (the commit hash is added below): (1) the `NotImplementedError` count excluded `derivations/` without saying so (66 in 24 → stated with the exclusion; 112 in 35 with it); (2) "MoC neither" was false, `MoCResult` carries a record; (3) `carrier-grid-double-category` defines neither the system restriction nor the lift: the lift now cites `cs4c-ends-select-the-body`, the restriction is "owed"; (4) `R∘M` is a projector only up to the frame's scalar (`M R = c_V I`); (5) "a Galerkin frame promises `M* = R`" was refuted by the cited equation's own warning: it is `M* = R/W` on a diagonal Gram under the Parseval metric, and the page and CLAUDE.md now say "each other's adjoint up to one scalar"; (6) `M` carried two operators (the pencil's production member and the splitting's implicit part): the transport pencil is now written `(A, F)` and the splitting's `M` is named as such; the eigen resolvent `A⁻¹F` is stated as never formed; (7) "the lift is the Reynolds projector" cited sections that do not say so: now `manifold-lift`, `manifold-reynolds-projector`, `manifold-reynolds-projector-section`, and the word "lift" is disambiguated from `BulkLift`; (8) coarsening cited `spaces-collapse-pair` for homogenisation and condensation, which that section does not mention: now the two Petrov–Galerkin frame labels; (9) the `NotYet` list is stated as the outcome module's own list with the issue states (#354 open, #353 open, #310 closed); (10) "one outer loop for every family" → every iterative family, the 0-D baseline solves directly; (11) the three-hub parenthetical named the wrong population (the regex found the exception, not `DiffusionMesh`); (12) the page now says the gate for its roles is `dead_references`, since Sphinx renders an unresolved role silently for the 56 of 76 targets no autodoc page renders; (13) `discrete_gram` is an `:attr:`; the two-kinds sentence in the ontology section is a pointer, not a second statement. Agreed exact by qa: 67/48/8/11 operator subclasses; pencil 2/2/1; `ALPHA_MAP` unminted; guard 1; TODO 1; `def T(` 0; no diffusion adjoint; 0 of 24 transposes in a driver (qa notes the grep behind that count has two cancelling errors; its own AST agrees).

- **Gates on the repaired tree** `[M]` 2026-09-21: `sphinx -E -W --keep-going` build succeeded, 0 warnings (`scratch/_claude_md/sphinx2.log`, 77 s); `dead_references` 0 dead of 66 checked; `--check` 24 targets, 0 problems, 0 drifted, always-on ≈13 397 by the estimate; citations by ID 244 resolved, 0 dangling; 48 harness tests under `-O`; pyright 0 on the three edited production files. The probe was read before the repairs (15 571); the repairs moved CLAUDE.md by +212 chars (≈ +60 tokens by the estimate), below the fixture's one-draw resolution, so the reading stands as recorded.

Status: ⏹ COMPLETE once merged; the merge hash is in the git log (the note's terminal state is the merge, `process-discipline`). (`scratch/_claude_md/qa_recount.md`), `sphinx -E -W`, `dead_references`, the harness-page numbers, the commits and the merge.
