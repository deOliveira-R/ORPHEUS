<!-- GENERATED by tools/harness from docs/development/rules/workflows.md — DO NOT EDIT. Edit the source page; `python -m tools.harness` (or any Sphinx build) rewrites this file. -->
...
2. **Depth 3 is the ceiling, 2 the norm**: the orchestrator dispatches a Key
   ...
   (measured to three layers: [the page's history](../../docs/development/workflows.md#history)).
3. **Continuity by name.** The test-architect that wrote a verification spec is
   ...
   [the page's history](../../docs/development/workflows.md#history)).
4. **Every brief carries** the workflow ID, the phase, the previous phase's
   ...
   [harness page](../../docs/development/harness.md#what-loads-and-what-it-costs)).
5. **Ask; never work around what was meant.** A question the brief does not
   ...
   [measured](../../docs/development/workflows.md#history)).
