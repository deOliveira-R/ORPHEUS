...
- **[Lessons — hot digest](lessons.md)** — ~3200 lines. One imperative rule per
...
- **[Lessons — cold archive](lessons_archive.md)** — ~10 200 lines, sections L1–L83,
...
- **Consumers campaign step 2 — the Problem builds its TERMINAL OBJECT (R-cc6: split `WithinGroupSystem` · the posed `F` forward · the pencil onto the hub)** — ⏹ PLAN + 2 ANCHOR FILES DELIVERED 2026-09-12, PRE-carve (`scratch/_consumers/test_architect_step2.md`, 1032 lines; **32 rows landed** — `tests/sn/architecture/test_step2_terminal_object_anchors.py` 14 + `tests/sn/operators/test_step2_posed_fission_anchors.py` 18 — `[M]` **29 passed / 3 xfailed / 7.83 s**, pyright 0/0; exit arithmetic closes exactly: architecture 162→174 p / 1→3 xf, operators 1327→1344 p / 5→6 xf; 10-arm battery, 9 red + 1 declared null that HELD + 1 that was REFUTED). ⭐⭐ FIVE brief refutations: the bit-identity is reachable only via the DERIVED energy binding (`array_equal` 200/200 vs the composite route's **0/200 at ≤1 nulp**); the record's field partition is FALSE on the seedless arm (`implicit_operator` is the bare `L+C`, `explicit_gains` the leaf triple); the §6c red-before ALREADY SHIPS as a strict xfail and the split DEMOTES it to a tautology (successor: `A = M−N` **per Strategy value**, in-class mutation `3.595068e+00`); the adjoint's "posed F" is THREE spellings; and H1's repair (`with_cross_sections`) is UNWRITABLE over today's generating data. ⛔ The splitting choice is TRACE-only (`bulk 0.0` / `trace 9.97e-01`) so every bulk functional is a non-catcher; the SN regression corpus pins NO full-solve `angular_flux`. 8 open rulings (O-1 which `F` face; O-5/O-6 the σ merge unit — blocking). → **`L83`**
- **Consumers campaign step 1 — a Problem's identity is the CONTENT of its generating data (#459, R-cc3/4/8/9)** — ⏹ PLAN + 2 ANCHOR FILES DELIVERED 2026-09-12, PRE-carve (`scratch/_consumers/test_architect_identity_step.md`, 651 lines; **57 rows landed** — `tests/data/test_mixture_identity_anchors.py` 20 + `tests/sn/mesh/test_problem_identity_anchors.py` 37 — `[M]` **19 passed / 38 xfailed / 2.93 s**, pyright 0/0; scope gates 239+23 and 319+15 green; 5-arm pre-carve battery, 3 attributable + 2 DECLARED NULLS with bites). ⭐⭐ TWO §6c red-befores MEASURED: `is_same_phase_space` reads **`True`** for two d=3 hubs with cells `(3,4,5)` vs `(2,2,2)` (vacuous on the SPATIAL half only), and `solve_sn(3)` vs `solve_sn_adjoint(3)` run at **different internal `L` (1 vs 3)** with `k` agreeing to 1.1e-11. ⛔⛔ THREE refutations: a "P0 vs P3" row is VACUOUS on the whole abstract library (`[M]` all 12 mixtures `len(SigS)=2` ⟹ every request ≥1 clamps to 1); a content hash costs **1022 µs/call** on 421-group data (1.60 µs at 2g — 640× blind); and a content `__hash__` makes `_GEOM_CACHE_INTERN` **ping-pong 2 → 6 builds**, invisible to both committed cache gates. 8 open rulings (O-2 `SNSolver`'s kwarg — 51 sites/31 files; O-3 the intern key; O-4 freeze `Mixture` — all blocking). → **`L82`**
