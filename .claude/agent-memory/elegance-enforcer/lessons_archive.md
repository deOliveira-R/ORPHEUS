...
**Sharpening 16 — a PARTIAL test-sweep is more insidious than forgetting tests entirely; a page-level `git mv` (not just a section move) makes the stale `:doc:` BROKEN, not merely page-qualifier-stale (task #10 V3, dissolve `theory/verification.rst` + `git mv methods/sn/verification.rst → verification/sn.rst`, 2026-07-23, 1 MUST-FIX).** The whole-chapter move + rump-dissolution certified LOSSLESS perfectly (every one of 668 rump lines + 4 donor Verification-h2 excisions char-diffed byte-identical modulo declared adaptations: `chapter`→`part`, `Verification`→`Test suite` retitles, 9-case-grid + Richardson h3→h2 promotions, `|times|`→`×`, include depth `../`→`../../`, 2 rel→abs role fixes; SN chapter = EXACTLY the 3 declared change classes in 4,680 ln; all 6 moved anchors + moc-mms-*/kinf-* labels defined once in the right home; all inbound rewires target real pages). The lone MUST-FIX was pure Sharpening-3 (L-004 test blast-radius) with two new teeth: (a) **PARTIAL sweep** — the author DID repoint the parallel `tests/gates/derivations/` triple AND 8 other SN-chapter inbounds incl. `structured_geometry.rst`'s identical `:doc:` — but missed all 6 refs in `tests/gates/sn/verification/`. A partial sweep LOOKS thorough (the diff shows test files being repointed) so a diff-scoped or "brief says tests swept" read is fooled; the tell was `test_mms_prescribed_inflow.py:43` citing the SAME `sn-mms-nonvacuum-*` labels that the repointed `test_sn_mms_nonvacuum_symbolic.py` cites — a direct twin pointing two different ways. RULE: never trust "tests were swept"; grep the bare moved path (`methods/sn/verification`) tree-wide and confirm ZERO, because the author's grep almost always covered `tests/<the-obvious-dir>/` and missed a parallel `tests/<other>/`. (b) **A page-level `git mv` upgrades the severity** vs a section move: the old page is DELETED, so a `:doc:`/theory/methods/sn/verification`` is BROKEN (dead docname), not the usual "page-qualifier stale but `:ref:` still resolves globally." Still silent under `-W` (test-`.py` docstrings aren't autodoc'd → the green build the brief cited is consistent with 4 broken `:doc:` roles) — L-015's "-W is blind to the test/​docstring blast radius" holds. Causation oracle (`test -f old-page` = DELETED + `git show HEAD:old-page` = EXISTED) nails it to THIS stage ⟹ MUST-FIX, bundle in the move commit (leaving it = the internally-inconsistent half-cleanup the repointed `tests/gates/derivations/` twin already contradicts). Also reusable: parked-verbatim scaffold pending a later authoring stage legitimately carries STALE framing (V3's principles/summary still list Richardson-as-live-reference for SN/MOC/diffusion, contradicting the accurate retirement account in the freshly-adapted reference_solutions.rst + diffusion.rst) — per L-013 S2/S3 hold only FRESH content to the current standard; the parked staleness is a CONCERN carry-forward to the authoring stage (V5/V6), NOT a move-stage MUST-FIX (the move's job was lossless parking, which it did).
