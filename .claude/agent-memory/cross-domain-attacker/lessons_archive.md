# Cross-Domain Attacker — Lessons ARCHIVE (cold)

This file is the pre-2026-09-21 `lessons.md`, moved unchanged. It is COLD: no
dispatch reads it whole. The hot digest is `lessons.md`; every entry there cites
the `L-0NN` id of the section here that holds its war story, so `→ L-013` means
"open §L-013 below". Nothing here is maintained: a correction goes in the digest
or in a `MEMORY.md` §3 memo, never in this file.

What each section holds, beyond the imperative the digest already carries:
the `file:line` measurements, the campaign ids (#168, #196, #208, #226, #240,
#257, #261, #326, #336, CS3, CS4a-c, D5, Q68, the consumers campaign), the
worked derivations, and the SKILL-PROMOTION STATUS notes as they stood at the
time (the live ledger is the digest's Part 3).

⚠ Campaign state here is a 2026-09-21 snapshot and was already stale in places
when it was frozen: re-ground every `file:line` against the worktree before
acting on it (`MEMORY.md` §2).
