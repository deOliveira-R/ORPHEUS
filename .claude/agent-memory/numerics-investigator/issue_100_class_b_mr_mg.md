...
**Diagnosis** (Probe-cascade B, C, D, E, F, G, H — `derivations/diagnostics/diag_class_b_rank_n_probe_*.py`, retired at `f36572c8` (R19); recover with `git show f36572c8^:<old path>`; the tables they printed are pinned by `tests/gates/derivations/test_peierls_rank_n_class_b_mr_mg.py`):
...
**Promotion suggestion**: probe G + H consolidated into `tests/gates/derivations/test_peierls_rank_n_normalization_consistency.py` as an XFAIL with detailed reproducer. Probe C is a clean MR-routing-invariance regression test (passes today within 1e-3 — pin it).
