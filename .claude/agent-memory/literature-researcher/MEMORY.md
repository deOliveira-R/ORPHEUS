...
- [dsa_primaries_full_extraction.md](dsa_primaries_full_extraction.md) — **#2 DSA memo DONE rev.2 (7 sources)** → `.claude/plans/dsa_literature_memo.md`: 5 primaries + **M4S §7** (step-2 lags surface-jump C ⟹ Fick collapse; ρ→0 thick; ≠ Larsen-LD; "partially consistent" = A&L's label, not the paper's) + **Warsa k-IRAM §8** (A=DH⁻¹F; both DSA postures; outers flat in δ); ⚠ NSE-147 TWIN trap (147(1) local ≠ 147(2) degraded-DSA ABSENT); Alcouffe sign slips; K_N table; Σw=1 vs A&L Σw=2; absent: WWM-147(2)/Yavuz-Larsen'88.
