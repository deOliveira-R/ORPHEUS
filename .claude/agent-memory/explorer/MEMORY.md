...
- [lessons.md](lessons.md) — the exploration lessons, L-001…L-038, each headed by
  its own one-line rule (L-039 2026-09-05: Nexus `callers` on METHODS ⟹ read the `unresolved` count; L-040 2026-09-07: a retirement's PROSE census = xrefs / PROMISSORY claims / CONVENTION statements — sort before counting; L-041 2026-09-08: an `is`-identity on an OPTIONAL constituent is vacuous on the `None` arm — probe identity predicates there and on a same-data rebuild; L-042 2026-09-12: dataclass introspection LIES about hashability — only `hash(x)`/`x == y` at runtime answer, and a 1-element array is a false green; L-043 2026-09-12: a kwarg's move-census is an AST RECEIVER split, and its clamp is a per-ENTRY fact; L-044 2026-09-12: a bit-identical verdict = ULP probe + the WALLS' PREDICATE, never the fixture list; spy on the CALLER's module binding, keyed on BUILDS; L-045 2026-09-13: a `Class.attr (file:line)` citation names a LINE not an OWNER — resolve the enclosing class body; a `\.method\(` grep counts docstrings and misses `def method` surrogates). The spine (blast-radius = graph+grep+constructors+doc-nodes;
...
- [Identity-step census — scattering_order + the three identities (#459)](identity_step_census_durable.md) — the order is clamped on ONE entry and PADDED on the adjoint entries (three spellings of the retained order); every generating datum with an array RAISES on `==`/`hash` today (Quadrature has no rule-name to fall back on); exactly ONE `is_same_phase_space` pin, and it FLIPS; `_GEOM_CACHE_INTERN` is the one hash-keyed SNMesh consumer; the closure-class row of R-cc3 is CONTESTED by two doc sites.
- [SN k-solve walls + build cadence (step 2, R-cc6)](sn_k_solve_walls_and_build_cadence.md) — NO bit-exact wall on the forward k-solve (every eigen fixture is a SAFETY×conv_tol band; `nulp` walls cover only `L+C`); `build_within_group_system` runs once per OUTER (spy 6/6/6, 3/3/3); the two `F` spellings differ by 2–5 ULP; the hub's 13 cached properties are all σ-free.
