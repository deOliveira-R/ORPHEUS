...
- [#236 readiness/scoping (post-#240, HEAD 3ac96b4)](issue_236_readiness_scoping.md) — done-vs-remaining for the (spatial⊗angular) product: issue body STALE (cell_update.py→scheme.py, CellUpdateBase→DiscretizationSchemeBase, LinearDiscontinuous EXISTS slab-only, _run_1d_sweep→_OneDimScanWalk); NO Angular/SpatialScheme enum; NO diffusion_limit_consistent property; NO space×angle separability probes (the $CLAUDE_JOB_DIR diag_sep_* never promoted); ST1 effectively done (2 separate registries), ST2/3/4/5 open; acceptance crit 1 MET, 2/3/4 NOT, 5 partial; #205≠home, #219 designs home (unbuilt), SpatialMomentSpace*SphericalHarmonicSpace is the built space-factor; #239/#237/#238 all PARALLEL (none block ST3). SUPERSEDES the stale Tier-2 entry below for #236.
- [Tier 2 space⊗angle live map (post-#206) — SUPERSEDED for #236](tier2_space_angle_live_map.md) — STALE: "DD-only, no LD" + loss_representation.py:2681 line refs no longer hold post-#240. Use the #236 readiness memo above. Kept only for the #233/#229/#235 verification-gate notes.
