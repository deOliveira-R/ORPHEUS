# Explorer — Lessons archive (cold)

The war stories behind the digest `lessons.md`, one section per lesson under the
SAME L-number the digest uses. This file is not read per dispatch: open one
section when a digest pointer needs checking. Append-only.

Provenance: the body below is `lessons.md` at `6e96b0b3` (2026-09-13), lines
22–1641, moved verbatim on 2026-09-21 with ONE edit: the second heading spelled
`L-034` (line 1411 of the source, the plan-vs-other-plans lesson) is renamed
`L-034b`, because two lessons had landed under one number from two parallel
dispatches and the digest needs distinct pointers. Verify the move with
`diff <(sed -n '22,1641p' <lessons.md at 6e96b0b3>) <(tail -n +17 lessons_archive.md)`:
the only difference is that heading. Entries appear in the order they were
written (L-006 after L-010, L-011 after L-012, …); the digest orders them
numerically.
...
## L-034b -- Reconciling a PLAN against HEAD includes reconciling it against the OTHER PLANS — and `git log -S <name>` discriminates a code-symbol premise from a paper-concept one
...
## L-045 -- A brief's `Class.attr (file:line)` citation names a LINE, not an OWNER — resolve the enclosing `class` body before repeating it; and a `\.method\(` grep counts DOCSTRING prose and misses duck-typed `def method` surrogates

**Rule.** When a memo/brief cites an instrument or member as `SomeClass.attr` with a `file:line`, do not inherit the class half: run the line through the file's class boundaries (`awk` over `^class ` / the AST parent chain) — `grep -n` returned the line, and the author attached the nearest famous class name. Separately, a method-call census by regex (`\.sweep\(` minus `def`) is TWO populations wrong: it counts prose lines that spell the call (docstrings: "``default_for(mesh).sweep(...)`` exactly once"), and it cannot see the duck-typed surrogate that DEFINES the method (`class _SpyStrategy: def sweep(...)`) which is a real §6b member under a signature change. The census is `ast.Call` for the calls + an `ast.FunctionDef(name=method)` scan outside the owning module for the surrogates + the Mode-11 `setattr(..., "method")`/`patch.object(Cls, "method")` spies.

**Why (2026-09-13, consumers step 2 C3 census).** The design memo said "`StreamingCoefficientCache._build_count` (ClassVar, `cache.py:441/:515`) instruments the geometry build" and hung an `[H]` on it. `[M]` `:441/:450/:456/:515` all sit inside `class CollisionCache` (`:398`); the geometry class (`:131`) has no counter at all — the geometry build count is measured by two monkeypatch spies on `from_mesh_and_quad` in two test files. Same memo: "18 test `.sweep(` sites / 7 files" — `[M]` 17 AST calls + 1 docstring line (`test_unified_sweep_dispatch.py:26`); the file's real member was `_SpyStrategy.sweep` at `:665`, reached through the operator seam, which the grep could never return. And the memo's "`_ensure_coll_cache` exists on the 1-D SCAN strategy" was the same defect one level up: the methods live on `_OneDimScanWalk`, a holder minted FRESH per `sweep` call by BOTH `CumprodScan` and `ScanMarch`'s 1-D branch — which is the structural reason the σ table is poked onto the mesh, and it changes what "bind σ on the strategy" has to thread.

**How to apply.** On any "instrument/member X on class Y" claim: one `awk 'NR>=a && NR<=b && /^class |^    def /'` over the cited range. On any method-consumer census: AST calls (state the receiver resolution), then a tree-wide `def <method>` scan minus the owning module for surrogates, then the `setattr`/`patch.object` string spellings — and report the three populations separately. When the census disagrees with the brief by ONE, the one is almost always a prose line or a surrogate, not a missed call.

## L-046 -- Graph DEGREE ranks connectivity, not canonicity: the most-connected primitive is the one most callers chose, and it may carry a convention the question must not inherit

Salvaged 2026-09-22 from the retired memo `phase5_mu_resolved_primitive_inventory.md` (vintage 2026-04-28, `f149c03f`), whose every other line was a `file:line` map of a module that no longer exists (`orpheus/derivations/peierls_geometry.py`). The source lines, verbatim (its "Risk #2"):
...
Still grounded at HEAD on 2026-09-22: `orpheus/derivations/continuous/peierls_nystrom/geometry.py` documents `compute_P_esc_mode`'s `(ρ_max/R)²` surface-to-observer Jacobian (line 3984) and names the "canonical no-Jacobian Mark-Lambert primitive" as a DIFFERENT helper (line 5639). `[M]` a grep for `most.connected|degree.*canonical|hub.*convention` over the digest, `AGENT.md`, the two Nexus skills and `.claude/rules/` found only the `god_nodes` tool descriptions: the caveat existed nowhere else.
