...
(`nexus-exploring`, `nexus-guide`); the standing directives are the
definition's sections; neither is restated here. L-numbers are stable
...
- **L-001** → the definition §4 (the four searches). Retained sharpening: two
...
- **L-003** → the definition's Return (durable shape leads, `file:line` is
  re-derive-via-Nexus). Home placement: durable shape into a topic
  file, proposed to the orchestrator when it belongs to the project; a transient line map into a topic file stamped
