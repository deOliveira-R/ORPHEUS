...
5. **NEVER** read "convergence rate is correct" as "result is correct" — verify the converged-TO value. **check:** is there an absolute reference, or only a slope? **tell:** an order table with no value column; O(h²) to the wrong limit is still O(h²). [case](../../../docs/development/evidence/vv-anti-patterns.md#ap5-rate-only)
6. **NEVER** trust a reference not traced to a structurally-independent analytical/symbolic ground — treat it as **reference contamination** until the trace is shown. **check:** walk the provenance to a closed form. **tell:** MC vs MC; CP vs unverified MC; method-of-images converged to the wrong BC. [case](../../../docs/development/evidence/vv-anti-patterns.md#ap6-untraced-reference)
