...
allowed-tools: Bash
---
<!-- GENERATED by tools/docs/generate_harness.py from docs/development/skills/vv-principles.md — DO NOT EDIT. Edit the source page; `python -m tools.docs.generate_harness` (or any Sphinx build) rewrites this file. -->
...
7. **NEVER** treat "two derivations agree" as proof — check *structural* independence. **check:** do both use the same upstream identity or integrand? **tell:** agreement at 1e-39 (ERR-032). [case](../../../docs/development/evidence/vv-anti-patterns.md#ap7-derivations-agree)
...
23. **NEVER** size an A-vs-B **invariance** gate ("bit-identical when knob K varies") by the breadth of what it computes — enumerate the production lines that READ K and prove the fixture makes each discriminating. **check:** grep the K-readers; the control must be K-dependent — neuter the knob so A and B become one object and require the *activation* leg to red; name the rows that cannot see K. **tell:** a catastrophic mutation leaves the gate fully green — reads as a dead harness (#17), is the wrong control. [case](../../../docs/development/evidence/vv-anti-patterns.md#ap23-invariance-sizing)
