...
35. **NEVER** report a derived COMPARISON quantity by its unit name alone when the name is overloaded — write its definition beside it: `Δk·10⁵`, `Δk/k₀·10⁵` and `Δρ·10⁵ = (1/k₀ − 1/k)·10⁵` differ by `k₀`. **check:** does the fixture set span a range of the normalising quantity? Then emit all three columns. **tell:** one `pcm` figure quoted across fixtures with different `k₀`. [case](../../../docs/development/evidence/vv-anti-patterns.md#ap35-overloaded-unit)
