...
class BoundaryFlux(Field):
    ...
    @classmethod
    def from_legacy_sn(
        cls, legacy_bf: object, mesh: "SNMesh",
    ) -> "BoundaryFlux":
        r"""Construct from a legacy :class:`orpheus.sn.boundary_flux.BoundaryFlux`.

        One-way adapter for D-H.1b consumer migration: reads the legacy
        bundle's per-face accessors (``bf.xmin``, ``bf.xmax``, ``bf.ymin``,
        ``bf.ymax``) and packs them into the L2 flat-layout form.

        The legacy bundle's geometry-conditional attributes (``xmin_face``
        / ``xmax_face`` for 1-D; ``xmin_xmax_buf`` / ``ymin_ymax_buf``
        for 2-D) are normalized via the legacy accessor properties — the
        boundary slot for each face (1-D: the standalone face buffer;
        2-D: the ``[..., 0, :]`` / ``[..., nx, :]`` slice of the
        conflated buffer).

        Faces missing from the legacy bundle (e.g. ``xmin`` on a
        curvilinear mesh where only ``xmax`` exists) are filled with
        zeros for the L2 flat layout's expected faces.

        **Transitional**: this method retires when D-H.2 deletes the
        legacy :class:`orpheus.sn.boundary_flux.BoundaryFlux`. Do not
        use in new code — construct directly via
        :meth:`zeros_for_sn_mesh` or :meth:`from_face_arrays`.

        Parameters
        ----------
        legacy_bf : object
            Legacy ``orpheus.sn.boundary_flux.BoundaryFlux`` instance.
            Duck-typed: must expose ``xmin``, ``xmax``, ``ymin``,
            ``ymax`` properties (any may be ``None``).
        mesh : SNMesh
            Phase-space carrier; provides :attr:`boundary_face_layout`.
        ...
        -------
        BoundaryFlux
            L2 pure-Field instance with the same face values as the
            legacy bundle.
        """
        layout = mesh.boundary_face_layout
        face_arrays: dict[str, NDArray] = {}
        for name, slot in layout.faces.items():
            # The legacy bundle's properties may return None for faces
            # the geometry doesn't carry — fall back to zeros.
            legacy_value = getattr(legacy_bf, name, None)
            if legacy_value is None:
                face_arrays[name] = np.zeros(slot.shape)
            else:
                arr = np.asarray(legacy_value)
                if arr.shape != slot.shape:
                    raise ValueError(
                        f"BoundaryFlux.from_legacy_sn: legacy face "
                        f"{name!r} shape {arr.shape!r} does not match "
                        f"L2 layout slot shape {slot.shape!r}"
                    )
                face_arrays[name] = arr.copy()
        return cls.from_face_arrays(mesh, face_arrays)
