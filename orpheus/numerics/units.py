...
# The four field dimensional-class signatures (built once, at import), each
# spelled as the registry parses the displayed unit. ``UREG.Unit(...)`` is
# typed :class:`pint.Unit` by pint's stubs on 0.25 and 0.26 alike, where the
# registry ARITHMETIC (``UREG.cm**-2 / UREG.s``) is typed as the facet base
# ``PlainUnit`` on 0.25 and as ``PlainUnit | PlainQuantity`` on 0.26 (`[M]`
# 2026-09-21, pyright 1.1.410: four errors on the CI runner's pint 0.26.1,
# none on 0.25.3). At runtime the two spellings build the same
# :class:`pint.Unit` (`[M]` both versions, all five signatures), so the
# runtime narrowing guard the arithmetic spelling needed (#258, ``_as_unit``)
# retired with it.
...
#: the trace).
ANGULAR_FLUX_UNITS: Unit = UREG.Unit("1/(cm**2 * s * sr)")
...
#: ANGULAR_RATE_UNITS``.
CROSS_SECTION_UNITS: Unit = UREG.Unit("1/cm")
