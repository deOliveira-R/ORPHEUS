...
infrastructure in :mod:`orpheus.derivations.continuous.peierls_nystrom.geometry`. This
module is a **registry-only façade** as of Issue #138 (2026-04-29):
it owns the
:func:`~orpheus.derivations.continuous.peierls_nystrom.cylinder._build_peierls_cylinder_case`
and
...
:data:`~orpheus.derivations.continuous.peierls_nystrom.cylinder.GEOMETRY` singleton
binding the canonical
:data:`~orpheus.derivations.continuous.peierls_nystrom.geometry.CYLINDER_1D`. The
...
:mod:`~orpheus.derivations.continuous.peierls_nystrom.geometry` and dispatches through
...
def _build_peierls_cylinder_case(
    ...
    return ContinuousReferenceSolution(
        name=reference_name("cylinder-1d", n_groups=ng, n_regions=n_regions),
        problem=ProblemSpec(
            materials=materials,
...
def _build_peierls_cylinder_hollow_f4_case(
    ...
    **1G residuals** vs :math:`k_\infty` at default quadrature and
    ...
    return ContinuousReferenceSolution(
        name=reference_name(
            "cylinder-1d", n_groups=ng, n_regions=n_regions,
            r0_over_R=r0_over_R,
        ),
        problem=ProblemSpec(
            materials=materials,
