...
# ═══════════════════════════════════════════════════════════════════════
# Phase G four-operator algebra — `StreamingOperator` (pure L = Ω·∇)
# and `CollisionOperator` (pure C = σ·) leaves.
# ═══════════════════════════════════════════════════════════════════════
#
...
#
#     L  →  StreamingOperator        (pure spatial streaming Ω·∇)
#     C  →  CollisionOperator        (pure collision σ·, diagonal in
#                                     position / group / direction)
#
...
# fusion of ``(L + C - S.foldable_part()).solve(q)`` through the within-
# group sweep is substep 3+4.b.ii.
#
# Vector layout (intentional alignment with :class:`SNStreamingOperator`)
# ----------------------------------------------------------------------
#
# Both leaves consume the **packed unknown vector** shape ``(n_unknowns,)``
# (group-major Fortran-order; see :class:`EquationMap` for the slot
# semantics). This is the SAME layout :class:`SNStreamingOperator.apply`
# consumes, which lets the load-bearing composition equivalence test
#
#     (StreamingOperator + CollisionOperator(σ_t)).apply(ψ)
#         ≈ SNStreamingOperator(σ_t).apply(ψ)        # rtol=1e-12
#
# run directly (mechanism criterion 3). The eventual sum
# ``A_wg = L + C - S.foldable_part()`` — where ``S.foldable_part().apply``
# is the ``(N, nx, ny, ng)``-shaped scattering source — does NOT call
# this packed-vector ``apply`` path; ``OperatorSum.solve``'s fusion hook
# (substep 3+4.b.ii) routes the within-group inversion through
# ``sweep_within_group_1d`` directly, bypassing per-leaf ``apply``.
#
# Boundary conditions
# -------------------
#
# ``StreamingOperator`` reads boundaries from the :class:`SNMesh`
# instance (``sn_mesh.bc_xmin`` / ``.bc_xmax`` / ``.bc_ymin`` / ``.bc_ymax``
# for Cartesian; ``sn_mesh.bc_right`` for spherical / cylindrical with the
# inner BC handled by reflection / pole closure). No ``boundary``
# constructor parameter — the mesh is the single source of truth.


@dataclass
class StreamingOperator(LinearOperatorMixin):
    r"""Pure streaming operator :math:`L = \Omega\cdot\nabla` as a
    ...
    streaming math only — collision is the separate
    :class:`CollisionOperator` leaf. The split lets the within-group
    operator be pure algebra (``L + C - S.foldable_part()``); no
    ``WithinGroupOperator`` wrapper class.

    Implementation
    --------------

    :meth:`apply` dispatches to the existing legacy primitives
    (:func:`transport_operator_matvec`,
    :func:`transport_operator_matvec_spherical`,
    :func:`transport_operator_matvec_cylindrical`) with ``Σ_t = 0`` — the
    matvec primitives compute ``Ω·∇ψ + Σ_t·ψ``, so with zero collision
    what remains is the streaming + angular-redistribution math. The
    cost is identical to a full :class:`SNStreamingOperator.apply`
    call; refactoring the legacy primitives to expose a streaming-only
    entry point is substep 3+4.c work, not this substep's concern.

    Curvilinear caveat (Carlson seed σ_t coupling)
    ----------------------------------------------

    For **sphere / cylinder**, the curvilinear matvec's Carlson
    coupled-pole seed (Hébert §3.9.4 Eqs. 3.434-3.435; see
    :func:`orpheus.sn.spatial.psi_half_angle_seed.carlson_inward_sweep_from_source`)
    couples streaming with σ_t through the seed equation's
    denominator. As a result the curvilinear ``apply`` with ``σ_t = 0``
    is NOT bit-equivalent to ``SNStreamingOperator(σ_t).apply -
    CollisionOperator(σ_t).apply``; the matvec is non-linear in σ_t
    (linear in the cell-collision term, NOT in the seed-coupled
    redistribution). The Cartesian matvec has no Carlson seed and
    composes cleanly.

    This caveat is diagnostic-only because the eventual algebra
    ``A_wg = L + C - S.foldable_part()`` calls ``solve`` (routed
    through ``sweep_within_group_1d`` consuming a removal cross-section
    ``σ_r``), not the leaves' individual ``apply`` paths. The
    within-group sweep is mathematically the same operation in
    either form. Substep 3+4.c retires the matvec primitive entirely;
    at that point this caveat disappears. The test gate
    ``tests/sn/test_streaming_operator.py::TestCompositionEquivalence``
    marks the curvilinear cases ``xfail(strict=False)`` with this
    rationale.
    ...
    -S_foldable).solve(q)`` will route to the within-group sweep via
    the fusion hook substep 3+4.b.ii adds. No ``apply_transpose`` yet —
    the analytic reverse-direction sweep is Step 6 work; advertising
    the capability now would require a harmful dense-probe stub.
    ...
    a single ``Σ_t`` constructor) remains in place; substep 3+4.c
    retires it after :class:`~orpheus.sn.solver.SNSolver` is wired to
    consume the leaf algebra.
    """
    ...
    # Cached zero-σ_t buffer matching the SNMesh shape, allocated on
    # first apply. Avoids per-call ``np.zeros`` allocation when consumers
    # iterate (Krylov inner loop).
    _zero_sigma_t: np.ndarray | None = field(default=None, init=False, repr=False)
    ...
    def _ensure_eq_map(self, ng: int) -> EquationMap:
        ...
        ----------
        ng : int
            Group count, inferred at the first :meth:`apply` call from
            the packed vector size or supplied by the caller. Stored
            on the cached :class:`EquationMap`'s ``n_unknowns`` field
    ...
    def _ensure_zero_sigma_t(self, ng: int) -> np.ndarray:
        """Lazily allocate a zero-σ_t buffer of the matvec-expected shape."""
        if self._zero_sigma_t is None or self._zero_sigma_t.shape[2] != ng:
            nx, ny = self.sn_mesh.nx, self.sn_mesh.ny
            self._zero_sigma_t = np.zeros((nx, ny, ng))
        return self._zero_sigma_t

    @property
    def n_unknowns(self) -> int:
        """Total packed scalar unknowns for the (1-group) eq_map.

        Multi-group consumers compute ``n_unknowns * ng`` themselves
        or call :meth:`apply` directly with a correctly-sized vector
        (the eq_map is shared across groups; ``n_unknowns = n_eq * ng``
        on the returned :class:`EquationMap`).
        """
        # Use ng=1 by default; consumers passing larger groups get the
        # correct n_eq · ng total via the cached eq_map's n_unknowns.
        return self._ensure_eq_map(ng=1).n_unknowns

    def apply(self, psi: np.ndarray) -> np.ndarray:
        r"""Forward action :math:`L\,\psi = \Omega\cdot\nabla\psi` on the
        packed solution vector.

        Dispatches by ``self.sn_mesh.curvature`` to the existing matvec
        primitives with ``Σ_t = 0`` — what remains IS pure streaming.
        ...
        """
        sn_mesh = self.sn_mesh
        # Infer ng from the packed vector size: n_eq is fixed by the
        # geometry's eq_map (built lazily); ng = len(psi) / n_eq.
        # First build the eq_map at ng=1 to determine n_eq.
        eq_map = self._ensure_eq_map(ng=1)
        ng = int(psi.size // eq_map.n_eq)
        # Re-materialise the eq_map at the correct ng if needed
        # (n_unknowns depends on ng).
        if eq_map.n_unknowns != psi.size:
            self._eq_map = None  # invalidate cached eq_map
            eq_map = self._ensure_eq_map(ng=ng)
        sigma_t = self._ensure_zero_sigma_t(ng)
...
class CollisionOperator(LinearOperatorMixin):
    ...
    ``σ_t`` AND the within-group removal cross-section
    ``σ_r = σ_t - Σ_{s,0}^{g\to g}``. The operator does NOT carry an
    interpretation flag — both quantities are
    ``(nx, ny, ng)`` arrays applied as :math:`\sigma\cdot\psi`. The
    fusion hook (substep 3+4.b.ii) will build a lazy ``σ_r``
    ...
    :class:`~orpheus.numerics.operator.OperatorSum`; the leaf doesn't
    care which σ it carries. (Pattern 4 — illegal states unrepresentable
    via the type, not via a runtime flag the operator never inspects.)

    Capability set
    --------------

    ``frozenset({CAP_APPLY, CAP_SOLVE, CAP_APPLY_TRANSPOSE})`` — collision
    is the simplest sort of operator. ``solve(q) = q / σ`` is
    element-wise division; ``apply_transpose = apply`` because the
    operator is self-adjoint (diagonal in every basis the SN method
    operates in). All three capabilities are advertised because all
    three are analytic.
    ...
    sigma : np.ndarray
        Per-cell per-group cross-section, shape ``(nx, ny, ng)``. May be
        σ_t (full collision) or σ_r (removal — within-group self-scatter
        folded). The operator's action is identical in either case.
    """
    ...
    def _sigma_at_unknowns(self, eq_map: EquationMap, ng: int) -> np.ndarray:
        ...
        side. The returned array is packed-vector-shaped
        ``(n_unknowns,)`` and can be multiplied element-wise with the
        input ``psi``.
        """
        # σ slice at each unknown's spatial cell: shape (n_eq, ng).
        sigma_per_eq = self.sigma[eq_map.ix, eq_map.iy, :]    # (n_eq, ng)
        # Layout matches solution_to_angular_flux's
        # ``psi.reshape(ng, n_eq, order='F')`` and the matvec's
        # ``lhs.ravel(order='F')`` — packed[k_ng + g] for the Fortran
        # ordering corresponds to (g, k) → broadcast σ across groups.
        # Concretely, the eq_map has n_eq slots; for each, ng group
        # cross-sections. The packed vector lays groups as the fast
        # axis (Fortran-order), so we need (ng, n_eq) then ravel('F').
        return sigma_per_eq.T.ravel(order='F')               # (n_unknowns,)

    def apply(self, psi: np.ndarray) -> np.ndarray:
        r"""Forward action :math:`C\,\psi = \sigma\cdot\psi` on the
        packed solution vector.

        Element-wise multiplication by the per-cell per-group cross
        section. No spatial or angular coupling — pure pointwise scaling.
        ...
        -------
        np.ndarray
            ``C·ψ`` as a packed vector, same shape as ``psi``.
        """
    ...
    def solve(self, q: np.ndarray) -> np.ndarray:
        ...
        constructing a removal cross-section :math:`\sigma_r = \sigma_t -
        \Sigma_{s,0}^{g\to g}` must guarantee positivity of the
        result; the operator does not check).
        """
        ng = int(self.sigma.shape[2])
        eq_map = self._ensure_eq_map(ng)
