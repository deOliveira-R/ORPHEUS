...
def test_vacuum_bc_returns_zero() -> None:
    """``VacuumBoundaryOperator.apply_to_incoming`` returns zeros exactly."""
...
def test_specular_bc_indexes_through_reflection_partner() -> None:
    """``SpecularBoundaryOperator.apply_to_incoming(psi)[n] == psi[reflection_index[n]]``."""
    quad = GaussLegendre1D.create(n_ordinates=8)
...
def test_albedo_bc_zero_is_vacuum_equivalent() -> None:
    ...
    np.testing.assert_array_equal(
        AlbedoBoundaryOperator(albedo=0.0).apply_to_incoming(psi_out, quad),
        VacuumBoundaryOperator().apply_to_incoming(psi_out, quad),
    )
