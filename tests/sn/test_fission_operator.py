...
class TestBitIdenticalExtraction:
    ...
    def test_apply_matches_legacy_compute_at_k_eq_1(self, solver_2g):
        ...
        # PR-INDEX-3: solver.sig_p / solver.chi are (ng, nx, ny);
        # phi is (nx, ny, ng).  Operator returns (nx, ny, ng).
        fission_rate = np.einsum("gxy,xyg->xy", solver_2g.sig_p, phi)
        expected = solver_2g.chi.transpose(1, 2, 0) * fission_rate[:, :, None]
...
class TestRank1EnergyStructure:
    ...
    def test_apply_linearity(self, solver_2g):
        ...
        nx, ny, ng = solver_2g.sn_mesh.nx, solver_2g.sn_mesh.ny, solver_2g.ng
        np.random.seed(13)
        phi1 = np.random.rand(nx, ny, ng) + 0.1
        phi2 = np.random.rand(nx, ny, ng) + 0.1
        alpha, beta = 2.5, -1.7
