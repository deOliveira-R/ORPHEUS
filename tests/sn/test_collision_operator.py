...
# ═══════════════════════════════════════════════════════════════════════
# Fixtures (same as test_streaming_operator.py)
# ═══════════════════════════════════════════════════════════════════════
...
def _packed_psi(sn_mesh: SNMesh, sigma: np.ndarray, seed: int = 42) -> np.ndarray:
...
def _sigma_total(sn_mesh: SNMesh, ng: int = 2) -> np.ndarray:
    """Random per-cell per-group cross-section, positive bounded away from 0."""
...
def _sigma_removal(sn_mesh: SNMesh, ng: int = 2) -> np.ndarray:
    """Synthetic σ_r — same shape as σ_t, smaller magnitude (within-group
    self-scatter folded out). The operator should handle this identically."""
    nx, ny = sn_mesh.nx, sn_mesh.ny
...
# ═══════════════════════════════════════════════════════════════════════
# 4. Composition apply ∘ solve = identity (mechanism criterion 6)
# ═══════════════════════════════════════════════════════════════════════
...
class TestSigmaInterpretation:
    """The same CollisionOperator works for σ_t AND for σ_r without API change.

    Pattern 4 (illegal states unrepresentable): the operator does not
    encode an interpretation of its σ — both are valid; the consumer
    ...
    def test_works_with_total_cross_section(self, name, builder):
        ...
        out = C.apply(psi)
        # σ_t > 0 everywhere → out non-trivial
    ...
    def test_works_with_removal_cross_section(self, name, builder):
        ...
        out = C.apply(psi)
        # σ_r > 0 everywhere → out non-trivial
        assert np.any(np.abs(out) > 0)
    ...
    def test_no_is_removal_kwarg(self, name, builder):
        ...
        Anti-recommendation 3 from the brief: the type does not encode
        an interpretation it doesn't act on.
        """
        sn_mesh = builder()
        sigma = _sigma_total(sn_mesh)
        with pytest.raises(TypeError):
            CollisionOperator(sn_mesh, sigma=sigma, is_removal=False)  # type: ignore[call-arg]


# ═══════════════════════════════════════════════════════════════════════
# 7. Sigma layout sanity — packed gather matches manual per-slot read
# ═══════════════════════════════════════════════════════════════════════


class TestSigmaLayout:
    """The packed-vector gather pattern σ[ix(k), iy(k), g] is correct.

    Cross-check the operator's apply via SNStreamingOperator(σ_t=0) +
    CollisionOperator(σ_t) against SNStreamingOperator(σ_t) — see
    test_streaming_operator.py for the full composition test. Here we
    check the simpler direct claim: when σ is non-zero in only ONE
    cell, the output is non-zero only at that cell's packed slots.
    """

    @pytest.mark.parametrize("name,builder", GEOMETRIES)
    def test_localised_sigma_localised_output(self, name, builder):
        """σ non-zero only at cell ix*, all other cells zero → out is
        zero at every unknown NOT at ix*."""
        sn_mesh = builder()
        ng = 2
        nx, ny = sn_mesh.nx, sn_mesh.ny
        ix_target = nx // 2
        ...
        C = CollisionOperator(sn_mesh, sigma)
        # Random ψ → output should be zero outside the target cell.
        sigma_ref = 0.5 * np.ones((nx, ny, ng))  # for sizing
        psi = _packed_psi(sn_mesh, sigma_ref, seed=33)
        out = C.apply(psi)
        # Use the eq_map to find which packed slots correspond to ix_target.
        ...
        # Reshape packed output as (ng, n_eq) Fortran-order.
        out_grid = out.reshape(ng, eq_map.n_eq, order='F')
        # Slots NOT at (ix_target, iy_target) MUST be zero.
        mask_target = (eq_map.ix == ix_target) & (eq_map.iy == iy_target)
        # Out must be 0 wherever mask_target is False
        np.testing.assert_array_equal(
            out_grid[:, ~mask_target], 0.0,
        )
        # And non-zero (matching psi) at the target.
