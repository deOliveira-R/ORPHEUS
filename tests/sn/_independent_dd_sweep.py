...
* :func:`solver_2g_case` — the 6 x 4 two-material (fuel A | moderator B,
  2 groups) Lebedev-17 case that ``tests/sn/operators/test_solver_components.py``
  hands to every test through its ``solver_2g`` fixture, and on which the
  frozen snapshot ``tests/sn/sweep_ref_2g.npy`` is taken;
...
``TestTransportSweep::test_matches_independent_hand_sweep``::
    ...
    .venv/bin/python -O -c "from tests.sn._independent_dd_sweep import regenerate_sweep_snapshot; regenerate_sweep_snapshot()"
...
def production_sweep(
    ...
    The same helper (:func:`tests.sn._test_helpers.sweep_once`) the
