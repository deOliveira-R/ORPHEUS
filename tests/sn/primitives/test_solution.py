...
class TestSolutionCompare:
    ...
    def test_compare_cross_mesh_rejected(self) -> None:
        """Two hubs over DIFFERENT quadratures realize different phase spaces,
        so the pairing guard (``same_phase_space``, P6 B2 — contractibility by
        CONTENT since 2026-09-12) rejects. (Until then this row built two
        ``_slab_mesh()`` calls — equal DATA in fresh objects — and expected
        refusal: the constituent tier's false negative, #459.)"""
        m1 = _slab_mesh()
        m2 = _quad8_mesh()
