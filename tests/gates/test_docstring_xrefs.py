...
class TestTheEmptyNamespacePackageIsNotAModule:
    ...
    def test_a_namespace_package_WITH_content_stays_alive(self) -> None:
        ...
        ``tests/gates/geometry/`` is a namespace package carrying its test modules.
        A rule keyed on ``__file__ is None`` alone would call it dead and
        redden every xref into the test tree.

        (The original exemplar was ``tests.gates.sn`` — regularized by ``627e64d6``,
        ...
        its job.)
        """
        import tests.gates.geometry

        assert tests.gates.geometry.__file__ is None, (
            "premise: tests.gates.geometry is a namespace package (no __init__.py). "
            "If an attribution fix regularized it too (as 627e64d6 did to "
            "tests.gates.sn), re-point this fixture at any surviving with-content "
            "namespace package, or synthesize one in tmp_path."
        )
        assert not _is_empty_namespace_package(tests.gates.geometry)
        assert resolve("tests.gates.geometry") == (True, None)
...
class TestProseKnowsWhichNamespaceItWasWrittenIn:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("tests/gates/test_docstring_xrefs.py", "tests.gates.test_docstring_xrefs"),
        ],
    )
...
def test_the_checker_can_import_tests_when_run_as_a_script() -> None:
    ...
    ``tests.gates.cross_method.protocol.CrossMethodCase``, …). With the repo root
