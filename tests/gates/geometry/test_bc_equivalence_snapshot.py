...
``tests/gates/geometry/snapshots/bc_equivalence_<case>.npz`` and is produced by
:mod:`tests.gates.geometry._generate_bc_equivalence_snapshots`.
...
::
    ...
    python -m tests.gates.geometry._generate_bc_equivalence_snapshots
...
from tests.gates.geometry._generate_bc_equivalence_snapshots import (
    BCEquivalenceCase,
    CASES,
    case_by_id,
)
...
_GENERATOR_HINT = (
    "Regenerate with `python -m tests.gates.geometry._generate_bc_equivalence_snapshots`. "
...
def _verified_space(case: BCEquivalenceCase, snapshot: np.lib.npyio.NpzFile):
    ...
    **Honest scope, MEASURED 2026-08-01.** This row is BLIND to the ``> 0.0``
    ...
    is covered by
    :class:`tests.gates.geometry.test_reemission_closure.TestDiffuseAgainstAnIndependentExpression`
...
class TestTheInversionIsStructural:
    ...
    def test_this_harness_imports_only_the_registry(self) -> None:
        generator_module = "tests.gates.geometry._generate_bc_equivalence_snapshots"
        pulled = {
            name.rsplit(".", 1)[-1]
...
class TestSpecularXLebedev17Snapshot:
    ...
    def test_albedo_with_specular_closure_folds_alpha_onto_the_same_image(
        ...
        output. (:class:`tests.gates.geometry.test_reemission_closure.TestEquivalenceTheorems`
