...
§F-2): under ν₂ₙ ``2 → 1`` **and** ``2 → 0``, ``tests/gates/diffusion`` read
...
``_fixture_materials()`` (``tests/gates/diffusion/test_operators.py``), which calls
...
def test_the_diffusion_loss_operator_carries_the_channel(case, reflective_mesh):
    ...
    `[M]` ``tests/gates/diffusion/test_operators.py``'s ``_loss()`` helper
