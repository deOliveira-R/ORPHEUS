...
from orpheus.derivations.continuous.peierls_nystrom.reference import (
...
from orpheus.derivations.continuous.peierls_nystrom import slab as peierls_slab
...
class TestSpherePescClosedForm:
    ...
    @pytest.mark.rests_on(
        "tests/gates/derivations/test_peierls_reference.py::TestSpherePescClosedForm::test_the_closed_form_is_the_defining_integral",
        "tests/gates/derivations/test_peierls_sphere_geometry.py::TestSphereGeometryConstants::test_prefactor_is_half",
        "tests/gates/derivations/test_peierls_sphere_geometry.py::TestSphereGeometryConstants::test_angular_weight_is_sin_theta",
        "tests/gates/derivations/test_peierls_sphere_geometry.py::TestSphereRhoMax",
        "tests/gates/derivations/test_peierls_sphere_geometry.py::TestSphereOpticalDepthAlongRay::test_homogeneous_1region_linear_in_rho",
        "tests/gates/derivations/test_peierls_assembly_drivers.py::test_per_observer_constant_integrand_recovers_angular_range",
    )
    ...
    @pytest.mark.rests_on(
        "tests/gates/derivations/test_peierls_reference.py::TestSpherePescClosedForm::test_interior_observers_match_the_E2_closed_form",
    )
    @pytest.mark.parametrize("R, sig_t", _SPHERE_PESC_CELLS)
...
class TestSlabRankNNotImplementedGuard:
    ...
    def test_P_esc_mode_raises_for_slab_polar_at_mode_n(self):
        from orpheus.derivations.continuous.peierls_nystrom.geometry import compute_P_esc_mode
        radii = np.array([1.0])
        sig_t = np.array([1.0])
    ...
    def test_G_bc_mode_raises_for_slab_polar_at_mode_n(self):
        from orpheus.derivations.continuous.peierls_nystrom.geometry import compute_G_bc_mode
        radii = np.array([1.0])
        sig_t = np.array([1.0])
        r_nodes, _, _ = composite_gl_r(radii, 1, 2, dps=20)
        with pytest.raises(NotImplementedError, match="slab-polar"):
