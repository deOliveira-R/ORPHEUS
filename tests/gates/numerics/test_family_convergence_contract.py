...
* ``tests/gates/numerics/test_power_iteration_record.py`` — the loop and the
  protocol in isolation, against a SCRIPTED solver;
* ``tests/gates/sn/solve/test_convergence_contract.py`` — SN end-to-end, including
...
# ``tests/gates/sn/solve/test_convergence_contract.py``, which owns the SN fixtures.
...
class TestTheAdviceIsFamilyAgnostic:
    ...
    ``tests/gates/sn/solve/test_convergence_contract.py``; the claim is family-WIDE,
