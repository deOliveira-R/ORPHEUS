...
* the REFERENCE anchor for ρ lives one directory over and is untouched by this
  file — :mod:`tests.gates.sn.solve.test_si_convergence_diagnostics` pins
...
# (:mod:`tests.gates.sn.solve.test_si_convergence_diagnostics`, Adams & Larsen
...
def test_the_fixture_is_in_the_c_to_one_regime(run) -> None:
    ...
    :mod:`tests.gates.sn.solve.test_si_convergence_diagnostics`.
    """
    got = _diagnostics(run)
    rho = got["ratios"][-1]
    if not (0.9 < rho < _C):
