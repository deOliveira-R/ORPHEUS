...
``tests/gates/sn/sweep/core/test_ordinate_scan_reset.py`` (it reds if the
...
* ``tests/gates/sn/sweep/core/test_ordinate_scan.py::test_ordinate_scan_small_attenuation``
  uses ``a ∈ [0.05, 0.2]`` (positive, bounded away from 0).
* ``tests/gates/sn/sweep/core/test_ordinate_scan.py::test_ordinate_scan_zero_attenuation``
...
def test_si_returns_finite_keff(homog_cyl_2g_thick2_n20):
    ...
    assert np.isfinite(res.outcome.keff), (
        ...
        f"tripwire live in "
        f"tests/gates/sn/sweep/core/test_ordinate_scan_reset.py; the "
