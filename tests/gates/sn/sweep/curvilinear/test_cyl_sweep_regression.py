...
Split from the legacy ``tests/gates/sn/test_cylindrical.py`` (SN taxonomy
...
claims moved to ``tests/gates/sn/eigenvalue/test_keff_curvilinear.py``; the CP
...
class TestCylindricalSweepRegression:
    ...
    # agreement on a cylinder — is unspellable once SNProblem(CYLINDRICAL)
    ...
    def test_requires_level_quadrature(self):
        """Cylindrical SNProblem with GL quadrature must raise ValueError."""
