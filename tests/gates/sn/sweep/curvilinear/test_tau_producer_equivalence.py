...
``tests/gates/test_layer_imports.py`` forbids from ``derivations/``).  Both
arms below use
...
``tests/gates/sn/sweep/test_angular_cell_partition.py``):
...
def test_cyl_tau_equals_the_ANALYTIC_closed_form_not_the_chord_convention(
    ...
    *procedural* implementation of the same edge recursion, i.e. only
    ...
    ``tests/gates/sn/sweep/test_angular_cell_partition.py``.  The tolerance is
