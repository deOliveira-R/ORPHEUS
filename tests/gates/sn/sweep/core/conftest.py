"""Capability marker for tests under ``tests/gates/sn/sweep/core/``.

The directory IS the capability (single source of truth). Every test
collected directly in this directory is stamped ``cap("sweep_core")``;
...
def pytest_collection_modifyitems(items):
    stamp_capability_marker(items, __file__, "sweep_core")
