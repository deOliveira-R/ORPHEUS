...
exercised here via ``tests.gates.sn._test_helpers.sweep_once``.)  The spy
...
``tests/gates/sn/regression/snapshots/`` (gate the bit-identical contract)
and the Wave C ``DiamondDifference`` hand-calc tests at
``tests/gates/sn/sweep/core/test_diamond.py`` (gate the per-cell algebra).
...
* L1 transport accuracy is verified transitively via the existing
  MMS suite at ``tests/gates/sn/l1_analytical/``.
"""
