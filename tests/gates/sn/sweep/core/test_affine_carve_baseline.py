...
* **Matvec leg:** ONE ``(L + C).apply(ψ).interior`` on a fixed-seed random
  ...
  — the canonical :func:`tests.gates.sn._test_helpers.het_operands` operands,
  which are tuned precisely so every loss-action term is activated.

Tolerance is the single-source-of-truth
:func:`tests.gates.sn.regression._regression_assert.assert_regression` with
...
:class:`~tests.gates.sn.regression._regression_assert.DriftWarning` tripwire
...
class TestAffineCarveMatvecBaseline:
    ...
    :func:`tests.gates.sn._test_helpers.het_operands` builds heterogeneous σ_t
