...
"""Absolute path to ``tests/gates/sn/`` — the anchor for shared test data."""
...
def volume_weighted_l2(
    ...
    byte-identical private copies that the ``tests/gates/sn/verification/mms/`` gates
...
def redistribution_via_live_path(
    ...
    * the geometry redistribution fold
    ...
    (``tests/gates/sn/sweep/curvilinear/test_angular_closure.py``) imports this
...
# ═══════════════════════════════════════════════════════════════════════
# Shared across tests/gates/sn/operators/ (the reciprocity gates) and
# tests/gates/sn/solve/ (the adjoint entry/certification batteries).  These
...
def _product_rule_with_ordering(tie_break: str, *, exact_nodes: bool):
    ...
    def build(n_mu: int, n_phi: int):
        ...
        measure = DiscreteMeasure(
            ...
            # (tests/gates/numerics/test_symmetry.py pins it); this field read the
