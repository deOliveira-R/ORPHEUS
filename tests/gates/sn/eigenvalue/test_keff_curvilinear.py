...
Split from the legacy ``tests/gates/sn/test_cylindrical.py`` /
...
regression guards moved to ``tests/gates/sn/sweep/curvilinear/``; the CP
cross-checks to ``tests/gates/sn/verification/analytical/``.
"""
...
def test_si_krylov_eigenvalue_equivalence_sphere():
    ...
    ``tests/gates/sn/solve/test_seed_threading_spy.py`` — value catcher here,
    path catcher there.
    """
...
# ``tests/gates/sn/architecture/test_monomorphic_leaves.py`` that DOES reach
...
#     (conftest → tests.gates.sn._test_helpers → orpheus.transport →
...
#       tests/gates/sn/primitives/test_quadrature_fold.py::TestFoldedHarmonics
