...
* 29 gates in ``tests/gates/sn/primitives/test_quadrature.py`` +
  ``tests/gates/sn/sweep/curvilinear/test_alpha_closed_form.py`` redden in
...
* ``redist_dAw`` / ``redist_dAw_per_level`` — **RETIRED 2026-08-26** as
  ...
  owned.  Its former catcher — ``tests/gates/sn/sweep/curvilinear/
...
----------------------------------------
This file was ``tests/gates/geometry/test_reduced_operator.py`` until the
...
``tests/gates/geometry/test_reduced_operator.py`` with :func:`~orpheus.sn.angular.redistribution.alpha_dome`,
...
class TestSNProblemBindsSphericalFactory:
    ...
    def test_face_areas_read_through_is_the_factory_value(self, pair):
        ...
        ``tests/gates/sn/primitives/test_snmesh_consumes_reduced.py``.
        """
    ...
    # ``tests/gates/sn/sweep/curvilinear/test_tau_producer_equivalence.py`` (closure
...
class TestStreamingTermsExtraction:
    ...
    def test_slab_streaming_terms_neutral_curvature(self):
        ...
        # a hand-written literal, at ``tests/gates/geometry/test_geometry.py``
