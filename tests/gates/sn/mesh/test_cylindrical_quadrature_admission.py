...
2. **structure-less** (``sn/mesh/reduced_operator.cylindrical_streaming``,
   ...
   ``tests/gates/sn/sweep/core/test_sweep_regression.py`` and
...
``tests/gates/sn/sweep/test_march_start_structure.py``; this module gates the
MESH's consumption of it.

Per the neighbouring ``tests/gates/sn/mesh/`` convention the whole module is
...
def test_the_structureless_guard_still_owns_the_slab_and_sphere_cubatures():
    ...
    it"): ``tests/gates/sn/sweep/core/test_sweep_regression.py`` and
    ``tests/gates/sn/sweep/curvilinear/test_cyl_sweep_regression.py``.
    """
