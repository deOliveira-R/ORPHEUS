...
:file:`tests/gates/sn/test_streaming_operator_decomposition.py`.  ``(L + C)``
...
from tests.gates.sn.regression._regression_assert import assert_regression
from orpheus.numerics.quadrature import Quadrature
from tests.gates.sn._test_helpers import placeholder_materials, radial_characteristic_edge_seed
...
class TestLinearity:
    ...
    def test_apply_is_linear(self, name, builder):
        ...
        # blend spelling could not — affine maps preserve affine
        # combinations; see test_declared_law_is_linear.py).
        c = 2.3
        ...
        out_combined = L.apply(state1 + state2)
        out_separate = L.apply(state1) + L.apply(state2)
        np.testing.assert_allclose(
...
PRE_T4_SNAPSHOTS_PATH = (
    "tests/gates/sn/_fixtures/wave_t_t4/pre_t4_snapshots.npz"
)
...
class TestT4bPreT4RegressionSnapshot:
    ...
    def _assert_cart2d_arm(
        ...
        """
        from tests.gates.sn._fixtures.wave_t_t4._capture_pre_t4_snapshots import (
