...
The sibling module :mod:`tests.gates.sn.operators.test_loss_kernel_gauge` verifies the
...
``tests/gates/derivations/_promotion_policy.md``; ``git log`` holds the investigation
record.
...
:mod:`tests.gates.sn.solve.test_every_entry_gauges_its_trace`, and it does not apply
...
from tests.gates.sn._singular_loss_box import (
    LS4,
    REFLECTIVE as _R,
    VACUUM as _V,
    absorber,
...
def singular_box():
    ...
    Shared through :func:`tests.gates.sn._singular_loss_box.diamond_singular_box`, so
...
def test_the_singularity_needs_TWO_closed_axis_pairs(label, bcs, expected):
    ...
    suite and ``tests/gates/sn/mesh/test_reflective_axis_pairs.py``: this row, plus
...
def test_a_kernel_mode_carries_metric_but_no_NET_CURRENT(singular_box):
    ...
    for index, (name, slot) in enumerate(
        ...
        assert net < 1e-11, (
            ...
            f"control in tests/gates/sn/solve/test_every_entry_gauges_its_trace.py "
...
def test_a_damping_closure_leaves_the_IDENTICAL_box_non_singular():
    ...
    **operator**, which is the form the theory page's remedy hierarchy actually
    ...
    :func:`tests.gates.sn.solve.test_boundary_gs_is_a_coherent_splitting.test_kernel_free_configs_give_the_SAME_TRACE_under_both_schedules`
    needs the identical object as its precondition. They share ONE assembly
    through :func:`tests.gates.sn._singular_loss_box.ld_reflective_box` rather than
