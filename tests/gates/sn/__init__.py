"""Marks ``tests/gates/sn`` as a package.

⛔ Not cosmetic, and not optional. Without it this directory is a NON-package
containing packages (`tests/gates/sn/operators/`, `tests/gates/sn/regression/`, … all carry
...
`tests.gates.sn.regression.test_dd_regression`.
...
**0** with `unknown_context` at 696 (`tests/gates/sn/operators`) and 373
(`tests/gates/sn/sweep`), against 0 unknown for `tests/gates/geometry` and
`tests/gates/homogeneous`. With this file the context reads
`tests.gates.sn.regression.test_dd_regression.test_dd_regression` and binds. The
