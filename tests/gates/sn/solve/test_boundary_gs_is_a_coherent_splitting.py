...
Adjacent surfaces: :mod:`tests.gates.sn.solve.test_every_entry_gauges_its_trace`
(that the gauge FIRES at every entry, and the physical witness it repairs) and
:mod:`tests.gates.sn.operators.test_loss_nullspace_reflective_box` (the structural
...
def _kernel_free(label):
    ...
    The LD row routes through :func:`~tests.gates.sn._singular_loss_box.ld_reflective_box`
...
def test_the_UNGAUGED_SI_driver_returns_a_splitting_dependent_trace(cells):
    ...
    ⛔ **Tier.** This runs the SI **driver** (:func:`tests.gates.sn._singular_loss_box.drive`),
    ...
    :mod:`tests.gates.sn.solve.test_every_entry_gauges_its_trace`
...
def test_the_PUBLIC_ENTRY_returns_the_SAME_trace_under_BOTH_schedules():
    ...
    asserted only indirectly — :mod:`tests.gates.sn.solve.test_every_entry_gauges_its_trace`
