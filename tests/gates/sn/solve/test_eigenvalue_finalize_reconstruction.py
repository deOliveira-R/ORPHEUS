...
gate attributes nothing.  Balanced (the ``tests/gates/cp``/``tests/gates/mc`` house
...
gates' helper ``tests/gates/sn/_test_helpers.py::reflect_outflow_into_inflow``)
...
* the fixed-source entries.  Their windowed reconstruction already builds
  ...
  ``fixed_point_step``; ``tests/gates/sn/solve/
...
def _balanced_n2n(mix: Mixture, p0: np.ndarray, l1_over_l0: float) -> Mixture:
    ...
    balance spelling is the one ``tests/gates/cp/test_verification.py`` and
    ``tests/gates/mc/test_gaps.py`` already use.
    """
    return replace(
        mix,
...
class TestTheReturnedTrace:
    ...
    ``tests/gates/sn/_test_helpers.py::reflect_outflow_into_inflow`` (what this
