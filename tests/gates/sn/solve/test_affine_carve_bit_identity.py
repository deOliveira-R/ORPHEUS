...
``tests/gates/sn/regression/test_dd_regression.py`` compares iterative results at
...
# ``tests/gates/sn/sweep/core/test_affine_carve_baseline.py`` uses) rather than
...
def test_converged_flux_matches_stored_reference(request, case: str) -> None:
    ...
    * a :class:`~tests.gates.sn.regression._regression_assert.DriftWarning`
