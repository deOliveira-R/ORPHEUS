...
2. **M-10 constrains only ``{L, S, F}``.** ``C`` and ``B`` are
   ...
   * ``B`` with a specular law is a signed **permutation** of the trace that
   ...
   ``tests/gates/sn/operators/test_g_adjoint_reciprocity.py``, which drops
...
``tests/gates/sn/architecture/_config.py`` is the campaign's shared config home and
...
``tests/gates/sn/operators/test_removal_form_matvec_sweep.py``'s ``_slab`` /
...
:func:`~tests.gates.sn._test_helpers.g_inner` IS reused (it is the designated
...
from tests.gates.transport._carrier_helpers import unit_cell_carrier
...
from tests.gates.sn.architecture._config import anisotropic_mixture
from tests.gates.sn._test_helpers import g_inner
...
def _fissile_anisotropic_2g(
    *,
    ...
    Delegates to :func:`~tests.gates.sn.architecture._config.anisotropic_mixture`
...
def _reciprocity_residual(
    ...
    The inner product is :func:`~tests.gates.sn._test_helpers.g_inner`, built from
...
def test_each_riesz_leg_is_individually_load_bearing(
    ...
    (and :func:`tests.gates.numerics.test_riesz_legs.
...
def test_reciprocity_row_is_non_vacuous(leaf, monkeypatch):
    ...
    ``tests/gates/sn/operators/test_g_adjoint_reciprocity.py`` drops
