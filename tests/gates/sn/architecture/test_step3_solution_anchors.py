...
``tests/gates/sn/architecture/test_step2_terminal_object_anchors.py`` and
...
# hypothesis.  Every ``tests/gates/sn/architecture/_config`` mesh is built from
...
class TestRecordTheFourProductionRateFunctionals:
    ...
    **1.2186e-01**).  Any step-3 gauge row written on the plain 2-G slab is a
    ...
    def test_record_a_SIGMA2_CARRYING_mixture_separates_them(self) -> None:
        ...
        from tests.gates.sn.solve.test_eigenvalue_finalize_reconstruction import (
            _LIBRARY_N2N,
...
class TestRecordNothingRecordsWhichGaugeApplied:
    ...
    ``tests/gates/numerics/test_gauge.py`` (U1).  This row records the SN half.
    """
