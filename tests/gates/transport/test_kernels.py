...
(:func:`tests.gates.sn.architecture._config.anisotropic_mixture`), because the
...
def test_kernel_equals_carrier_cache_bit_identical(build_mixture):
    ...
    is pinned by ``tests/gates/homogeneous`` — 17 reds incl. the L1 anchor —
...
def test_p0_and_emission_are_what_the_iso_pair_consumes():
    ...
    ``2.0`` in the reference rows of ``tests/gates/transport/test_material_field.py``,
    two hand-written homes). The
    absolute storage convention is pinned by G1.4b's hand-authored
    literal and by ``tests/gates/homogeneous`` (M1.5's two-tier separation
    from G1.3).
...
def test_no_operator_of_the_family_dispatches_on_the_carrier():
    ...
    ``tests/gates/transport/test_no_carrier_dispatch.py``.
    """
    from orpheus.transport.operators.lift import BulkLift
...
# The shipped 4g→2g fixture of tests/gates/data/test_mixture_condense.py,
...
def _xd9_fine_mixture():
    from tests.gates.data.test_mixture_condense import _balanced_fissile_4g
