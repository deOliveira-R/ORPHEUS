...
core; the value rows for that are ``tests/gates/transport/test_transfer_kernel.py``,
``tests/gates/transport/test_material_field.py`` and the tier-2 equivalence family.
...
def test_the_angular_lift_has_exactly_the_declared_subclasses():
    ...
    assert got == declared, (
        ...
        f"the base's laws a witness in tests/gates/transport/test_angular_lift.py); "
